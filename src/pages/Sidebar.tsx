...
import { BRAND_LOGO_PATH, BRAND_NAME, BRAND_TAGLINE, PRIVACY_POLICY_URL, TERMS_OF_SERVICE_URL, USER_DATA_DELETION_URL } from "../config/brand";
  ...
  return <>
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 flex h-dvh shrink-0 flex-col border-r border-slate-200/80 bg-white text-slate-800 shadow-sm transition-all duration-300 md:sticky md:top-0 md:z-auto md:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"} ${isCollapsed ? "md:w-20" : "md:w-64"}`}>
      <div className={`flex items-center border-b border-slate-100 ${isCollapsed ? "justify-center px-3 py-4" : "px-5 py-4"}`}>
        <div className={`flex min-w-0 items-center ${isCollapsed ? "justify-center" : "gap-3"}`}><img src={BRAND_LOGO_PATH} alt={BRAND_NAME} onClick={() => setActiveTab("TỔNG QUAN")} className="h-10 w-10 shrink-0 cursor-pointer rounded-xl border border-sky-100 object-cover shadow-sm" />{!isCollapsed && <div className="min-w-0"><h2 className="truncate text-base font-bold text-sky-700">{BRAND_NAME}</h2><p className="truncate font-mono text-[10px] uppercase tracking-widest text-slate-400">{BRAND_TAGLINE}</p></div>}</div>
      </div>
