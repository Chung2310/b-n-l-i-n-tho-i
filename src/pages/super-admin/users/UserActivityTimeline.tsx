...
export function UserActivityTimeline({ tenantId, userId }: Props) {
  ...
  React.useEffect(() => {
    ...
    setLoading(true);
    setError("");
    superAdminUserAccessService.activity(tenantId, userId, { from, to, category, page, limit })
    ...
    return () => { active = false; };
  }, [tenantId, userId, from, to, category, page]);
  ...
  return (
    <section className="rounded-xl border border-white/10 p-4">
      ...
      {!loading && !error && events.length === 0 ? <p className="py-6 text-center text-sm text-slate-400">Không có hoạt động trong khoảng thời gian này.</p> : null}
      <div className="mt-4 space-y-4">
        {Object.entries(grouped).map(([day, dayEvents]) => <section key={day}>
          ...
          <div className="space-y-2">{dayEvents.map((event) => <article key={event.eventId} className="flex gap-3 rounded-lg bg-slate-950/60 p-3 text-sm">
            ...
            <div className="min-w-0 flex-1"><p className="font-medium text-slate-200">{event.description}</p><p className="mt-1 text-xs text-slate-500">{new Date(event.occurredAt).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" })} · {event.actionType}</p></div>
          </article>)}</div>
        </section>)}
      </div>
