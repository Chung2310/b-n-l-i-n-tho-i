...
export const repairService = {
  ...
  async quote(id: string, amount: number, note?: string, laborFee?: number) { return (await apiFetch<Envelope<RepairTicket>>(`${root}/${id}/quote`, { method: "POST", body: JSON.stringify({ amount, ...(laborFee === undefined ? {} : { laborFee }), ...(note ? { note } : {}) }) })).data; },
...
export const repairExtras = {
  ...
  async loyaltySettings() { return (await apiFetch<Envelope<{ loyaltyDiscountRate: number }>>("/repair/settings/loyalty")).data; },
  async saveLoyaltySettings(input: { loyaltyDiscountRate: number }) { return (await apiFetch<Envelope<{ loyaltyDiscountRate: number }>>("/repair/settings/loyalty", { method: "PUT", body: JSON.stringify(input) })).data; },
};
