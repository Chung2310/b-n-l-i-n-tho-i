...
export type GoodsReceiptItem = { productId: string; variantId: string; barcode?: string; sku?: string; productName?: string; displayName?: string; quantity: number; unitCost: number; lineTotal?: number; note?: string; trackingMode?: "none" | "quantity" | "unit_barcode" | "lot" | "serial"; supplierWarrantyMonths?: number; serialNumbers?: string[]; unitDetails?: Array<{ internalBarcode: string; serialNumber?: string; imei1?: string; imei2?: string }> };
export type ReceiptFinanceTerms = { dueOn: string; paidAmount: number; paymentMethod?: string };
export type GoodsReceipt = { financeTerms?: ReceiptFinanceTerms; _id: string; receiptCode: string; supplierId: string; supplierName: string; warehouseId: string; status: "draft" | "pending" | "receiving" | "confirmed" | "cancelled"; receivedAt?: string; items: Array<GoodsReceiptItem & { sku: string; productName: string; lineTotal: number }>; subtotal: number; notes?: string; createdAt: string; createdByName?: string; createdBy?: string };
...
export const inventoryReceivingService = {
  ...
  async createReceipt(input: { financeTerms?: ReceiptFinanceTerms | null; supplierId: string; warehouseId?: string; receivedAt?: string; notes?: string; items: GoodsReceiptItem[] }) { const result = await apiFetch<Envelope<GoodsReceipt>>(`${root}/receiving/receipts`, { method: "POST", body: JSON.stringify(input) }); return result.data; },
  async updateReceipt(id: string, input: { financeTerms?: ReceiptFinanceTerms | null; supplierId: string; notes?: string; items: GoodsReceiptItem[] }) { const result = await apiFetch<Envelope<GoodsReceipt>>(`${root}/receiving/receipts/${id}`, { method: "PATCH", body: JSON.stringify(input) }); return result.data; },
  async submitReceipt(id: string) { const result = await apiFetch<Envelope<GoodsReceipt>>(`${root}/receiving/receipts/${id}/submit`, { method: "POST" }); return result.data; },
  async startReceiving(id: string) { const result = await apiFetch<Envelope<GoodsReceipt>>(`${root}/receiving/receipts/${id}/start-receiving`, { method: "POST" }); return result.data; },
  ...
  async updateBalanceThresholds(id: string, input: { minStock: number; maxStock?: number }) { const result = await apiFetch<Envelope<InventoryBalance>>(`${root}/warehouses/balances/${id}/thresholds`, { method: "PATCH", body: JSON.stringify(input) }); return result.data; },
};
