...
export default function CustomerList(props: Props) {
  return <div className="space-y-4">
    ...
    <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white">
      <table className="w-full min-w-[720px] text-left text-sm"><thead className="bg-slate-50 text-slate-600"><tr><th className="p-3">Mã khách</th><th className="p-3">Khách hàng</th><th className="p-3">Điện thoại</th><th className="p-3">Loại</th><th className="p-3">Trạng thái</th><th className="p-3 text-right">Thao tác</th></tr></thead>
      <tbody>{props.items.map((customer) => <tr key={customer._id} className="border-t border-slate-100"><td className="p-3 font-semibold text-cyan-700">{customer.customerCode}</td><td className="p-3"><p className="font-semibold">{customer.name}</p><p className="text-xs text-slate-500">{customer.email || "—"}</p></td><td className="p-3">{customer.phone}</td><td className="p-3">{customer.type === "vat" ? "Xuất VAT" : "Khách thường"}</td><td className="p-3">{customer.status === "active" ? "Đang hoạt động" : "Ngừng hoạt động"}</td><td className="p-3 text-right"><button type="button" aria-label={`Chi tiết ${customer.name}`} onClick={() => props.onOpen(customer)} className="font-semibold text-cyan-700">Chi tiết</button></td></tr>)}</tbody></table>
