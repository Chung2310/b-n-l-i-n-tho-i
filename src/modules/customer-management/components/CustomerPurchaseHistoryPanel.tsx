...
export default function CustomerPurchaseHistoryPanel({ customerId, companyCode, branchId }: Props) {
  ...
  return <section className="mt-6 space-y-4" aria-label="Lịch sử mua hàng">
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{cards.map(([label, value]) => <div key={label} className="rounded-xl border bg-white p-3 shadow-sm"><p className="text-xs text-slate-500">{label}</p><p className="mt-1 font-bold text-slate-800">{value}</p></div>)}</div>
    {history.items.length === 0 ? <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600">Khách hàng chưa có đơn mua tại chi nhánh này.</div> : <div className="space-y-3">{history.items.map((order) => <article key={order._id} className="rounded-xl border bg-white p-4 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-bold text-slate-800">{orderLabel(order)}</p><p className="mt-1 text-sm text-slate-500">{date(order.businessDate)} · {statusLabel(order.status)}</p></div><p className="font-bold text-cyan-700">{currency.format(order.grandTotal)}</p></div><div className="mt-3 grid gap-2 text-sm text-slate-600 sm:grid-cols-3"><p>Đã thanh toán: <span className="font-semibold text-slate-800">{currency.format(order.paidAmount)}</span></p><p>Công nợ: <span className="font-semibold text-slate-800">{currency.format(order.dueAmount)}</span></p><p>{order.itemCount} sản phẩm</p></div><p className="mt-2 text-sm text-slate-500">Nhân viên: {order.salespersonName || "—"}</p></article>)}</div>}
  </section>;
}
