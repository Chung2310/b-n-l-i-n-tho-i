...
import { FileText, User, Calendar, Clock, AlertTriangle, Warehouse } from 'lucide-react';
...
export function BookingModal({ bookingResource, onClose, onSuccess }: BookingModalProps) {
      ...
      <form onSubmit={handleBook} className="space-y-4">
        ...
        <div className={cn(
          ...
          <div className="space-y-1 text-left">
            <div className="flex flex-wrap items-center gap-2">
              <span className={cn("text-[9px] font-black uppercase tracking-widest", darkMode ? "text-slate-400" : "text-slate-550")}>
            ...
            <h4 className={cn("text-xs font-black", darkMode ? "text-slate-100" : "text-slate-800")}>
              {bookingResource.name}
            </h4>
            <p className={cn("text-[10px] font-bold text-slate-500")}>
              Khả năng đáp ứng: <span className="font-black text-slate-700">{bookingResource.capacity}</span>
            </p>
          </div>
          <div className={cn(
            "p-3 rounded-2xl flex items-center justify-center shrink-0 shadow-sm",
            darkMode ? "bg-slate-850 text-brand-primary/80" : "bg-white border border-slate-150 text-brand-primary"
          )}>
            ...
            <div className="relative group">
              ...
              <ErpInput
                ...
                className={cn(
                  ...
                  isDateInPast && "border-rose-300 focus:border-rose-500 focus:ring-rose-500/10 bg-rose-50/20"
                )}
              />
            </div>
          </ErpField>
        ...
        {isTimeInvalid && (
          <div className="flex items-start gap-2.5 p-3 rounded-xl border border-rose-100 bg-rose-50/80 text-[10px] font-bold text-rose-600 transition-all duration-300">
            ...
            <div className="space-y-0.5 text-left">
              <p className="font-extrabold uppercase tracking-wider text-[9px]">Lỗi chọn giờ</p>
              <p className="text-rose-550/90 font-medium">Giờ kết thúc phải sau giờ bắt đầu sử dụng.</p>
            </div>
          </div>
        )}
        ...
        {isDateInPast && (
          <div className="flex items-start gap-2.5 p-3 rounded-xl border border-rose-100 bg-rose-50/80 text-[10px] font-bold text-rose-600 transition-all duration-300">
            <AlertTriangle className="w-4 h-4 text-rose-500 shrink-0 mt-0.5" />
            <div className="space-y-0.5 text-left">
              <p className="font-extrabold uppercase tracking-wider text-[9px]">Lỗi chọn ngày</p>
              <p className="text-rose-550/90 font-medium">Không thể đăng ký đặt mượn trong quá khứ.</p>
            </div>
          </div>
        )}

        <ErpSubmitButton disabled={isSubmitting || isTimeInvalid || isDateInPast}>
          {isSubmitting
            ? 'Đang đặt lịch...'
