  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    stdFields.forEach((f) => {
      if (f.isVisible && !f.isArchived && f.isRequired) {
        if (f.key === 'name' && !formData.name.trim()) missingFields.push(f.label);
        if (f.key === 'phone' && !formData.phone.trim()) missingFields.push(f.label);
        if (f.key === 'email' && !formData.email.trim()) missingFields.push(f.label);
      }
    });
      ...
      <form onSubmit={handleSubmit} className="space-y-4 text-left grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
        ...
        {isFieldVisible('name') && (
          <div className="relative group/std">
            ...
            <ErpField label={getFieldLabel('name', 'Tên đối tác / Cộng tác viên')}>
              <ErpInput
                name="name"
                required={isFieldRequired('name', true)}
                value={formData.name}
                onChange={handleInputChange}
                placeholder={getFieldPlaceholder('name', 'Nhập tên đối tác...')}
              />
            </ErpField>
          </div>
        )}
