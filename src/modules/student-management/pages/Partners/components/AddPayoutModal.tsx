...
import { formatVND, getBankDisplayName, getVietQRBankCode } from '../../../lib/utils';
...
export function AddPayoutModal({
  ...
  const [method, setMethod] = useState<'Tiền mặt' | 'Chuyển khoản'>('Chuyển khoản');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        toast.success(`Đã chi trả thành công ${formatVND(String(cleanAmount))} cho ${partnerName}!`);
        onSuccess();
        onClose();
        // Reset form
        setAmount('');
      ...
      const msg = error instanceof Error ? error.message : 'Lỗi chi trả hoa hồng.';
      toast.error(msg);
    } finally {
  ...
  return (
    <ErpModal title={`Chi trả hoa hồng - ${partnerName}`} onClose={onClose} maxWidth="max-w-md">
      <form onSubmit={handleSubmit} className="space-y-4 text-left">
        <div className="p-4 bg-cyan-50/50 border border-cyan-100 rounded-2xl flex justify-between items-center text-xs font-bold text-slate-800">
          ...
          <span className="text-sm font-black text-cyan-600">{formatVND(String(unpaidBalance))}</span>
        </div>
        ...
        <ErpField label="Phương thức *">
          <ErpSelect value={method} onChange={(e) => setMethod(e.target.value as 'Tiền mặt' | 'Chuyển khoản')}>
        ...
        {method === 'Chuyển khoản' && (
          bankAccountNo && bankName ? (
            <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                  <QrCode className="w-3.5 h-3.5 text-cyan-600" /> Quét mã chuyển khoản hoa hồng (VietQR)
                </span>
              ...
              <div className="flex gap-4 items-center">
                ...
                <div className="text-xs space-y-1.5 select-all flex-1 min-w-0">
                  <p className="font-medium text-slate-500">Ngân hàng: <span className="text-slate-800 font-bold uppercase">{getBankDisplayName(bankName)}</span></p>
                  <p className="font-medium text-slate-500">Số tài khoản: <span className="text-slate-800 font-bold">{bankAccountNo}</span></p>
                  <p className="font-medium text-slate-500">Chủ tài khoản: <span className="text-slate-800 font-bold uppercase">{bankAccountName || 'N/A'}</span></p>
            ...
            <div className="p-4 bg-amber-50 border border-amber-100 rounded-2xl text-xs text-amber-700 font-bold">
              CTV này chưa cập nhật tài khoản ngân hàng. Hãy cập nhật trong thông tin CTV để dùng mã QR chuyển khoản hoa hồng.
            </div>
          )
        )}
