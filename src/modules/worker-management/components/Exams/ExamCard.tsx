...
const handleDownloadTemplate = (
  ...
  try {
    ...
    XLSX.writeFile(
      wb,
      `mau_ket_qua_thi_${exam.name.replace(/\s+/g, "_")}.xlsx`,
    );
  } catch (error) {
...
export const ExamCard: React.FC<ExamCardProps> = ({
  ...
  const [invalidPreviewList, setInvalidPreviewList] = useState<
    InvalidStudent[]
  >([]);
  const [rawResults, setRawResults] = useState<
    { phone: string; overallResult: "Đậu" | "Trượt" | "Chưa có" }[]
  >([]);
    ...
    reader.onload = async (evt) => {
      try {
        ...
        if (phoneIdx === -1 || resultIdx === -1) {
          ...
          alert(
            `Không tìm thấy các cột cần thiết ('Số điện thoại' và 'Kết quả thi') trong file Excel.\n\nCác cột tìm thấy trong file của bạn: ${foundHeaders || "Không có cột nào"}`,
          );
          return;
        }
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          let overallResult = row[resultIdx]
            ? String(row[resultIdx]).trim()
            : "Chưa có";
          ...
          if (rawResult === "đậu" || rawResult === "pass") {
            ...
            importErrors.push(
              `Dòng ${i + 1} (SĐT ${phone}): Kết quả thi "${overallResult}" không hợp lệ (chỉ chấp nhận: Đậu, Trượt, Chưa có).`,
            );
            continue;
          }
        ...
        if (importErrors.length > 0 && resultsList.length === 0) {
          alert(
            `File Excel không hợp lệ. Chi tiết các dòng lỗi:\n- ${importErrors.join("\n- ")}`,
          );
          return;
        }
        ...
        try {
          ...
          const res = await apiFetch<ImportPreviewResponse>(
            `/exams/${exam.id}/import-results`,
            {
              method: "POST",
          ...
          const msg =
            error instanceof Error
              ? error.message
              : "Lỗi kết nối khi tải dữ liệu lên.";
          toast.error(msg);
        } finally {
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      const res = await apiFetch<ConfirmImportResponse>(
        `/exams/${exam.id}/import-results`,
        {
          method: "POST",
      ...
      if (res.success) {
        toast.success(
          `Đã cập nhật kết quả: ${res.successCount} thành công, ${res.failedCount} thất bại.`,
        );
      ...
      const msg =
        error instanceof Error ? error.message : "Lỗi khi nhập kết quả thi.";
      toast.error(msg);
    } finally {
      setIsImporting(false);
    }
  ...
  const showStudentRank =
    businessType !== "general" &&
    (!!exam.rank || assignedStudents.some((s) => s.rank));
          ...
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
            ...
            <div className="flex flex-wrap items-center gap-1">
              <button
                ...
                className={cn(
                  "flex items-center gap-1 px-1.5 py-0.2 rounded-full text-[9px] font-semibold border hover:opacity-80 transition-opacity active:scale-95 cursor-pointer",
                  status.color,
                )}
              >
          ...
          <div className="flex flex-wrap items-center gap-3 sm:gap-4">
            ...
            {exam.officialDate && (
              <div className="flex items-center gap-1.5 group">
                ...
                <div>
                  <p className="text-[8px] font-bold text-emerald-400 uppercase tracking-wide leading-none">
                    Chính thức
                  </p>
            ...
            <div className="flex items-center gap-1.5 group">
              ...
              <div>
                ...
                <p className="text-[11px] font-bold text-slate-700 mt-0.5 truncate max-w-[150px] sm:max-w-[250px]">
                  {exam.location}
                </p>
              </div>
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 pt-2.5 sm:pt-0 border-t sm:border-t-0 border-slate-50">
          <div className="flex items-center gap-1.5 w-full sm:w-auto">
            <div className="flex-1 sm:flex-none px-2 py-1 bg-slate-50 rounded-lg text-center min-w-[50px] sm:min-w-[60px] border border-slate-100/50">
              ...
              <p className="text-[8px] sm:text-[9px] font-bold text-slate-400 mt-0.5 uppercase tracking-wide">
            ...
            <div className="flex-1 sm:flex-none px-2 py-1 bg-emerald-50 rounded-lg text-center min-w-[45px] sm:min-w-[50px] border border-emerald-100/30">
              ...
              <p className="text-[8px] sm:text-[9px] font-bold text-emerald-500 mt-0.5 uppercase tracking-wide">
                Đậu
              </p>
            </div>
            <div className="flex-1 sm:flex-none px-2 py-1 bg-rose-50 rounded-lg text-center min-w-[45px] sm:min-w-[50px] border border-rose-100/30">
              <p className="text-sm sm:text-base font-extrabold text-rose-600 leading-none">
                {exam.failCount}
              </p>
              <p className="text-[8px] sm:text-[9px] font-bold text-rose-500 mt-0.5 uppercase tracking-wide">
                Trượt
              </p>
            </div>
          </div>
        ...
        {isExpanded && (
          ...
          >
            <div className="p-3 space-y-2">
              <CustomFieldDetails
                moduleKey="exams"
                values={exam.customFields || {}}
              />

              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div>
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    Danh sách {entityLabel.singular} đăng ký (
                    {assignedStudents.length} {entityLabel.singular})
                  </h4>
                  <p className="text-[9px] text-slate-400 font-medium mt-0.5 max-w-md">
                    * File Excel nhập cần chứa các cột:{" "}
                    <strong className="text-slate-500">Số điện thoại</strong> và{" "}
                    <strong className="text-slate-500">Kết quả thi</strong> (Đậu
                    / Trượt / Chưa có). Tải file mẫu bên cạnh để xem ví dụ.
                  </p>
                </div>
                <div className="flex items-center gap-1.5">
                  <button
                    onClick={() =>
                      handleDownloadTemplate(exam, assignedStudents)
                    }
              ...
              {assignedStudents.length === 0 ? (
                <p className="text-xs text-slate-450 italic">
                  Đợt thi này chưa xếp {entityLabel.singular} nào.
                </p>
              ) : (
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50/60 border-b border-slate-100">
                        <th className="px-2.5 py-1.5 text-[9px] font-black text-slate-400 uppercase tracking-wider">
                          {entityLabel.tabLabel}
                        </th>
                        {showStudentRank && (
                          <th className="px-2.5 py-1.5 text-[9px] font-black text-slate-400 uppercase tracking-wider text-center">
                            Hạng
                          </th>
                        )}
                        <th className="px-2.5 py-1.5 text-[9px] font-black text-slate-400 uppercase tracking-wider">
                    ...
                    <tbody>
                      {assignedStudents.map((student) => {
                        ...
                        const resultText =
                          examEntry?.result?.overall || "Chưa có";
                                ...
                                className={cn(
                                  ...
                                  (
                                    Array.isArray(student.status)
                                      ? student.status.includes("Đã đậu")
                                      : student.status === "Đã đậu"
                                  )
                            ...
                            <td className="px-2.5 py-1.5 text-center">
                              <select
                                ...
                                className={cn(
                                  ...
                                  resultText === "Đậu"
                                    ? "bg-emerald-50 text-emerald-600 border-emerald-100 focus:ring-emerald-500/10"
                                    : resultText === "Trượt"
                                      ? "bg-rose-50 text-rose-600 border-rose-100 focus:ring-rose-500/10"
                                      : "bg-slate-50 text-slate-500 border-slate-200 focus:ring-slate-500/10",
                                )}
                              >
                                <option
                                  value="Chưa có"
                                  className="bg-white text-slate-700 font-bold uppercase"
                                >
                                  Chưa có
                                </option>
                                <option
                                  value="Đậu"
                                  className="bg-white text-emerald-600 font-bold uppercase"
                                >
                                  Đậu
                                </option>
                                <option
                                  value="Trượt"
                                  className="bg-white text-rose-600 font-bold uppercase"
                                >
                                  Trượt
                                </option>
                              </select>
                            ...
                            <td className="px-2.5 py-1.5 text-right">
                              <button
                                onClick={(e) => {
                                  ...
                                  if (
                                    window.confirm(
                                      `Bạn có chắc chắn muốn xóa ${entityLabel.singular} ${student.fullName} khỏi đợt thi này?`,
                                    )
                                  ) {
