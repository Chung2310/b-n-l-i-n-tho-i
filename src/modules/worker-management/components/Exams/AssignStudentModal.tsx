...
import {
  getExamSegmentLabel,
  isStudentEligibleForExamRank,
} from "../../pages/Exams/exam-utils";
...
export function AssignStudentModal({
  ...
  const eligibleStudents = students
    .filter((student) => {
      const matchesRank = isStudentEligibleForExamRank(
        businessType,
        exam.rank,
        student.rank,
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(
        `Đã thêm ${selectedStudentIds.length} ${entityLabel.singular} vào đợt thi thành công!`,
      );
      onSuccess();
      onClose();
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : `Lỗi gán ${entityLabel.singular} vào đợt thi.`,
      );
    } finally {
        ...
        >
          <div className="px-6 py-5 border-b border-slate-100">
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-lg font-bold text-slate-800">
                Thêm {entityLabel.singular} vào đợt thi
              </h3>
              <button
                onClick={onClose}
            ...
            <p className="text-sm text-slate-400 font-medium">
              Đợt thi:{" "}
              <span className="text-cyan-600 font-bold">{exam.name}</span>
          ...
          <div className="p-6 pb-2">
            ...
            <p className="text-[10px] text-rose-500 font-bold mt-2 flex items-center gap-1.5">
              ...
              Lưu ý: {entityLabel.titleCase} hoàn thành học phí mới được thêm
              vào đợt thi
            </p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 pt-2 space-y-2">
            {loading ? (
              <div className="py-10 text-center text-slate-400 italic text-sm">
                Đang nạp danh sách {entityLabel.singular}...
              </div>
              ...
              eligibleStudents.map((student) => (
                <button
                  ...
                  className={cn(
                    ...
                    !student.isFullyPaid &&
                      "opacity-60 grayscale-[0.5] cursor-not-allowed border-dashed bg-slate-50/50",
                  )}
                >
            ...
            <div>
              <p className="text-sm font-bold text-slate-800">
                ...
                <span className="text-cyan-600">
                  {selectedStudentIds.length} {entityLabel.singular}
                </span>
              </p>
            </div>
            <div className="flex items-center gap-3">
