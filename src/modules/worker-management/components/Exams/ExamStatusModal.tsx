...
export function ExamStatusModal({
  exam,
  isOpen,
  onClose,
  onSuccess,
}: ExamStatusModalProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [selectedStatus, setSelectedStatus] = useState<ExamStatus | "">(
    exam?.status || "",
  );
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Lỗi cập nhật trạng thái đợt thi.",
      );
    } finally {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <form
            ...
            className="p-6 space-y-8 overflow-y-auto max-h-[80vh]"
          >
            <div className="space-y-4">
              <label className="text-xs font-black text-slate-400 uppercase tracking-[0.1em]">
                Chọn trạng thái đợt thi
              </label>
            ...
            <div className="space-y-3">
              <label className="text-xs font-black text-slate-400 uppercase tracking-[0.1em]">
                Ngày thi chính thức (Nếu có)
              </label>
              <div className="relative">
                <input
                  type="date"
                  value={
                    officialDate.includes("/")
                      ? officialDate.split("/").reverse().join("-")
                      : officialDate
                  }
