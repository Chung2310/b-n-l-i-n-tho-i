...
export function ImportStudentModal({
    ...
    try {
      ...
      data = [
        ...
        [
          ...
          "ttb@gmail.com",
          "",
          "456 Đường Nguyễn Huệ, H.Hóc Môn",
          "20/06/2026",
        ],
      ];
      cols = [
  ...
  const mapHeaders = (row: unknown[]): Record<string, number> => {
    ...
    row.forEach((cell, idx) => {
      ...
      if (val.includes("họ và tên") || val.includes("họ tên") || val === "tên")
        ...
        val.includes("số điện thoại") ||
        val.includes("điện thoại") ||
        val === "sdt"
      )
        map.phone = idx;
      else if (
        val.includes("hạng bằng") ||
        val.includes("hạng") ||
        val === "hang" ||
        val.includes("khóa học") ||
        val.includes("khoa hoc")
      )
        map.rank = idx;
      else if (val.includes("học phí") || val.includes("hoc phi"))
        map.fee = idx;
      else if (
        val.includes("đã đóng") ||
        val.includes("da dong") ||
        val === "dong"
      )
        map.paidAmount = idx;
      else if (
        val.includes("ngày sinh") ||
        val.includes("ngay sinh") ||
        val === "năm sinh"
      )
        map.birthday = idx;
      else if (
        ...
        val.includes("dia chi") ||
        val.includes("nơi ở")
      )
        map.address = idx;
      else if (
        val.includes("ngày nhập học") ||
        val.includes("ngay nhap hoc") ||
        val.includes("nhập học")
      )
        map.enrollmentDate = idx;
    });
    return map;
  };
  ...
  const processFile = (file: File) => {
    if (
      ...
      setErrorMsg(
        "Vui lòng chọn file Excel đúng định dạng (.xlsx, .xls, .csv)",
      );
      return;
    }
    ...
    reader.onload = (e) => {
      try {
        ...
        if (missingHeaders.length > 0) {
          setErrorMsg(
            `File thiếu các cột bắt buộc sau: ${missingHeaders.join(", ")}`,
          );
          return;
        }
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          const feeNum =
            parseInt(getCellValue("fee").replace(/\D/g, ""), 10) || 0;
          const paidAmount =
            parseInt(getCellValue("paidAmount").replace(/\D/g, ""), 10) || 0;
          ...
          if (paidAmount > feeNum)
            errors.push(
              `Số tiền đã đóng (${paidAmount.toLocaleString("vi-VN")}đ) không được vượt quá học phí (${studentData.fee}đ)`,
            );
          ...
          if (
            ...
            !DATE_PATTERN.test(studentData.enrollmentDate)
          ) {
        ...
        if (validationResults.length === 0)
          setErrorMsg(
            `Không tìm thấy ${entityLabel.singular} hợp lệ nào trong file.`,
          );
        else setValidationRows(validationResults);
      } catch (error) {
        console.error("Error parsing file:", error);
        setErrorMsg(
          "Lỗi khi đọc file Excel. Vui lòng kiểm tra lại cấu trúc file.",
        );
      }
    };
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.success(
          `Đã nhập thành công ${res.importedCount} ${entityLabel.singular}!`,
        );
      ...
      setErrorMsg(
        error instanceof Error
          ? error.message
          : `Lỗi khi gửi yêu cầu nhập ${entityLabel.singular}.`,
      );
    } finally {
          ...
          <div className="flex items-center justify-between px-8 py-5 border-b border-slate-100 flex-shrink-0">
            <div>
              <h2 className="text-xl font-bold text-slate-800">
                Nhập danh sách {entityLabel.singular}
              </h2>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-sm mt-1">
                    Đã xử lý xong danh sách {entityLabel.singular} từ file{" "}
                ...
                <div className="grid grid-cols-2 gap-4 max-w-md mx-auto">
                  <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                    ...
                    <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-wider">
                      {entityLabel.titleCase} mới đã tạo
                    </p>
                  </div>
                  <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
              ...
              <div className="space-y-6">
                ...
                >
                  ...
                  <h3 className="text-sm font-bold text-slate-700 mt-4">
                    Kéo & thả file Excel của bạn vào đây
                  </h3>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        Tải ngay file mẫu chuẩn của hệ thống để nhập dữ liệu
                        chính xác và không bị lỗi.
                      </p>
                    </div>
                  </div>
                  <button
                  ...
                  <div className="overflow-x-auto max-h-[350px] no-scrollbar">
                    <table className="w-full text-left text-xs min-w-[900px]">
                      <thead className="bg-slate-50 border-b border-slate-100 sticky top-0 z-10">
                        <tr>
                          <th className="px-4 py-3 text-slate-400 font-bold text-center w-12">
                            Dòng
                          </th>
                          <th className="px-4 py-3 text-slate-400 font-bold w-40">
                            Họ và tên
                          </th>
                          <th className="px-4 py-3 text-slate-400 font-bold w-32">
                            Số điện thoại
                          </th>

                          <th className="px-3 py-3 text-slate-400 font-bold w-24">
                            Học phí
                          </th>
                          <th className="px-3 py-3 text-slate-400 font-bold w-24">
                            Đã đóng
                          </th>
                          <th className="px-3 py-3 text-slate-400 font-bold w-28">
                            Ngày nhập học
                          </th>
                          <th className="px-4 py-3 text-slate-400 font-bold">
                      ...
                      <tbody className="divide-y divide-slate-100 font-medium">
                        {validationRows.map((row) => (
                          <tr
                            key={row.rowNum}
                            className={`hover:bg-slate-50/50 ${!row.isValid ? "bg-rose-50/10" : ""}`}
                          >
                            <td className="px-4 py-3 text-slate-400 text-center font-bold">
                              {row.rowNum}
                            </td>
                            ...
                            <td className="px-3 py-3 text-emerald-600 font-semibold">
                              {(row.data.paidAmount || 0).toLocaleString(
                                "vi-VN",
                              )}
                              đ
                            </td>
                            <td className="px-3 py-3 text-slate-600 font-semibold">
                              {row.data.enrollmentDate || (
                                <span className="text-slate-300 italic">
                                  Không có
                                </span>
                              )}
              ...
              {validationRows.length > 0 && (
                ...
                >
                  ...
                  {isUploading
                    ? "Đang nhập dữ liệu..."
                    : totalValid > 0
                      ? `Nhập ${totalValid} ${entityLabel.singular} hợp lệ`
