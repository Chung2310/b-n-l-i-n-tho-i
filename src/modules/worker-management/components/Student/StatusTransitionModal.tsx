...
export function StatusTransitionModal({
  ...
  const workflowIndices = studentStatuses
    ...
    .filter((idx) => idx !== -1);
  const currentIndex =
    workflowIndices.length > 0 ? Math.max(...workflowIndices) : -1;
  const nextStatus =
    currentIndex !== -1 && currentIndex < statusWorkflow.length - 1
  ...
  const handleConfirm = async () => {
    ...
    if (
      ...
      if (paidAmount < totalFee) {
        setFeeError(
          `${entityLabel.titleCase} chưa hoàn tất học phí, không thể chuyển sang trạng thái Đang thi!`,
        );
        return;
      }
    }
          ...
          <div className="p-6">
            ...
            <div className="mb-8">
              <p className="text-sm font-medium text-slate-600 mb-2">
                Chuyển bước {entityLabel.singular}{" "}
              ...
              {nextStatus ? (
                <div className="space-y-3">
                  ...
                  {feeError && nextStatus === "Đang thi" && (
                    <p className="text-[11px] font-bold text-rose-500 bg-rose-50 px-3 py-2 rounded-lg">
                      {feeError}
                    </p>
                  )}
                </div>
                ...
                <p className="text-xs italic text-slate-400">
                  {entityLabel.titleCase} đã ở bước cuối cùng của quy trình.
                </p>
              )}
            </div>
