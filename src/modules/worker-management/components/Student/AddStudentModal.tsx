...
export function AddStudentModal({
  ...
  useEffect(() => {
    const fetchPartners = async () => {
      try {
        const ownerId =
          user?.role === "superadmin" ? selectedCenterId : undefined;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    stdFields.forEach((f) => {
      if (f.isVisible && !f.isArchived && f.isRequired) {
        if (f.key === "fullName" && !formData.fullName)
        ...
        if (
          ...
          missingFields.push(f.label);
        if (f.key === "birthday" && !formData.birthday)
          missingFields.push(f.label);
        if (f.key === "idCard" && !formData.idCard) missingFields.push(f.label);
        if (f.key === "enrollmentDate" && !formData.enrollmentDate)
          missingFields.push(f.label);
        if (f.key === "address" && !formData.address)
          missingFields.push(f.label);
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        if (faceBlob) {
          try {
            ...
            faceBody.append(
              "file",
              new File([faceBlob], "face.jpg", {
                type: faceBlob.type || "image/jpeg",
              }),
            );
            await apiFetch(`/students/${res.data._id}/face`, {
              method: "POST",
              body: faceBody,
            });
            ...
            const msg =
              faceError instanceof Error
                ? faceError.message
                : "Không thể lưu mẫu khuôn mặt.";
            toast.warning(
              `Đã tạo ${entityLabel.singular} nhưng chưa lưu được mẫu khuôn mặt: ${msg}`,
            );
          }
        }
      ...
      const message =
        error instanceof Error
          ? error.message
          : `Lỗi lưu hồ sơ ${entityLabel.singular}.`;
      setErrorMsg(message);
      toast.error(message);
    } finally {
  ...
  const handleInputChange = (
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: name === "fee" ? formatVND(value) : value,
    }));
  };
                ...
                {isFieldVisible("referral") && (
                  <div className="sm:col-span-2 relative group/std space-y-1">
                    ...
                    <label className="text-[10px] font-bold text-slate-800 uppercase tracking-wider block">
                      ...
                      {isFieldRequired("referral", false) && (
                      ...
                      <div className="relative">
                        ...
                        >
                          ...
                          <option value="partner">
                            Đối tác / CTV hệ thống
                          </option>
                          <option value="custom">
                            Nhập người giới thiệu khác
                          </option>
                        </select>
                        <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
                      </div>
                ...
                entityLabel.preset !== "customer" && (
                  <div className="pt-2 border-t border-slate-50">
                    <FaceCaptureInput
                      ...
                      disabled={isSubmitting}
                      entityName={entityLabel.singular}
                    />
                  </div>
                )}
