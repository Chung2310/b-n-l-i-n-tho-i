...
export function StudentDetailModal({
  ...
  const [editingPayment, setEditingPayment] =
    React.useState<EditingPaymentState | null>(null);
  ...
  const handleStartEditPayment = (
    p: NonNullable<Student["paymentHistory"]>[number],
    idx: number,
  ) => {
    setEditingPayment({
      index: idx,
  ...
  const handleSavePaymentEdit = async () => {
    ...
    const parsedAmount =
      parseInt(editingPayment.amount.replace(/\D/g, ""), 10) || 0;
    if (parsedAmount <= 0) {
  ...
  const handleDeletePaymentClick = async (
    ...
    const confirmDelete = window.confirm(
      `Bạn có chắc chắn muốn xóa đợt thanh toán ${new Intl.NumberFormat("vi-VN").format(p.amount)}đ ngày ${p.date} không?`,
    );
                ...
                <div className="flex items-center gap-4 sm:gap-6">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-1 sm:mt-2">
                      <span className="text-slate-500 text-[10px] sm:text-xs font-medium">
                        {student.phone}
                      </span>
                    </div>
                  </div>
                  ...
                  activeTab !== "Lịch sử" && (
                    <div className="flex flex-col items-center justify-center py-20 px-4 bg-white rounded-[2rem] border border-slate-100">
                      ...
                      <p className="text-slate-400 text-sm text-center">
                        Chúng tôi đang cập nhật phân hệ {activeTab} sớm nhất có
                        thể.
                      </p>
                    </div>
                  )}
