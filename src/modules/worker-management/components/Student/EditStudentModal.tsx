...
export function EditStudentModal({
  ...
  useEffect(() => {
    if (student) {
      setTimeout(() => {
        ...
        setReferralMode(
          isPartnerRef ? "partner" : isCustomRef ? "custom" : "none",
        );
        ...
        setFormData({
          ...
          status: Array.isArray(student.status)
            ? student.status
            : student.status
              ? [student.status]
              : [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (duplicateField) {
      toast.error(
        `${duplicateField.label} đã tồn tại trong hệ thống, không được trùng.`,
      );
      return;
    }
    ...
    try {
      ...
      toast.success(
        `Đã cập nhật thông tin ${entityLabel.singular} thành công!`,
      );
      onSuccess();
      onClose();
    } catch (error: unknown) {
