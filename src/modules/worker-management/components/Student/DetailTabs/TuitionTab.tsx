...
export function TuitionTab({
  ...
  const [vietqrConfig, setVietqrConfig] = useState(() => {
    ...
    const accountNo =
      localConfig?.accountNo || (user as any)?.bankAccountNo || "";
    const accountName =
      ...
      "";
    const enabled =
      localConfig?.enabled ?? (user as any)?.bankQrEnabled !== false;
          ...
          <div className="space-y-3">
            {(student.paymentHistory || []).length > 0 ? (
              student.paymentHistory?.map((p, idx) => (
                <div
                  key={idx}
                  className="group relative flex items-center justify-between p-4 bg-slate-50 hover:bg-slate-100/50 rounded-2xl border border-slate-100 transition-all"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      {p.note && (
                        <p
                          className="text-[10px] text-slate-400 italic mt-0.5 max-w-[200px] truncate"
                          title={p.note}
                        >
                          Ghi chú: {p.note}
                        </p>
        ...
        <div className="lg:col-span-1 space-y-6">
            ...
            {totalFee === 0 ? (
              ...
              <div className="w-full py-6 text-center space-y-3">
                ...
                <p className="text-[10px] text-slate-400">
                  Vui lòng truy cập trang <strong>Cài đặt ERP</strong> hoặc cấu
                  hình cá nhân để thiết lập VietQR.
                </p>
              </div>
            ) : (
              <div className="w-full space-y-4">
                  ...
                  <div className="w-full space-y-1.5 text-xs text-slate-600">
                    ...
                    <div className="pt-1.5 border-t border-slate-200/60 mt-1 flex flex-col gap-1">
                      ...
                      <div className="flex items-center justify-between bg-white px-3 py-1.5 rounded-lg border border-slate-100">
                        <span className="font-mono text-[10px] font-black text-slate-800 tracking-wider select-all">
                          {student.id}
                        </span>
                        <button
                ...
                <p className="text-[10px] text-slate-400 leading-normal text-center italic">
                  * Hệ thống tự động ghi nhận học phí ngay lập tức sau khi nhận
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col items-center text-center">
            ...
            <p className="text-xs text-slate-400 mt-1 max-w-[180px]">
              {entityLabel.titleCase} cần hoàn tất học phí trước ngày thi sát
              hạch 15 ngày.
            </p>
            ...
            <div className="w-full mt-6 space-y-3">
              {(() => {
                ...
                return (
                  <div
                    className={cn(
                      "p-3 rounded-xl text-left border shadow-sm",
                      bgColor,
                    )}
                  >
...
function FeeCard({
  ...
  return (
    ...
    >
      ...
      <p
        className={cn(
          "text-2xl font-black tracking-tight whitespace-nowrap",
          color,
        )}
      >
        {formatVND(amount)}đ
