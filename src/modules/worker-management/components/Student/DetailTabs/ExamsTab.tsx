...
export function ExamsTab({
  ...
  const [focusedInput, setFocusedInput] = React.useState<{
    idx: number;
    field: "theory" | "simulation" | "practice";
  } | null>(null);
  return (
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-bold text-slate-800">
            Lịch sử và Kết quả thi
          </h3>
          <p className="text-slate-500 text-sm mt-1">
            Ghi nhận chi tiết kết quả thi Tốt nghiệp và Sát hạch quốc gia.
          </p>
        </div>
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="text-center md:text-left flex-1">
              ...
              <h4 className="text-2xl font-black">
                {examData.length > 0
                  ? examData[examData.length - 1].name
                  : "Chưa có dữ liệu thi"}
              </h4>
                ...
                {[
                  {
                    ...
                    val:
                      examData.length > 0
                        ? `${examData[examData.length - 1].result?.theory || "--"}`
                        : "--",
                  },
                  {
                    label: "Mô phỏng",
                    val:
                      examData.length > 0
                        ? `${examData[examData.length - 1].result?.simulation || "--"}`
                        : "--",
                  },
                  {
                    label: "Thực hành",
                    val:
                      examData.length > 0
                        ? `${examData[examData.length - 1].result?.practice || "--"}`
                        : "--",
                  },
                ].map((item, i) => (
                  <div
                    key={i}
                    className="px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-center min-w-[100px]"
                  >
                    <p className="text-[9px] font-bold text-white/50 uppercase tracking-tight">
                      {item.label}
                    </p>
            ...
            <div className="px-8 py-4 bg-emerald-500 rounded-3xl text-center shadow-xl shadow-emerald-500/20">
              ...
              <p className="text-xl font-black text-white mt-1">
                {examData.length > 0
                  ? examData[examData.length - 1].result?.overall || "CHỜ KQ"
                  : "--"}
              </p>
            </div>
          </div>
                    ...
                    <td className="px-4 py-4 text-center">
                      {isEditingExams ? (
                        <select
                          ...
                          onChange={(e) => {
                            const newExams = [...examData];
                            newExams[idx].type = e.target.value as
                              | "Tốt nghiệp"
                              | "Sát hạch";
                            setExamData(newExams);
                          }}
                    ...
                    <td className="px-4 py-4 text-center">
                      {isEditingExams ? (
                        <input
                          ...
                          value={
                            focusedInput?.idx === idx &&
                            focusedInput?.field === "theory" &&
                      ...
                      {isEditingExams ? (
                        <input
                          ...
                          onChange={(e) => {
                            const val = e.target.value.replace(/[^0-9-]/g, "");
                            const newExams = [...examData];
                            newExams[idx].result.simulation =
                              val === "" ? 0 : parseInt(val) || 0;
                            setExamData(newExams);
                          }}
                        />
                      ...
                      {isEditingExams ? (
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              if (newExams[idx].result) {
                                newExams[idx].result.overall = e.target
                                  .value as
                                  | "Đậu"
                                  | "Trượt"
                                  | "Chưa có"
                        ...
                        <span
                          className={cn(
                            ...
                            exam.result?.overall === "Đậu"
                              ? "bg-emerald-50 text-emerald-600 border border-emerald-100"
                              : exam.result?.overall === "Chưa có"
