...
export function FaceEnrollmentTab({
  student,
  onUpdated,
}: FaceEnrollmentTabProps) {
  ...
  const fetchStatus = React.useCallback(async () => {
    ...
    try {
      ...
      setStatus({
        ...
        registeredAt: res.registeredAt ?? null,
      });
    } catch (err) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      body.append(
        "file",
        new File([capture.blob], "face.jpg", {
  ...
  const handleRemove = async () => {
    ...
    const confirmDelete = window.confirm(
      `Bạn có chắc chắn muốn xóa mẫu khuôn mặt của ${student.fullName}?`,
    );
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Không thể xóa mẫu khuôn mặt.";
      setError(message);
      toast.error(message);
    } finally {
      setRemoving(false);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-800">
              Mẫu khuôn mặt điểm danh
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Dùng để xác thực khuôn mặt khi {entityLabel.singular} điểm danh
              qua QR hoặc mã online.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[minmax(0,320px)_1fr] gap-6">
        ...
        <div className="flex flex-col justify-between gap-4">
          <div className="space-y-2 text-xs text-slate-500">
            <p>
              1. Bật camera và đưa khuôn mặt {entityLabel.singular} vào giữa
              khung hình, ánh sáng đủ rõ.
            </p>
            ...
            {status?.registeredAt && (
              <p className="text-slate-400">
                Đăng ký lần cuối:{" "}
                {new Date(status.registeredAt).toLocaleString("vi-VN")}
              </p>
            )}
          </div>
