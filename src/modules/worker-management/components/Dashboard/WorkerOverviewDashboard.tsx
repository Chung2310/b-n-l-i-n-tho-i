...
export function WorkerOverviewDashboard({
  ...
  const submittedDossierCount = filteredWorkers.filter((s) =>
    hasStatus(s, "Đã nộp HS"),
  ).length;
  const inTrainingCount = filteredWorkers.filter(
    (s) => hasStatus(s, "Đang học") || hasStatus(s, "Đang thi"),
  ).length;
  const referredWorkersCount = filteredWorkers.filter((s) =>
    Boolean(s.referral || s.partnerId),
  ).length;
  const completedCount = filteredWorkers.filter((s) =>
    hasStatus(s, "Đã đậu"),
  ).length;
  ...
  const rankDistribution = useMemo(() => {
    ...
    const items = Array.from(map.entries())
      .map(([rank, count]) => ({
        rank,
        count,
        percent: totalWorkersCount
          ? Math.round((count / totalWorkersCount) * 100)
          : 0,
      }))
  ...
  const getStatusBadge = (statusList: string[]) => {
    ...
    switch (main) {
      case "Đã nộp HS":
        return (
          <span className="px-2.5 py-1 rounded-full text-[10px] font-black bg-cyan-50 text-cyan-700 border border-cyan-200">
            Đã nộp HS
          </span>
        );
      case "Đang học":
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="text-sm font-black text-slate-800 tracking-tight">
                  Biểu đồ Cột - Đăng ký {entityLabel.titleCase} theo Tháng năm{" "}
                  {currentYear}
                </h3>
          ...
          <div className="pt-6 pb-2">
            <div className="h-56 flex items-end justify-between gap-2 px-2 border-b border-slate-200 pb-2 relative">
              ...
              {monthlyData.months.map((m, idx) => {
                const heightPercent = Math.max(
                  8,
                  Math.round((m.count / monthlyData.maxCount) * 100),
                );
                return (
                  <div
                    key={idx}
                  ...
                  >
                    ...
                    <div className="opacity-0 group-hover:opacity-100 transition-all duration-200 absolute -top-8 bg-slate-900 text-white text-[10px] font-extrabold px-2 py-1 rounded-lg shadow-lg pointer-events-none whitespace-nowrap z-20">
                      {m.fullLabel}: <strong>{m.count}</strong>{" "}
            ...
            <div className="flex justify-between gap-2 px-2 pt-2">
              {monthlyData.months.map((m, idx) => (
                <div
                  key={idx}
                  className="flex-1 text-center text-[10px] font-black text-slate-400"
                >
                  {m.monthLabel}
                </div>
              ))}
              ...
              rankDistribution.map((item, idx) => (
                <div key={idx} className="space-y-1.5">
                  <div className="flex justify-between text-xs font-bold">
                    <span className="text-slate-700 truncate max-w-[180px] font-black">
                      {item.rank}
                    </span>
                    <span className="text-slate-500 font-extrabold">
                      {item.count} {entityLabel.singular} ({item.percent}%)
                    </span>
                  </div>
      ...
      <ErpCard className="p-6 space-y-4 bg-white border border-slate-150 shadow-xs">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 pt-2">
          {referralSources.map((ref, idx) => (
            <div
              key={idx}
              className="p-3.5 rounded-2xl border border-slate-150 bg-slate-50/50 space-y-1 hover:border-sky-300 transition-all"
            >
      ...
      <ErpCard className="p-6 space-y-4 bg-white border border-slate-150 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-black text-slate-800 tracking-tight">
                Danh sách Chi tiết {entityLabel.titleCase} (
                {tableWorkers.length})
              </h3>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                Hiển thị dữ liệu chuẩn theo đúng các trường thông tin{" "}
                {entityLabel.singular} trong hệ thống
              </p>
            </div>
          </div>
              ...
              {studentsLoading ? (
                ...
                <tr>
                  <td
                    ...
                    className="py-8 text-center text-slate-400 text-xs italic"
                  >
                    Không tìm thấy thông tin {entityLabel.singular} nào khớp với
                    bộ lọc.
                  </td>
                </tr>
              ) : (
                tableWorkers.slice(0, 10).map((worker) => (
                  ...
                  >
                    ...
                    <td
                      className="px-4 py-3.5 text-slate-600 text-[11px] max-w-[180px] truncate"
