...
export function DrivingDashboardTables({
  onSelectStudent,
  onNavigate,
  selectedCenter,
}: DrivingDashboardTablesProps) {
  ...
  const upcomingExams = exams
    ...
    .sort((a, b) => {
      const dateA = parseDateString(
        a.officialDate || a.tentativeDate,
      ).getTime();
      const dateB = parseDateString(
        b.officialDate || b.tentativeDate,
      ).getTime();
      return dateA - dateB;
    });
  ...
  const paginatedRecentStudents = recentStudents.slice(
  ...
  return (
    <div
      className={cn("grid grid-cols-1 gap-6", isEducation && "lg:grid-cols-2")}
    >
      {isEducation && (
        <LuxuryCard padding="none" className="bg-white">
          <div className="p-4 border-b border-slate-100 flex items-center justify-between">
            <h3 className="flex items-center gap-2 text-sm font-bold text-slate-800">
              <Calendar className="w-4 h-4 text-brand-primary" /> Lịch thi sắp
              tới (7 ngày)
            </h3>
            <button
                ...
                {loading && user ? (
                  ...
                  <tr>
                    <td
                      ...
                      className="px-6 py-8 text-center text-slate-400 text-xs italic"
                    >
                      Không có lịch thi trong 7 ngày tới.
                    </td>
                  </tr>
                ) : (
        ...
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          <h3 className="flex items-center gap-2 text-sm font-bold text-slate-800">
            ...
            {isEducation
              ...
              : `${entityLabel.titleCase} mới đăng ký (7 ngày)`}{" "}
            {user && `(${recentStudents.length})`}
          </h3>
          <button
            onClick={() => onNavigate("Students")}
            className="text-xs text-slate-400 hover:text-slate-600 font-medium whitespace-nowrap ml-2"
          >
            Xem tất cả
          </button>
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="px-3 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center">
                  Hạng
                </th>
                <th className="px-3 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center">
                  Ngày ĐK
              ...
              {loading && user ? (
                ...
                mockRegistrations.map((item, idx) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 text-center text-[10px] text-slate-400 font-medium">
                      -
                    </td>
                    <td className="px-3 py-3 text-center">
                      <span
                ...
                <tr>
                  <td
                    ...
                    className="px-6 py-8 text-center text-slate-400 text-xs italic"
                  >
                    Không có {entityLabel.singular} đăng ký trong 7 ngày qua.
                  </td>
                </tr>
              ) : (
                paginatedRecentStudents.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 text-center text-[10px] font-bold text-slate-500 whitespace-nowrap">
                    ...
                    <td className="px-3 py-3 text-center">
                      <span
                        className={cn(
                          "px-3 py-1 rounded-full text-[10px] font-bold whitespace-nowrap",
                          getStatusInfo(item.status[0] || "Đang học"),
                        )}
                      >
