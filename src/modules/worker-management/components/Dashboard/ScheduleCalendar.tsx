...
export function ScheduleCalendar({
  ...
  const toDate = useMemo(
    () => `${year}-${pad2(month + 1)}-${pad2(daysInMonth)}`,
  ...
  const monthLabel = currentMonth.toLocaleString("vi-VN", {
    month: "long",
    year: "numeric",
  });
  ...
  const daysArray = Array.from({ length: totalGridCells }, (_, index) => {
    ...
    const dateString = isValidDay
      ? `${year}-${pad2(month + 1)}-${pad2(dayNumber)}`
      : "";
    return { dayNumber, isValidDay, dateString };
  });

  const filteredEvents = events.filter(
    (evt) => selectedType === "all" || evt.type === selectedType,
  );
      ...
      <ErpCard className="xl:col-span-2 p-6 space-y-4">
        ...
        <div className="grid grid-cols-7 gap-1">
          {["CN", "T2", "T3", "T4", "T5", "T6", "T7"].map((day) => (
            <div
              key={day}
              className="text-center text-[9px] font-black uppercase tracking-widest text-slate-400 py-2"
            >
              {day}
            </div>
          ))}
        ...
        <div className="grid grid-cols-7 gap-1">
          {daysArray.map((day, idx) => {
            const dayEvents = day.isValidDay
              ? eventsByDate.get(day.dateString) || []
              : [];
                    ...
                    <div className="space-y-0.5 mt-1">
                      {dayEvents.slice(0, 2).map((evt) => (
                        <div
                          ...
                          className={cn(
                            "px-1.5 py-0.5 rounded-md border text-[8px] font-black truncate",
                            typeStyle(evt.type),
                          )}
                        >
                          {evt.title}
