  ...
  const getRealStats = () => {
    ...
    const hasStatus = (s: (typeof students)[number], st: string) =>
      Array.isArray(s.status)
        ? s.status.includes(st as (typeof s.status)[number])
        : s.status === st;
    ...
    if (entityLabel.preset === "worker") {
      ...
      return [
        ...
        {
          label: "Đã tiếp nhận / Đi làm",
          value: students.filter(
            (s) => hasStatus(s, "Đã đậu") || hasStatus(s, "Đang học"),
          ).length,
          icon: UserCheck,
    ...
    const statsMap: Record<string, number> = {
      ...
      "Còn nợ học phí": students.filter((s) => hasStatus(s, "Nợ học phí"))
        .length,
    };
    ...
    switch (count) {
      ...
      case 9:
        return {
          ...
          getItemClass: (idx: number) =>
            idx === 8 ? "col-span-2 md:col-span-1 lg:col-span-1" : "",
        };
      case 10:
        return {
  ...
  return (
    <div className={gridClass}>
      {displayStats.map((stat, idx) => (
        ...
        >
          <div
            className={`p-2 rounded-lg ${stat.bg} ${stat.color} transition-transform duration-300 group-hover:scale-110 shrink-0`}
          >
