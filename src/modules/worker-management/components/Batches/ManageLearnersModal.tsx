...
export function ManageLearnersModal({
  ...
  const availableStudents = students.filter(
    (s) => !batch.learnerIds.includes(s.id),
  );
  ...
  const toggleAllFiltered = () => {
    setSelectedStudentIds((current) => {
      ...
      const additions = filteredStudentIds.filter(
  ...
  const handleAddLearners = async () => {
    ...
    try {
      ...
      toast.success(
        `Đã thêm ${addedIds.length} ${entityLabel.singular} vào ${copy.entityNameLower}.`,
      );
      onSuccess();
    } catch (error: unknown) {
      setSelectedStudentIds((current) =>
      ...
      toast.error(
        addedIds.length > 0
          ? `Đã thêm ${addedIds.length} ${entityLabel.singular}. ${msg}`
          : msg,
      );
    } finally {
      setIsAdding(false);
  ...
  const handleRemoveLearner = async (studentId: string) => {
    ...
    try {
      await apiFetch(`/batches/${batch.id}/learners/${studentId}`, {
        method: "DELETE",
      });
      toast.success(
    ...
    >
      <div className="space-y-6">
        ...
        >
          ...
          {batch.maxLearners ? `/${batch.maxLearners}` : ""}{" "}
        ...
        <div className="space-y-3 text-left">
          ...
          {remainingSlots === 0 ? (
            <p className="py-6 text-center text-xs font-semibold text-slate-500">
              {copy.entityName} đã đủ{" "}
              {copy.capacityLabel.toLocaleLowerCase("vi")}.
            </p>
          ) : availableStudents.length === 0 ? (
            <p className="py-6 text-center text-xs text-slate-400">
              Không còn {entityLabel.singular} để thêm vào{" "}
              {copy.entityNameLower}.
            </p>
          ) : filteredStudents.length === 0 ? (
            <p className="py-6 text-center text-xs text-slate-400">
              Không tìm thấy {entityLabel.singular} phù hợp.
            </p>
          ) : (
              ...
              <div className="max-h-60 divide-y divide-slate-100 overflow-y-auto">
                {filteredStudents.map((student) => {
                  ...
                  const isDisabled =
                    isAdding ||
                    (!isSelected &&
                      selectedStudentIds.length >= remainingSlots);
                  return (
                    <label
                      key={student.id}
                      className={cn(
          ...
          <div className="flex justify-end">
            ...
            >
              ...
              {isAdding
                ? `Đang thêm ${selectedStudentIds.length} ${entityLabel.singular}...`
        ...
        <div className="space-y-2 text-left">
          ...
          {enrolledStudents.length === 0 ? (
            ...
            <div
              className={cn(
                "border rounded-2xl p-2 max-h-72 overflow-y-auto divide-y",
                darkMode
