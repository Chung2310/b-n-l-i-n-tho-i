...
export function AttendanceViewModal({
  ...
  const takenCount = allDates.filter((d) =>
    (batch.attendanceSessions || []).some(
      (s) => s.date === d && s.records && s.records.length > 0,
    ),
  ).length;
  const missedCount = allDates.filter(
    (d) =>
      d < today &&
      !(batch.attendanceSessions || []).some(
        (s) => s.date === d && s.records && s.records.length > 0,
  ...
  const selectedSession = selectedDate
    ? (batch.attendanceSessions || []).find((s) => s.date === selectedDate)
    : null;
  ...
  const topAbsentees = Object.entries(absentMap)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5);
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-2xl bg-cyan-50/60 border border-cyan-100 p-3.5 text-center">
            <p className="text-xl font-black text-cyan-800">
              {takenCount}
              <span className="text-sm font-semibold text-cyan-600/60">
                /{allDates.length}
              </span>
            </p>
          ...
          >
            <p
              className={cn(
                "text-xl font-black",
                missedCount > 0 ? "text-amber-700" : "text-slate-400",
              )}
            >
              {missedCount}
            </p>
            <p
              className={cn(
                ...
                missedCount > 0 ? "text-amber-600" : "text-slate-400",
              )}
            >
              Buổi bỏ sót
            </p>
          ...
          >
            <p
              className={cn(
                "text-xl font-black",
                topAbsentees.length > 0 ? "text-rose-700" : "text-slate-400",
              )}
            >
            ...
            <p
              className={cn(
                "text-[10px] font-bold mt-0.5",
                topAbsentees.length > 0 ? "text-rose-500" : "text-slate-400",
              )}
            >
              HV có vắng mặt
            </p>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="flex flex-col gap-2">
            <p className="text-[10px] font-black uppercase tracking-wider text-cyan-700 flex items-center gap-1.5">
              <Calendar className="w-3.5 h-3.5 text-cyan-500" /> Chọn buổi để
              xem chi tiết
            </p>
            ...
            {relevantDates.length === 0 ? (
              ...
              <div className="space-y-1 max-h-72 overflow-y-auto">
                {relevantDates.map((date) => {
                  ...
                  const hasTaken =
                    !!session && (session.records?.length || 0) > 0;
                  const isToday = date === today;
                  const isPast = date < today;
                  const isSelected = selectedDate === date;
                  const absentCount = session?.records
                    ? session.records.filter((r) => r.status !== "present")
                        .length
                    : 0;
                  const presentCount = session?.records
                    ? session.records.filter((r) => r.status === "present")
                        .length
                    : 0;
                ...
                <div className="space-y-1 max-h-[260px] overflow-y-auto">
                  {batch.learnerIds.length === 0 ? (
                    <p className="text-xs text-slate-400 text-center py-8">
                      {copy.entityName} chưa có {entityLabel.singular}.
                    </p>
                  ) : (
                    batch.learnerIds.map((studentId) => {
                      ...
                      const cfg = {
                        ...
                        absent: {
                          ...
                          text: "Vắng mặt",
                          row: "border-rose-100 bg-rose-50/20",
                        },
                        excused: {
                          icon: (
                          ...
                          text: "Vắng có phép",
                          row: "border-amber-100 bg-amber-50/20",
                        },
                        unmarked: {
                          icon: (
                ...
                {topAbsentees.length > 0 ? (
                  <div className="space-y-1.5">
                    {topAbsentees.map(([studentId, count], idx) => {
                      ...
                      return (
                        <div
                          key={studentId}
                          className="flex items-center gap-3 px-3 py-2.5 rounded-xl border border-slate-100 bg-white"
                        >
                          <span
                            className={cn(
                  ...
                  <div className="flex flex-col items-center justify-center flex-1 gap-3 py-6">
                    ...
                    <p className="text-xs text-slate-400 text-center">
                      Chọn một buổi học bên trái
                      <br />
                      để xem danh sách điểm danh
                    </p>
                  </div>
                )}
