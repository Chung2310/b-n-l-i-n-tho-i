...
export function AttendanceModal({
  ...
  const getScheduledDates = (
    ...
    const dates: string[] = [];
    if (!startDateStr || !endDateStr || !daysOfWeek || daysOfWeek.length === 0)
      return dates;
  ...
  const handleUpdateAttendance = async () => {
    ...
    try {
      const recordsPayload = Object.keys(attendanceRecords).map(
        (studentId) => ({
          studentId,
  ...
  const handleDeleteSession = async (date: string) => {
    if (
      !window.confirm(
        `Bạn có chắc chắn muốn xóa dữ liệu điểm danh ngày ${formatDate(date)} không?`,
      )
    )
      return;
    try {
      ...
      const msg =
        error instanceof Error
          ? error.message
          : "Có lỗi xảy ra khi xóa dữ liệu điểm danh.";
      toast.error(msg);
    }
  };
        ...
        <div className="space-y-6 text-left">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-[10px] text-slate-400">
                {copy.capacityLabel}: {batch.learnerIds.length}{" "}
                {entityLabel.singular} • Lịch hoạt động:{" "}
                {formatDays(batch.daysOfWeek)} ({batch.startTime} -{" "}
                {batch.endTime})
              </p>
            </div>
            {!showAddSession && (
              ...
              >
                <CalendarRange className="w-4 h-4 text-cyan-600" /> Bổ sung ngày
                học khác
              </button>
            )}
          </div>
          ...
          {batch.learnerIds.length === 0 && (
            <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-xs text-amber-700 font-medium">
              ⚠️ {copy.entityName} này hiện chưa có {entityLabel.singular} nào.
              Hãy bấm <strong>Quản lý {entityLabel.singular} (👥)</strong> để
              thêm {entityLabel.singular} vào {copy.entityNameLower} trước khi
              điểm danh.
            </div>
          )}
          ...
          {showAddSession && (
            <div className="p-4 border border-cyan-100 rounded-2xl bg-cyan-50/40 space-y-4">
              <h5 className="text-xs font-bold text-cyan-900">
                Thêm ngày học bổ sung (ngoài lịch cố định)
              </h5>
          ...
          <div className="space-y-2">
            ...
            {(() => {
              const sDates = getScheduledDates(
                batch.startDate,
                batch.endDate,
                batch.daysOfWeek,
              );
              const exDates = (batch.attendanceSessions || [])
                .map((s) => s.date)
                .filter((d) => !sDates.includes(d));
              const allDates = [...sDates, ...exDates].sort(
                ...
                return (
                  <div className="text-center py-6 border border-dashed border-slate-200 rounded-2xl">
                    <p className="text-xs text-slate-400">
                      Không có ngày học nào trong lịch trình đã chọn.
                    </p>
                  </div>
                );
              }
              ...
              return (
                <div className="border border-cyan-100 rounded-2xl p-2 divide-y divide-cyan-50/60 max-h-72 overflow-y-auto bg-white">
                  {allDates.map((date) => {
                    const session = batch.attendanceSessions?.find(
                      (s) => s.date === date,
                    );
                    ...
                    return (
                      <div
                        key={date}
                        className="flex items-center justify-between py-3 px-3 hover:bg-cyan-50/40 transition-all"
                      >
                        <div
          ...
          <div className="flex items-center justify-between p-3 bg-cyan-50/50 border border-cyan-100 rounded-2xl">
            <div className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={
                  bulkSelectStudents.length === batch.learnerIds.length &&
                  batch.learnerIds.length > 0
                }
                onChange={(e) => {
                  if (e.target.checked) {
              ...
              <span className="text-[11px] font-bold text-slate-600">
                Đã chọn {bulkSelectStudents.length}/{batch.learnerIds.length}{" "}
          ...
          <div className="border border-cyan-100 rounded-2xl divide-y divide-cyan-50/60 max-h-72 overflow-y-auto bg-white">
            {batch.learnerIds.length === 0 ? (
              <p className="text-center py-6 text-xs text-slate-400">
                {copy.entityName} hiện chưa có {entityLabel.singular} nào.
              </p>
            ) : (
              batch.learnerIds.map((studentId) => {
                ...
                return (
                  <div
                    key={studentId}
                    className="flex items-center justify-between py-2.5 px-4 hover:bg-cyan-50/30 transition-all"
                  >
