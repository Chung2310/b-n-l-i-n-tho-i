...
export function QRAttendanceModal({
      ...
      try {
        ...
        if (res.sessionId) {
          ...
          const remainingSecs = Math.max(
            0,
            Math.floor((res.expiresAt - now) / 1000),
          );
          const tokenRemainingSecs = Math.max(
            0,
            Math.floor((res.tokenExpiresAt - now) / 1000),
          );
  ...
  useEffect(() => {
    ...
    console.log(
      `[QRAttendance] Registering socket listener for session: ${sessionId}`,
    );
    ...
    const statusInterval = setInterval(async () => {
      try {
        const res = await apiFetch(
          `/qr-attendance/session/${sessionId}/status`,
        );
        if (res.students) {
          setCheckedInMap((prev) => {
            const next = new Map(prev);
            res.students.forEach((student: any) => {
              if (student.checkedIn) {
                next.set(student.studentId, {
                  checkinAt: student.checkinAt || Date.now(),
                });
              }
            });
            return next;
  ...
  useEffect(() => {
    ...
    countdownIntervalRef.current = setInterval(() => {
      ...
      const tokenRemainingSecs = Math.max(
        0,
        Math.floor((tokenExpiresAt - now) / 1000),
      );
      setTokenTimeRemaining(tokenRemainingSecs);
  ...
  const checkedInCount = classStudents.filter((s) =>
          ...
          <div className="space-y-0.5">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-black text-slate-800 tracking-tight">
                Điểm danh {copy.entityNameLower} {batch.code}
              </h3>
            </div>
              ...
              <div className="md:col-span-5 flex flex-col items-center justify-between border-b md:border-b-0 md:border-r border-slate-100 pb-6 md:pb-0 md:pr-6 space-y-6">
                  ...
                  <div className="relative bg-white border-2 border-slate-100 p-3 rounded-2xl shadow-lg max-w-[280px] mx-auto overflow-hidden group">
                    {qrDataUrl ? (
                      <img
                        src={qrDataUrl}
                        alt="QR Code"
                        className="w-60 h-60 mx-auto transition-transform group-hover:scale-102"
                      />
                    ) : (
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 text-xs font-semibold text-slate-500 space-y-2.5 max-w-[285px] w-full shadow-inner">
                  <div className="flex items-start gap-2">
                    <span className="bg-brand-primary/10 text-brand-primary w-5 h-5 rounded-full flex items-center justify-center shrink-0 font-bold">
                      1
                    </span>
                    <p>
                      {entityLabel.titleCase} mở camera điện thoại quét mã QR.
                    </p>
                  </div>
                  <div className="flex items-start gap-2">
                    <span className="bg-brand-primary/10 text-brand-primary w-5 h-5 rounded-full flex items-center justify-center shrink-0 font-bold">
              ...
              <div className="md:col-span-7 flex flex-col min-h-0">
                <div className="flex justify-between items-center mb-3 shrink-0">
                  <h4 className="text-xs font-black uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
                    <Users className="w-4 h-4 text-slate-400" /> Danh sách buổi
                    học
                  </h4>
                  ...
                  {classStudents.length === 0 ? (
                    <div className="text-center py-12">
                      ...
                      <p className="text-xs text-slate-400 font-bold">
                        Không có {entityLabel.singular} nào trong{" "}
                        {copy.entityNameLower} này.
                      </p>
                    </div>
                  ) : (
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            ...
                            isNew &&
                              "bg-emerald-500/15 border-emerald-500/20 scale-101 shadow-md z-10",
