...
export const ErpStatCard: React.FC<ErpStatCardProps> = ({
  name,
  value,
  change,
  icon: Icon,
      ...
      <div className="space-y-0.5">
        ...
        <h3
          className={cn(
            ...
            darkMode ? "text-white" : "text-slate-855",
          )}
        >
          {value}
        </h3>
...
export const ErpFilterTab: React.FC<ErpFilterTabProps> = ({
  active,
  onClick,
  children,
}) => {
...
export function ErpFilterRail({
  ...
  return (
    <div
      className={cn(
        "w-full md:w-[26rem] lg:w-[28rem] overflow-x-auto no-scrollbar",
        className,
      )}
    >
      <div className="flex min-w-max items-center gap-2 pr-1">{children}</div>
    </div>
  );
}
...
export function ErpField({
  ...
  return (
    <div className="space-y-1.5">
      <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">
        {label}
      </div>
      {children}
    </div>
...
export const erpInputClass = (darkMode: boolean) =>
  cn(
...
export function ErpInput(props: React.InputHTMLAttributes<HTMLInputElement>) {
  ...
  return (
    <input
      {...props}
      className={cn(erpInputClass(darkMode), props.className)}
    />
  );
}
...
export function ErpSubmitButton({
  children,
  ...props
...
export function ErpEmptyState({
  ...
  return (
    <div className="p-20 text-center text-slate-400 space-y-3">
      <div
        className={cn(
          "w-16 h-16 rounded-3xl flex items-center justify-center mx-auto text-slate-500",
          darkMode ? "bg-slate-800/40" : "bg-slate-100",
        )}
      >
...
export function ErpTableHead({ columns }: { columns: string[] }) {
  ...
  return (
    <thead>
      ...
      >
        {columns.map((col) => (
          <th key={col} className="py-2.5 px-4 text-left">
            {col}
          </th>
        ))}
      </tr>
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      >
        <div className="flex flex-col items-center text-center space-y-3">
          ...
          <p
            className={cn(
              "text-[11px] font-bold leading-relaxed px-2",
              darkMode ? "text-slate-400" : "text-slate-500",
            )}
          >
            {message}
