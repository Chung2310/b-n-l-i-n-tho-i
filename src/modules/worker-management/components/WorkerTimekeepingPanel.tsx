...
export function WorkerTimekeepingPanel({ projectId, workers, canManage = true }: { projectId: string; workers: Worker[]; canManage?: boolean }) {
  ...
  const reload = React.useCallback(async () => { setLoading(true); setError(""); try { setLogs(await workerAttendanceApi.list(projectId)); } catch (e) { setError(e instanceof Error ? e.message : "Không thể tải chấm công"); } finally { setLoading(false); } }, [projectId]);
  React.useEffect(() => { void reload(); }, [reload]);
  const mark = async (workerId: string) => { try { await workerAttendanceApi.mark({ projectId, workerId }); await reload(); } catch (e) { setError(e instanceof Error ? e.message : "Không thể chấm công"); } };
