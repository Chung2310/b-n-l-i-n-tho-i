...
export function WorkerNotificationsPage({ canManage = true }: { canManage?: boolean }) {
  ...
  const load = React.useCallback(async () => { setLoading(true); try { setItems((await workerNotificationsApi.list()) as NotificationItem[]); } catch (e) { setError(getApiErrorMessage(e, "Không thể tải thông báo")); } finally { setLoading(false); } }, []);
  React.useEffect(() => { void load(); }, [load]);
  const send = async (event: React.FormEvent) => { event.preventDefault(); if (!title.trim() || !content.trim()) return; setSending(true); setError(""); try { await workerNotificationsApi.create({ title: title.trim(), content: content.trim() }); setTitle(""); setContent(""); await load(); } catch (e) { setError(getApiErrorMessage(e, "Không thể gửi thông báo")); } finally { setSending(false); } };
