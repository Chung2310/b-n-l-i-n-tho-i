...
export function PartnerDetailModal({
  ...
  const entityLabel = {
    ...ENTITY_LABEL_PRESETS[entityPreset],
    preset: entityPreset,
  };
  const operationalCopy = getWorkerOperationalCopy(entityPreset);
  const [partner, setPartner] = useState<Partner | null>(null);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<"info" | "students" | "payouts">(
    "info",
  );
  ...
  const getStatusBadge = (statusArray: string[] | string) => {
    ...
    switch (status) {
      ...
      case "Trượt":
        return (
          <span className="px-2.5 py-1 bg-rose-50 text-rose-700 rounded-full text-[10px] font-bold">
            {getOperationalStatusLabel(entityLabel.preset, status)}
          </span>
        );
      case "Đang học":
        return (
          <span className="px-2.5 py-1 bg-blue-50 text-blue-700 rounded-full text-[10px] font-bold">
            {getOperationalStatusLabel(entityLabel.preset, status)}
          </span>
        );
      case "Chờ KSK":
      ...
      default:
        return (
          <span className="px-2.5 py-1 bg-slate-55 bg-slate-100 text-slate-700 rounded-full text-[10px] font-bold">
            {status || "Khác"}
          </span>
        );
    }
        ...
        <div className="space-y-6 text-left">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 bg-slate-50 p-6 rounded-3xl border border-slate-150/40">
            ...
            <div className="space-y-1 border-t sm:border-t-0 sm:border-l border-slate-200/60 pt-3 sm:pt-0 sm:pl-4">
              ...
              <p className="text-[10px] font-bold text-slate-500">
                Tổng hoa hồng: {formatVND(String(partner.totalCommission))}
              </p>
            </div>
            <div className="space-y-1 border-t sm:border-t-0 sm:border-l border-slate-200/60 pt-3 sm:pt-0 sm:pl-4">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">
          ...
          <div className="flex border-b border-slate-200/60 gap-4">
            ...
            >
              {entityLabel.titleCase} giới thiệu (
              {partner.referredStudentsCount})
            </button>
            <button
                ...
                <div className="space-y-4">
                  ...
                  {partner.bankAccountNo ? (
                      ...
                      <div className="flex items-center gap-3 text-xs font-semibold text-slate-600">
                        ...
                        <span>
                          ...
                          <span className="font-bold text-slate-800">
                            {getBankDisplayName(partner.bankName) || "N/A"}
                          </span>
                        </span>
                      </div>
                      <div className="flex items-center gap-3 text-xs font-semibold text-slate-600">
                    ...
                    <p className="text-xs font-bold text-slate-400 italic">
                      Chưa bổ sung tài khoản ngân hàng chi trả.
                    </p>
                  )}
                </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs font-semibold">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {partner.referredStudents.map(
                          (student: PartnerReferredStudent) => (
                            <tr
                              key={student._id}
                            ...
                            (payout: PartnerPayout) => (
                              ...
                              >
                                ...
                                <td className="py-4 px-6 text-slate-500 italic max-w-xs truncate">
                                  {payout.note || "(Không có)"}
                                </td>
                                <td className="py-4 px-6 text-right font-black text-slate-900">
                                  {formatVND(String(payout.amount))}
                                </td>
                              </tr>
                            ),
                          )}
