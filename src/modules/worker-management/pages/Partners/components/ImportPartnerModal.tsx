...
export function ImportPartnerModal({
  isOpen,
  onClose,
  onSuccess,
}: ImportPartnerModalProps) {
  ...
  const mapHeaders = (row: unknown[]): Record<string, number> => {
    ...
    row.forEach((cell, idx) => {
      ...
      if (
        val.includes("tên đối tác") ||
        val.includes("tên cộng tác viên") ||
        val === "tên" ||
        val === "name"
      )
        map.name = idx;
        ...
        val.includes("số tài khoản") ||
        val === "stk" ||
        val.includes("account no")
      )
        map.bankAccountNo = idx;
      else if (val.includes("chủ tài khoản") || val.includes("account name"))
        map.bankAccountName = idx;
      else if (val.includes("trạng thái") || val === "status")
        map.isActive = idx;
      else if (val.includes("ghi chú") || val === "notes") map.notes = idx;
      else if (
        val.includes("owner id") ||
        val.includes("center id") ||
        val.includes("mã trung tâm") ||
        val.includes("mã công ty") ||
        val.includes("company code")
      )
        map.centerId = idx;
    });
    return map;
    ...
    reader.onload = (e) => {
      try {
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          else if (seenPhones.has(partnerData.phone))
          ...
          if (
            ...
            !partnerOwnerIdRequired(partnerData)
          ) {
            errors.push(
              "Mã trung tâm / mã công ty (Owner ID / Company Code) là bắt buộc cho tài khoản superadmin",
            );
          }
        ...
        if (validationResults.length === 0)
          setErrorMsg("Không tìm thấy đối tác hợp lệ nào trong file.");
    ...
    try {
      ...
      setErrorMsg(
        error instanceof Error
          ? error.message
          : "Lỗi khi gửi yêu cầu nhập đối tác.",
      );
    } finally {
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-sm mt-1">
                    Đã xử lý xong danh sách đối tác từ file{" "}
                    ...
                    <table className="w-full text-left text-xs min-w-[900px]">
                      <thead className="bg-slate-50 border-b border-slate-100 sticky top-0 z-10">
                        <tr>
                          ...
                          <th className="px-3 py-3 text-slate-400 font-bold w-32">
                            Ngân hàng
                          </th>
                          <th className="px-3 py-3 text-slate-400 font-bold w-32">
                            Số tài khoản
                          </th>
                          <th className="px-3 py-3 text-slate-400 font-bold text-center w-28">
                            Trạng thái
                          </th>
                      ...
                      <tbody className="divide-y divide-slate-100 font-medium">
                        {validationRows.map((row) => (
                          <tr
                            key={row.rowNum}
                            className={`hover:bg-slate-50/30 ${!row.isValid ? "bg-rose-50/10" : ""}`}
                          >
          ...
          {!importResult && (
            <div className="flex items-center justify-end gap-4 px-8 py-5 border-t border-slate-100 flex-shrink-0">
              <button
                ...
                onClick={onClose}
                disabled={isUploading}
                className="text-xs font-bold text-slate-450 hover:text-slate-650 transition-colors disabled:opacity-50"
              >
                Hủy
              </button>
              {validationRows.length > 0 && (
                ...
                >
                  ...
                  {isUploading
                    ? "Đang nhập dữ liệu..."
                    : totalValid > 0
                      ? `Nhập ${totalValid} đối tác hợp lệ`
