...
export function ExamsPage({
  ...
  const [statusModalExam, setStatusModalExam] = useState<ExamSession | null>(
    null,
  );
  const [assignModalExam, setAssignModalExam] = useState<ExamSession | null>(
    null,
  );
  const [deleteModalExam, setDeleteModalExam] = useState<ExamSession | null>(
    null,
  );
  ...
  const stats = {
    ...
    upcoming: exams.filter(
      (e) => e.status === "Sắp diễn ra" || e.status === "Đã xác nhận",
    ).length,
    confirmed: exams.filter((e) => e.status === "Đã xác nhận").length,
    completed: exams.filter((e) => e.status === "Đã hoàn thành").length,
    unassignedStudents: students.filter((s) => s.status.includes("Đang học"))
  ...
  const rankOptions = React.useMemo(() => {
    const ranks = [
      ...new Set(exams.map((e) => e.rank).filter(Boolean)),
    ] as string[];
  ...
  const filteredExams = exams.filter((exam) => {
    if (hasRankData && rankFilter !== "Tất cả hạng" && exam.rank !== rankFilter)
      return false;
  ...
  const handleExport = () => {
    ...
    try {
      ...
      XLSX.writeFile(
        wb,
        `danh_sach_lich_thi_${new Date().toLocaleDateString("vi-VN").replace(/\//g, "-")}.xlsx`,
      );
  ...
  return (
    <div className="space-y-4">
      ...
      {activeTab === "exams" ? (
          ...
          <div className="space-y-3">
            {examsLoading ? (
              <div className="py-20 text-center text-slate-400 text-sm italic">
                Đang nạp dữ liệu đợt thi...
              </div>
            ) : paginatedExams.length === 0 ? (
              <div className="py-20 bg-white rounded-3xl border border-slate-100 text-center text-slate-400 text-sm italic">
              ...
              paginatedExams.map((exam) => (
                <ExamCard
                  ...
                  onUnassignStudent={async (studentId) => {
                    try {
                      ...
                      toast.error(
                        getApiErrorMessage(
                          error,
                          "Có lỗi xảy ra khi xóa học viên khỏi đợt thi.",
                        ),
                      );
                    }
                  }}
                  onUpdateStudentResult={async (studentId, overallResult) => {
                    try {
                      await apiFetch(
                        `/exams/${exam.id}/students/${studentId}/result`,
                        {
                          method: "POST",
                      ...
                      toast.error(
                        getApiErrorMessage(
                          error,
                          "Có lỗi xảy ra khi cập nhật kết quả thi.",
                        ),
                      );
                    }
                  }}
        ...
        <div className="py-20 bg-white rounded-3xl border border-slate-100 text-center text-slate-400 text-sm italic">
          Danh sách học viên chưa có lịch thi.
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          >
            ...
            <p className="text-slate-500 text-sm font-medium mb-8 leading-relaxed">
              Bạn có chắc chắn muốn xóa đợt thi{" "}
              <span className="font-bold text-slate-800">
                "{deleteModalExam.name}"
              </span>
  ...
  return (
    <div className="bg-white p-1.5 px-2.5 rounded-lg border border-slate-100 shadow-sm flex items-center gap-2.5">
      ...
      <div>
        ...
        <p className="text-[9px] font-semibold text-slate-550 uppercase tracking-wider mt-0.5">
          {label}
        </p>
      </div>
    </div>
...
function FilterItem({ label, children }: FilterItemProps) {
  return (
    <div className="space-y-0.5">
      <label className="text-[9px] font-bold text-slate-550 uppercase tracking-wider">
        {label}
      </label>
