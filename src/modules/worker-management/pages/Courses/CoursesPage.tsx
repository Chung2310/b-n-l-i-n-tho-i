...
export function CoursesPage({
  ...
  const getFieldLabel = (fieldKey: string, defaultLabel: string) => {
    ...
    if (entityLabel.preset === "worker" || entityLabel.preset === "customer")
      return defaultLabel;
    const fieldConfig = stdFields.find((f) => f.key === fieldKey);
  ...
  const { categories, loading: categoriesLoading } =
    useCourseCategories(resolvedCenter);
  ...
  const [newCourse, setNewCourse] =
    useState<NewCourseFormState>(DEFAULT_NEW_COURSE);
  ...
  const handleAddCourse = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    stdFields.forEach((f) => {
      if (f.isVisible && !f.isArchived && f.isRequired) {
        ...
        if (f.key === "category" && !newCourse.category)
          missingFields.push(f.label);
        if (f.key === "duration" && !newCourse.duration)
          missingFields.push(f.label);
        if (f.key === "fee" && !newCourse.fee) missingFields.push(f.label);
        if (f.key === "maxLearners" && !newCourse.maxLearners)
          missingFields.push(f.label);
      }
    });
    ...
    try {
      const payload: CreateCoursePayload = {
        ...
        fee:
          usesCourseFeePolicy && isFieldVisible("fee")
            ? `${formatVND(newCourse.fee)}d`
            : "0d",
  ...
  const handleEditCourse = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    stdFields.forEach((f) => {
      if (f.isVisible && !f.isArchived && f.isRequired) {
        ...
        if (f.key === "category" && !editForm.category)
          missingFields.push(f.label);
        if (f.key === "duration" && !editForm.duration)
          missingFields.push(f.label);
        if (f.key === "fee" && !editForm.fee) missingFields.push(f.label);
        if (f.key === "maxLearners" && !editForm.maxLearners)
          missingFields.push(f.label);
      }
    });
  ...
  const handleToggleStatus = async (course: Course) => {
    const nextStatus: Course["status"] =
  ...
  const handleAddCategory = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const msg =
        error instanceof Error ? error.message : copy.categoryCreateError;
      toast.error(msg);
    } finally {
  ...
  const handleDeleteCategory = async () => {
    ...
    try {
      await apiFetch<MutationResponse>(
        `/courses/categories/${deleteConfirm.id}`,
        {
          method: "DELETE",
        },
      );
    ...
    <div className="space-y-4 text-left">
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div
            className={cn(
              "flex items-center border p-0.5 rounded-lg gap-0.5 shrink-0 shadow-sm",
              darkMode
      ...
      {loading && courses.length === 0 ? (
        <ErpCard>
          <ErpLoadingState message={copy.loadingMessage} />
        </ErpCard>
            ...
            {paginatedCourses.map((course) => (
              ...
              >
                <div className="space-y-2">
                  ...
                  <h4
                    className={cn(
                      "text-sm font-black line-clamp-1 transition-colors",
                      darkMode
                        ? "text-slate-100 group-hover:text-white"
                        : "text-slate-850 group-hover:text-slate-950",
                    )}
                  >
                    {course.title}
                  </h4>
                  ...
                  <div
                    className={cn(
                      "grid grid-cols-2 gap-y-2.5 gap-x-2 pt-2 text-[10px] font-bold border-t",
                      darkMode
                        ? "text-slate-400 border-slate-800/30"
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {copy.capacitySummary(course.maxLearners)}
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Layers className="w-3.5 h-3.5 text-slate-400" />{" "}
                      {course.activeBatches} {copy.activeBatchUnit}
                    </div>
                  </div>
                </div>
                ...
                <div
                  className={cn(
                    "flex items-center justify-between pt-3 mt-2 border-t",
                    darkMode ? "border-slate-800/30" : "border-slate-100",
                  )}
                >
                  <span
                    className={cn(
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    >
                      {course.status === ACTIVE_COURSE_STATUS ? (
                        ...
                        <>
                          <Play className="w-3 h-3 text-brand-primary" /> Kích
                          hoạt
                        </>
                      )}
                    </button>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left border-collapse">
              <ErpTableHead
                columns={[
                  "Mã",
                  copy.tableTitleColumn,
                  copy.categoryLabel,
                  copy.durationLabel,
                  courseFeeLabel,
                  copy.tableCapacityColumn,
                  "Trạng thái",
                  "Thao tác",
                ]}
              />
                ...
                {paginatedCourses.map((course) => (
                  ...
                  >
                    ...
                    <td className="py-2 px-4 font-bold">
                      {usesCourseFeePolicy ? course.fee : "Không áp dụng"}
                    </td>
                    <td className="py-2 px-4 font-bold">
                      {copy.tableCapacitySummary(
                        course.maxLearners,
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          className={cn(
                            "p-1 rounded-lg transition-all border cursor-pointer shadow-sm",
                            darkMode
                              ? "bg-slate-800 hover:bg-indigo-900/40 text-slate-450 hover:text-indigo-455 border-transparent"
                              : "bg-slate-50 hover:bg-indigo-50 text-slate-450 hover:text-indigo-550 border-slate-200/60",
                          )}
                          title={copy.editTitle}
                        >
                          <Pencil className="w-3 h-3" />
                        ...
                        <button
                          ...
                          className={cn(
                            "p-1 rounded-lg transition-all border cursor-pointer shadow-sm",
                            darkMode
                              ? "bg-slate-800 hover:bg-rose-900/40 text-slate-450 hover:text-rose-455 border-transparent"
          ...
          <form onSubmit={handleAddCourse} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {isFieldVisible("category") && (
                <div className="relative group/std">
                  ...
                  <ErpField
                    label={getFieldLabel("category", copy.categoryLabel)}
                  >
                    <ErpSelect
              ...
              {isFieldVisible("duration") && (
                <div className="relative group/std">
                  ...
                  <ErpField
                    label={getFieldLabel("duration", copy.durationLabel)}
                  >
                    <ErpInput
                      type="text"
            ...
            <div className="grid grid-cols-2 gap-4">
              {isFieldVisible("fee") && (
                <div className="relative group/std">
                  ...
                  <ErpField
                    label={getFieldLabel("fee", `${copy.feeLabel} (VND)`)}
                  >
                    <ErpInput
                      type="text"
              ...
              {isFieldVisible("maxLearners") && (
                <div className="relative group/std">
                  ...
                  <ErpField
                    label={getFieldLabel("maxLearners", copy.capacityLabel)}
                  >
                    <ErpInput
                      type="number"
                      min={0}
                      ...
                      onChange={(e) => {
                        ...
                        setNewCourse({
                          ...newCourse,
                          maxLearners: Number.isNaN(parsed)
                            ? 20
                            : Math.max(0, parsed),
                        });
                      }}
                      onBlur={() => {
                        if (
                          newCourse.maxLearners === "" ||
                          typeof newCourse.maxLearners !== "number" ||
                          newCourse.maxLearners < 0
                        ) {
            ...
            <CustomFieldsSection
              ...
              onChange={(customFields) =>
                setNewCourse((previous) => ({ ...previous, customFields }))
              }
              mode="create"
            ...
            <ErpSubmitButton>
              {isSubmitting ? copy.creatingSubmit : copy.createSubmit}
            </ErpSubmitButton>
          </form>
        </ErpModal>
      )}
      ...
      {editingCourse && (
        <ErpModal
          title={copy.editModalTitle(editingCourse.code)}
        ...
        >
          <form onSubmit={handleEditCourse} className="space-y-4">
                ...
                <div className="relative group/std">
                  ...
                  >
                    <ErpInput
                      ...
                      onBlur={() => {
                        if (
                          editForm.maxLearners === "" ||
                          typeof editForm.maxLearners !== "number" ||
                          editForm.maxLearners < 0
                        ) {
            ...
            <CustomFieldsSection
              ...
              onChange={(customFields) =>
                setEditForm((previous) => ({ ...previous, customFields }))
              }
              mode="edit"
              disabled={isSubmitting}
            ...
            <ErpSubmitButton disabled={isSubmitting}>
              {isSubmitting ? copy.updatingSubmit : copy.updateSubmit}
            </ErpSubmitButton>
          </form>
        </ErpModal>
      )}
