...
import {
  getBatchPageCopy,
  getBatchStatusLabel,
} from "../../config/workerRecruitmentCopy";
...
const formatDays = (days: number[]) =>
  DAY_OPTIONS.filter((d) => (days || []).includes(d.value))
    .map((d) => d.label)
    .join(", ");
...
export function BatchesPage({
  ...
  const statusLabel = (status: BatchStatus) =>
    getBatchStatusLabel(entityLabel.preset, status);
  ...
  React.useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        const data = await authService.getUsersByCompany(
          companyCode,
          activeBranchId,
        );
        setUsers(data || []);
  ...
  const [attendanceBatchId, setAttendanceBatchId] = useState<string | null>(
    null,
  );
  const [viewAttendanceBatchId, setViewAttendanceBatchId] = useState<
    string | null
  >(null);
  ...
  const manageBatch = manageLearnersId
    ? batches.find((b) => b.id === manageLearnersId)
    : undefined;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    stdFields.forEach((f) => {
      if (f.isVisible && !f.isArchived && f.isRequired) {
        ...
        if (f.key === "startDate" && !form.startDate)
          missingFields.push(f.label);
    ...
    try {
      ...
      if (entityLabel.preset === "worker" && !resolvedCourseId) {
        if (courses.length > 0) {
          ...
          if (
            newCourseRes?.success &&
            (newCourseRes?.data?.id || newCourseRes?.data?._id)
          ) {
            resolvedCourseId = newCourseRes.data.id || newCourseRes.data._id;
          } else {
            throw new Error(
              "Không thể tự động tạo danh mục tuyển dụng mặc định.",
            );
          }
        }
      }
      ...
      const payload = {
        ...
        ...(editingId
          ? {
              expectedVersion: batches.find((batch) => batch.id === editingId)
                ?.__v,
            }
          : {}),
        ...(!editingId && resolvedCenter
          ? { companyCode: resolvedCenter }
          : {}),
      };
      if (editingId) {
        await apiFetch(`/batches/${editingId}`, {
          method: "PATCH",
        ...
        toast.success(
          `Đã tạo ${copy.entityNameLower} ${payload.code} thành công!`,
        );
      }
      notifyBatchMutation();
      ...
      const msg =
        error instanceof Error
          ? error.message
          : `Có lỗi xảy ra khi lưu ${copy.entityNameLower}.`;
      toast.error(msg);
    } finally {
  ...
  const handleChangeStatus = async (batch: Batch, status: BatchStatus) => {
    try {
      ...
      notifyBatchMutation();
      toast.success(
        `${copy.entityName} ${batch.code} đã chuyển sang "${statusLabel(status)}".`,
      );
    } catch (error: unknown) {
      const msg =
        error instanceof Error
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const msg =
        error instanceof Error
          ? error.message
          : `Có lỗi xảy ra khi xóa ${copy.entityNameLower}.`;
      toast.error(msg);
    } finally {
  ...
  const handleAddLearner = async () => {
    ...
    try {
      ...
      toast.success(
        `Đã thêm ${entityLabel.singular} vào ${copy.entityNameLower}.`,
      );
    } catch (error: unknown) {
      const msg =
        error instanceof Error
          ? error.message
          : `Có lỗi xảy ra khi thêm ${entityLabel.singular}.`;
      toast.error(msg);
    }
  };
  ...
  const handleRemoveLearner = async (studentId: string) => {
    ...
    try {
      await apiFetch(`/batches/${manageBatch.id}/learners/${studentId}`, {
        method: "DELETE",
      });
      notifyBatchMutation();
      toast.success(
        `Đã bỏ ${entityLabel.singular} khỏi ${copy.entityNameLower}.`,
      );
    } catch (error: unknown) {
      const msg =
        error instanceof Error
          ? error.message
          : `Có lỗi xảy ra khi bỏ ${entityLabel.singular}.`;
      toast.error(msg);
    }
  };
  ...
  return (
    <div className="space-y-4 text-left">
      ...
      {loading && batches.length === 0 ? (
        <ErpCard>
          <ErpLoadingState
            message={`Đang tải danh sách ${copy.entityNameLower}...`}
          />
        </ErpCard>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left border-collapse">
              ...
              <tbody
                className={cn(
                  "divide-y",
                  darkMode ? "divide-slate-800/30" : "divide-slate-100",
                )}
              >
                {paginatedBatches.map((b) => (
                  <tr
                    ...
                    className={cn(
                      ...
                      darkMode
                        ? "text-slate-355 hover:bg-slate-800/10"
                        : "text-slate-600",
                    )}
                  >
                    ...
                    <td className="py-2 px-4">
                      ...
                      >
                        ...
                        {b.learnerIds.length}
                        {b.maxLearners ? `/${b.maxLearners}` : ""} HV
                      </button>
                    </td>
                    <td className="py-2 px-4">
                      <select
                        value={b.status}
                        onChange={(e) =>
                          handleChangeStatus(b, e.target.value as BatchStatus)
                        }
                        className={cn(
                      ...
                      <div className="flex items-center gap-1.5">
                        <button
                          ...
                          className={cn(
                            ...
                            darkMode
                              ...
                              : "bg-slate-50 hover:bg-slate-100 text-slate-450 hover:text-slate-700 border-slate-200/60",
                          )}
                        >
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            darkMode
                              ...
                              : "bg-slate-50 hover:bg-rose-50 text-slate-450 hover:text-rose-550 border-slate-200/60",
                          )}
                        >
                        ...
                        <button
                          ...
                          className={cn(
                            "p-1 rounded-lg transition-all border cursor-pointer shadow-sm",
                            darkMode
                              ? "bg-slate-800 hover:bg-brand-primary/20 text-slate-450 hover:text-brand-primary border-transparent"
                              : "bg-slate-50 hover:bg-brand-primary/10 text-slate-450 hover:text-brand-primary border-slate-200/60",
                          )}
                        >
                        ...
                        <button
                          ...
                          className={cn(
                            "p-1 rounded-lg transition-all border cursor-pointer shadow-sm",
                            darkMode
                              ? "bg-slate-800 hover:bg-emerald-900/40 text-slate-450 hover:text-emerald-400 border-transparent"
                              : "bg-slate-50 hover:bg-emerald-50 text-slate-450 hover:text-emerald-600 border-slate-200/60",
                          )}
                        >
                        ...
                        <button
                          ...
                          className={cn(
                            "p-1 rounded-lg transition-all border cursor-pointer shadow-sm",
                            darkMode
                              ? "bg-slate-800 hover:bg-sky-900/40 text-slate-450 hover:text-sky-400 border-transparent"
      ...
      {showFormModal && (
        <ErpModal
          title={editingId ? copy.editTitle : copy.createTitle}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
                      ...
                      <div className="relative">
                        ...
                        >
                          <option
                            value=""
                            disabled
                          >{`-- Chọn ${copy.courseLabel.toLocaleLowerCase("vi")} --`}</option>
                          {courses.map((c) => (
              ...
              {isFieldVisible("teacherId") && (
                <div className="relative group/std">
                  ...
                  <ErpField
                    label={getFieldLabel("teacherId", copy.instructorLabel)}
                  >
                    <InstructorCombobox
            ...
            <div className="space-y-4 pt-2">
              ...
              {isFieldVisible("room") && (
                <div className="relative group/std">
                  ...
                  <ErpField
                    label={getFieldLabel("room", "Địa điểm (tùy chọn)")}
                  >
                    <div className="relative">
                      <ErpInput
                        type="text"
                        ...
                        placeholder={getFieldPlaceholder(
                          "room",
                          entityLabel.preset === "worker"
                            ? "Ví dụ: Công trường số 2 / Nhà máy A"
                            : "Ví dụ: Phòng 201 / Sân tập số 2",
                        )}
                        value={form.location}
                        onChange={(e) =>
            ...
            <CustomFieldsSection
              ...
              onChange={(customFields) =>
                setForm((previous) => ({ ...previous, customFields }))
              }
      ...
      {manageBatch && (
        ...
        >
          <div className="space-y-6">
            ...
            >
              {manageBatch.courseTitle} • {copy.capacityLabel}:{" "}
              {manageBatch.learnerIds.length}
              {manageBatch.maxLearners
                ? `/${manageBatch.maxLearners}`
                : ""}{" "}
            ...
            <div className="space-y-2">
              ...
              {enrolledStudents.length === 0 ? (
                <p className="text-xs text-slate-400">
                  {copy.entityName} chưa có {entityLabel.singular} nào.
                </p>
              ) : (
                <div
                  className={cn(
                    "border rounded-xl p-1 max-h-72 overflow-y-auto divide-y",
                    darkMode
                      ? "border-slate-800 divide-slate-800/40"
                      : "border-slate-100 divide-slate-100/60",
                  )}
                >
      ...
      {assignmentBatchId &&
        (() => {
          const assignmentBatch = batches.find(
            (b) => b.id === assignmentBatchId,
          );
          if (!assignmentBatch) return null;
          const batchStudents = students.filter((s) =>
            assignmentBatch.learnerIds.includes(s.id),
          );
          return (
            <AssignmentModal
              isOpen={true}
