...
import {
  ...
  usesEducationBilling as presetUsesEducationBilling,
...
function categoryIcon(
  ...
  if (n.includes("ngoại ngữ") || n.includes("ngoai ngu") || n.includes("tiếng"))
    return Languages;
...
export function StudentsPage({
  ...
  const studentCategories = useMemo(() => {
    const categoryByCourseId = new Map<string, string>(
      courses.map((c) => [c.id, c.category]),
    );
  ...
  const getBatchLabels = (studentId: string) =>
    (studentBatches.get(studentId) || [])
      .map((b) => b.code || b.courseTitle)
      .filter(Boolean);
  ...
  const filteredStudents = students.filter((student) => {
    ...
    if (selectedStatus !== "Tất cả") {
      ...
      const studentStatuses = Array.isArray(student.status)
        ? student.status
        : [student.status];
    ...
    if (
      hasRankData &&
      rankFilter !== "Tất cả hạng" &&
      student.rank !== rankFilter
    )
      return false;
    ...
    if (usesEducationBilling && feeStatusFilter !== "Tất cả học phí") {
      const totalFeeNum =
        parseInt(String(student.fee).replace(/\D/g, ""), 10) || 0;
  ...
  const handleBulkDelete = async () => {
    ...
    if (
      !window.confirm(
        `Bạn có chắc chắn muốn xóa ${selectedStudentIds.length} ${entityLabel.singular} đã chọn? ${relatedDataWarning}`,
      )
    ) {
      return;
    }
  ...
  const handleExport = () => {
    ...
    const commonHeadersAfter = [
      ...
      "Ngày đăng ký",
      ...(usesEducationBilling ? ["Trạng thái học phí"] : []),
      "Trạng thái",
      "Lớp / Dự án",
      "Người phụ trách",
      "Ngày sinh",
    ...
    const getCommonRowDataAfter = (student: Student) => {
      ...
      return [
        ...
        (Array.isArray(student.status) ? student.status : [student.status])
          .map((status) =>
            getOperationalStatusLabel(entityLabel.preset, status),
          )
          .join(", "),
  ...
  const handlePrint = () => {
    ...
    if (!printWindow) {
      toast.warning(
        "Trình duyệt đã chặn cửa sổ bật lên. Vui lòng cho phép bật lên để in hoặc mở ứng dụng trong tab mới.",
      );
      return;
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
        <div>
          <h1 className="text-lg font-bold text-cyan-700 tracking-tight">
        ...
        <div className="flex items-center gap-1.5">
          {userProfile?.role === "admin" && (
            <button
              onClick={() => {
                setListScope(isUnassignedScope ? "branch" : "unassigned");
                setSelectedStudentIds([]);
              }}
              className={cn(
                "flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[11px] font-bold transition-all cursor-pointer shadow-sm",
                isUnassignedScope
          ...
          {canManage && usesEducationBilling && (
            <button
        ...
        {[
          ...
          >
            <item.icon
              className={cn(
                "w-4 h-4",
                category === item.id ? "text-cyan-600" : "text-slate-400",
              )}
            />
            {item.id === TAB_UNASSIGNED
              ? operationalCopy.unassignedGroupLabel
              : item.id}
            {category === item.id && (
              <motion.div
                layoutId="catLine"
          ...
          <table className="w-full text-left min-w-[1100px]">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                      ...
                      if (e.target.checked) {
                        ...
                        setSelectedStudentIds(
                          selectedStudentIds.filter(
                            (id) => !paginatedStudents.some((s) => s.id === id),
                          ),
                        );
                      }
                    }}
                ...
                <th className="px-3 py-2 text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                  Họ và tên
                </th>
                <th className="px-3 py-2 text-[9px] font-bold text-slate-400 uppercase tracking-widest text-center">
                  Ngày ĐK
                </th>
                <th className="px-3 py-2 text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                  Lớp / Dự án
                ...
                <th className="px-3 py-2 text-[9px] font-bold text-slate-400 uppercase tracking-widest text-right no-print">
                  Thao tác
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                <tr>
                  <td
                    colSpan={usesEducationBilling ? 8 : 7}
                    className="px-4 py-16 text-center text-slate-400 text-xs italic"
                  >
                    Đang nạp dữ liệu...
                  </td>
                </tr>
                ...
                <tr>
                  <td
                    ...
                    className="px-4 py-16 text-center text-slate-400 text-xs italic"
                  >
                    Không tìm thấy {entityLabel.singular} nào phù hợp với bộ
                    lọc.
                  </td>
                </tr>
              ) : (
                  ...
                  >
                        ...
                        <div className="flex flex-wrap items-center gap-x-2 text-[10px] font-medium text-slate-400">
                          ...
                          {student.idCard && (
                            <>
                              ...
                              <span className="bg-slate-100 px-1 py-0.5 rounded text-[9px] text-slate-600 font-semibold">
                                CCCD: {student.idCard}
                              </span>
                            </>
                          )}
                          ...
                          {getBranchLabel(student.branchId) ? (
                            <span className="bg-cyan-50 px-1 py-0.5 rounded text-[9px] text-cyan-700 font-semibold">
                              {getBranchLabel(student.branchId)}
                            </span>
                          ) : (
                            <span className="bg-amber-50 px-1 py-0.5 rounded text-[9px] text-amber-700 font-semibold">
                              Chưa gán chi nhánh
                            </span>
                          )}
                          {student.birthday && (
                    ...
                    <td className="px-3 py-1.5 text-[11px] font-medium text-slate-500 whitespace-nowrap">
                      {student.createdByName || (
                        <span className="text-slate-300">Chưa xác định</span>
                      )}
                    </td>
                        ...
                        <div className="flex flex-col gap-0.5 w-24">
                          <div className="flex items-center text-[10px] font-bold">
                            {(() => {
                              const totalFeeNum =
                                parseInt(
                                  String(student.fee).replace(/\D/g, ""),
                                  10,
                                ) || 0;
                              const paidSoFar = student.paidAmount || 0;
                              const remaining = totalFeeNum - paidSoFar;
                              return (
                                <>
                                  {totalFeeNum === 0 ? (
                                    <span className="text-rose-500/80 whitespace-nowrap">
                                      Chưa đóng
                                    </span>
                                  ) : remaining > 0 ? (
                    ...
                    <td className="px-3 py-1.5 no-print">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button
                          ...
                          title="Sửa thông tin"
                          className={cn(
                            "p-1 rounded-lg text-slate-400 hover:text-cyan-600 hover:bg-cyan-50 transition-colors cursor-pointer",
                            isUnassignedScope && "hidden",
                          )}
                        >
                        ...
                        >
                          <button
                            ...
                            disabled={
                              isDeleting === student.id || isUnassignedScope
                            }
                            title="Xóa"
                            className={cn(
