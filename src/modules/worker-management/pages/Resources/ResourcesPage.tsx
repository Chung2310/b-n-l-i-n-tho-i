...
export function ResourcesPage({ canManage = true }: { canManage?: boolean }) {
  ...
  const [bookingResource, setBookingResource] = useState<ResourceItem | null>(
    null,
  );

  const [viewMode, setViewMode] = useState<"list" | "grid">(() => {
    return (
      (localStorage.getItem("erp_view_mode_resources") as "list" | "grid") ||
      "grid"
    );
  });
  ...
  const handleCancelBooking = async (
    ...
    try {
      await apiFetch(
        `/student-resources/${resource.id}/bookings/${bookingId}`,
        { method: "DELETE" },
      );
      ...
      const msg =
        error instanceof Error ? error.message : "Có lỗi xảy ra khi hủy lịch.";
      toast.error(msg);
    }
  };
  ...
  const handleToggleMaintenance = async (resource: ResourceItem) => {
    const nextStatus =
      resource.status === "MAINTENANCE" ? "AVAILABLE" : "MAINTENANCE";
    try {
    ...
    <div className="space-y-6 text-left">
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div
            className={cn(
              "flex items-center border p-1 rounded-xl gap-0.5 shrink-0",
              darkMode
          ...
          >
            <button
              ...
              onClick={() => {
                setViewMode("list");
                localStorage.setItem("erp_view_mode_resources", "list");
              }}
              className={cn(
            ...
            <button
              ...
              onClick={() => {
                setViewMode("grid");
                localStorage.setItem("erp_view_mode_resources", "grid");
              }}
              className={cn(
                "p-1.5 rounded-lg active:scale-95 transition-all cursor-pointer",
                viewMode === "grid"
      ...
      {loading && resources.length === 0 ? (
        <ErpCard>
          <ErpLoadingState message="Đang tải danh sách tài nguyên..." />
        </ErpCard>
