...
function ResourceTableRow({
  ...
  return (
    ...
    >
      <td className="py-2.5 px-4 font-black text-sm text-left">
        {resource.identifier}
      </td>
        ...
        {resource.status === "MAINTENANCE" ? (
          ...
          <div className="space-y-0.5">
            <span className="inline-flex items-center gap-1 text-brand-primary font-black text-[10px]">
              <Clock className="w-3 h-3" /> Đang bận ({currentBooking.startTime}
              -{currentBooking.endTime})
            </span>
            <div className="text-[9px] text-slate-400 font-bold truncate max-w-xs">
              {currentBooking.purpose} ({currentBooking.by})
            </div>
          </div>
        ) : (
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={cn(
                ...
                darkMode
                  ? "text-slate-400 hover:text-cyan-400 bg-slate-800 hover:bg-slate-700 border-transparent"
                  : "text-slate-500 hover:text-cyan-600 bg-slate-50 hover:bg-cyan-50 border-slate-200/60",
              )}
            >
            ...
            <button
              ...
              className={cn(
                "p-1.5 rounded-lg transition-colors border cursor-pointer",
                darkMode
                  ? "text-slate-400 hover:text-amber-400 bg-slate-800 hover:bg-slate-700 border-transparent"
                  : "text-slate-500 hover:text-amber-500 bg-slate-50 hover:bg-slate-100 border-slate-200/60",
              )}
            >
            ...
            <button
              ...
              className={cn(
                "p-1.5 rounded-lg transition-colors border cursor-pointer",
                darkMode
                  ? "text-slate-450 hover:text-rose-455 bg-slate-800 hover:bg-rose-900/40 border-transparent"
                  : "text-slate-500 hover:text-rose-500 bg-slate-50 hover:bg-rose-550 border-slate-200/60",
              )}
            >
