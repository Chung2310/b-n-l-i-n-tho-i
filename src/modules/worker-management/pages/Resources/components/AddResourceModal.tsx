...
export function AddResourceModal({
  ...
  const handleAddResource = async (e: React.FormEvent) => {
    ...
    stdFields.forEach((f) => {
      if (f.isVisible && !f.isArchived && f.isRequired) {
        ...
        if (f.key === "identifier" && !newResource.identifier)
          missingFields.push(f.label);
        if (f.key === "capacity" && !newResource.capacity)
          missingFields.push(f.label);
      }
    });
    ...
    try {
      await apiFetch(
        resource ? `/student-resources/${resource.id}` : "/student-resources",
        {
      ...
      const msg =
        error instanceof Error
          ? error.message
          : "Có lỗi xảy ra khi khai báo tài nguyên.";
      toast.error(msg);
    } finally {
  ...
  return (
    <ErpModal
      title={resource ? "Cập nhật tài nguyên" : "Khai báo tài nguyên mới"}
      onClose={onClose}
    >
      <form onSubmit={handleAddResource} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          {isFieldVisible("identifier") && (
            <div className="relative group/std">
              ...
              <ErpField
                label={getFieldLabel("identifier", "Mã nhận diện / Số phòng")}
              >
                <ErpInput
                  type="text"
        ...
        {isFieldVisible("capacity") && (
          <div className="relative group/std">
            ...
            <ErpField
              label={getFieldLabel("capacity", "Sức chứa / Khả năng đáp ứng")}
            >
              <ErpInput
                type="text"
        ...
        <CustomFieldsSection
          ...
          onChange={(customFields) =>
            setNewResource((previous) => ({ ...previous, customFields }))
          }
