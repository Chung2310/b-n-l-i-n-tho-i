...
export function ResourceCard({
  ...
  const currentBooking = useMemo(() => {
    ...
    return resource.bookings.find(
      (b) => b.date === today && b.startTime <= hhmm && b.endTime > hhmm,
    );
  }, [resource.bookings]);
  ...
  const upcomingBookings = useMemo(() => {
    ...
    return resource.bookings
      .filter((b) => {
        ...
        if (b.date === today && b.startTime <= hhmm && b.endTime > hhmm)
          return false;
        return true;
      })
      .sort((a, b) =>
  ...
  return (
    ...
    >
      <div className="space-y-3">
        ...
        <h4
          className={cn(
            "text-xs font-black line-clamp-1 text-left mt-1",
        ...
        >
          ...
          <span
            className={cn(
              "font-black",
              darkMode ? "text-slate-200" : "text-slate-705",
            )}
          >
            {resource.capacity}
          </span>
        ...
        >
          {resource.status === "MAINTENANCE" ? (
            ...
            <div
              className={cn(
                "p-2 border rounded-lg space-y-1 text-left",
                darkMode
                  ? "bg-brand-primary/10 border-brand-primary/20"
                  : "bg-cyan-50/50 border-cyan-100/60",
              )}
            >
              <div className="flex items-center gap-1.5 text-brand-primary text-[9px] font-black uppercase">
                <Clock className="w-3 h-3" />
                <span>
                  Đang bận: {currentBooking.startTime} -{" "}
      ...
      {canManage && (
        <div
          className={cn(
            "flex items-center justify-between pt-2.5 mt-1.5 border-t",
            darkMode ? "border-slate-800/30" : "border-slate-100",
          )}
        >
          <div className="flex items-center gap-1">
            <button
              ...
              className={cn(
                ...
                darkMode
                  ? "text-slate-500 hover:text-cyan-400 bg-slate-800 hover:bg-slate-700 border-transparent"
                  : "text-slate-400 hover:text-cyan-600 bg-slate-55 hover:bg-cyan-50 border-slate-205",
              )}
            >
            ...
            <button
              ...
              className={cn(
                "p-1 rounded-md transition-colors border cursor-pointer",
                darkMode
                  ? "text-slate-500 hover:text-amber-400 bg-slate-800 hover:bg-slate-700 border-transparent"
                  : "text-slate-400 hover:text-amber-500 bg-slate-55 hover:bg-slate-100 border-slate-205",
              )}
            >
            ...
            <button
              ...
              className={cn(
                "p-1 rounded-md transition-colors border cursor-pointer",
                darkMode
                  ? "text-slate-500 hover:text-rose-455 bg-slate-800 hover:bg-slate-750 border-transparent"
                  : "text-slate-400 hover:text-rose-500 bg-slate-55 hover:bg-rose-50 border-slate-205",
              )}
            >
