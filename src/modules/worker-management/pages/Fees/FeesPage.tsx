...
export function FeesPage({ onSelectStudent, selectedCenter }: FeesPageProps) {
  ...
  const hasRankData = React.useMemo(
    () => students.some((s) => s.rank && s.rank.trim() !== ""),
    [students],
  );
  ...
  const handleExport = () => {
    ...
    link.setAttribute(
      "download",
      `bao_cao_hoc_phi_${new Date().toLocaleDateString("vi-VN").replace(/\//g, "-")}.csv`,
    );
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead>
              <tr className="bg-slate-50/50">
                <th className="px-3 py-2 text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                  Học viên
                </th>
                {hasRankData && (
                  <th className="px-3 py-2 text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-center">
                    Hạng
                  </th>
                )}
                <th className="px-3 py-2 text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-right whitespace-nowrap">
                  Tổng HP
                </th>
                <th className="px-3 py-2 text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-right whitespace-nowrap">
                  Đã đóng
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td
                    colSpan={hasRankData ? 8 : 7}
                    className="px-6 py-20 text-center text-slate-400 italic font-medium tracking-tight"
                  >
                    ...
                    >
                      <td className="px-3 py-1.5 border-b border-slate-50">
                        <div className="flex flex-col items-start">
                          <button
                            onClick={() =>
                              onSelectStudent?.(student, "Học phí")
                            }
                      ...
                      <td className="px-3 py-1.5 border-b border-slate-50 min-w-[100px]">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[9px] font-extrabold text-slate-400 w-7">
                            {progress}%
                          </span>
                        </div>
                      </td>
