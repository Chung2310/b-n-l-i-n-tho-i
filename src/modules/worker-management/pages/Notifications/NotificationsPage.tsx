    ...
    <div className="p-3 rounded-xl border border-slate-100 hover:border-cyan-100 hover:bg-cyan-50/20 transition-all group relative">
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="space-y-0.5">
          <h4 className="text-xs font-black text-slate-800 line-clamp-1 text-left">
            {notification.title}
          </h4>
          ...
          {installmentPlan && (
            <div className="px-1.5 py-0.5 rounded text-[9px] font-black border bg-violet-50 text-violet-600 border-violet-100/50 flex items-center gap-1">
              ...
              {installmentPlan.label ||
                `Đợt ${installmentPlan.installmentNo}`}{" "}
              · {installmentPlan.percent}%
            </div>
          )}
          <div
            className={cn(
      ...
      <div className="flex items-center justify-between pt-2.5 border-t border-slate-100/50">
        <div className="flex items-center gap-1.5">
          {notification.channels.map((channel) => (
            <span
              key={channel}
              className="px-1.5 py-0.2 bg-slate-100 rounded text-[8px] font-black text-slate-500 uppercase tracking-wider"
            >
              {channel}
            </span>
          ))}
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-[9px] font-extrabold text-slate-600">
            {notification.recipientCount} HV
          </span>
        </div>
      </div>
    </div>
  ...
  const addInstallment = () => {
    ...
    onChange([
      ...plan,
      {
        ...
        percent: remaining > 0 ? remaining : 10,
        label: `Đợt ${nextNo}`,
        dueDate: "",
      },
    ]);
    ...
    <div className="space-y-3">
          ...
          >
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  updatePercent(idx, item.percent - 5);
                }}
              ...
              <div className="flex items-center gap-0.5">
                <input
                  ...
                  onClick={(e) => e.stopPropagation()}
                  onChange={(e) =>
                    updatePercent(idx, parseInt(e.target.value) || 0)
                  }
                  className={cn(
      ...
      <div className="flex items-center justify-between">
        ...
        >
          ...
          Tổng: {totalPercent}%{totalPercent === 100 && " ✓"}
          {isOver && " — Vượt quá 100%!"}
        </div>
      </div>
    </div>
...
export function NotificationsPage({
  ...
  useEffect(() => {
    ...
    operationalCopy.notificationAudienceLabel,
  ]);
  ...
  const [apiStatus, setApiStatus] = useState<
    "Checking" | "Ready" | "Missing Key" | "Error"
  >("Checking");
  const [smsApiStatus, setSmsApiStatus] = useState<
    "Checking" | "Ready" | "Sandbox" | "Error"
  >("Checking");
  ...
  const vietqrConfig = {
    ...
    accountNo:
      localVietqrConfig?.accountNo || (user as any)?.bankAccountNo || "",
    accountName:
      localVietqrConfig?.accountName ||
      (user as any)?.bankAccountName ||
      user?.displayName ||
      "",
    template:
      localVietqrConfig?.template ||
      "[Mã HV] - [Họ tên] - Nộp học phí khóa {hang}",
  };
  ...
  const checkApiStatus = async () => {
    try {
      ...
      if (
        msg.includes("SMTP_CONFIG_missing") ||
        msg.includes("cấu hình") ||
        msg.includes("SMTP")
      ) {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      ...
      if (res.success && res.notifications) {
        const mapped = res.notifications.map(
          (n: Omit<BroadcastNotification, "id"> & { _id: string }) => ({
            ...n,
            id: n._id,
          }),
    ...
    switch (recipientFilter) {
      ...
      case "Học viên sắp thi":
        return students.filter(
          (s) =>
            s.status.includes("Đang thi") ||
            s.exams?.some((e) => e.status === "Sắp thi"),
        );
  ...
  const replaceVariables = (
    ...
    const formattedTiendot = new Intl.NumberFormat("vi-VN", {
      style: "currency",
      currency: "VND",
    }).format(dotAmount);

    // {nhac_dong_phi} = gợi ý văn bản nhắc đóng phí
    const currentInstallment = installmentPlan.find(
      (p) => p.installmentNo === selectedInstallmentNo,
    );
    const dotLabel =
      currentInstallment?.label || `Đợt ${selectedInstallmentNo}`;
    const nhacDongPhi =
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    if (useInstallment && recipientFilter === "Học viên còn nợ học phí") {
      ...
      if (totalPercent > 100) {
        toast.error(
          `Tổng % các đợt đang là ${totalPercent}%, không được vượt quá 100%.`,
        );
        return;
      }
    }
    ...
    if (targetStudents.length === 0) {
      toast.warning(
        `Không tìm thấy ${entityLabel.singular} phù hợp với bộ lọc này.`,
      );
      return;
    }
    ...
    const currentInstallment =
      useInstallment && recipientFilter === "Học viên còn nợ học phí"
          ...
          try {
            ...
            if (!response.ok || !data.success) {
              ...
              errorMessage =
                ...
                (data.error || "Lỗi gửi SMS");
            }
          } catch {
            isSuccess = false;
            errorMessage =
              (errorMessage ? errorMessage + " & " : "") +
              "Lỗi kết nối server (SMS)";
          }
        }
  ...
  const handleDeleteNotification = async (id: string) => {
    try {
      ...
      toast.error(
        getApiErrorMessage(
          error,
          "Không thể xóa thông báo này. Vui lòng thử lại.",
        ),
      );
    }
  };
  ...
  const handleMarkInstallmentPaid = async (resultIdx: number) => {
    ...
    setSendProgress((prev) => ({
      ...
      results: prev.results.map((r, i) =>
        i === resultIdx ? { ...r, markingPaid: true } : r,
      ),
    }));

    try {
      await apiFetch(
        `/students/${result.student.id}/installment/${result.installmentNo}/mark-paid`,
        {
          method: "PATCH",
        },
      );
      setSendProgress((prev) => ({
        ...prev,
        results: prev.results.map((r, i) =>
          i === resultIdx ? { ...r, markingPaid: false, markedPaid: true } : r,
        ),
      }));
      toast.success(
        `Đã đánh dấu đã thu đợt ${result.installmentNo} cho ${result.student.fullName}!`,
      );
    } catch (error) {
      setSendProgress((prev) => ({
        ...prev,
        results: prev.results.map((r, i) =>
          i === resultIdx ? { ...r, markingPaid: false } : r,
        ),
      }));
      toast.error(
        "Không thể đánh dấu đã thu: " +
  ...
  const recipientCounts = {
    ...
    "Tất cả học viên đang học": students.filter((s) =>
      s.status.includes("Đang học"),
    ).length,
    "Học viên sắp thi": students.filter(
      (s) =>
        s.status.includes("Đang thi") ||
        s.exams?.some((e) => e.status === "Sắp thi"),
    ).length,
    "Học viên còn nợ học phí": students.filter(
      (s) => (s.paidAmount || 0) < parseInt(parseVND(s.fee) || "0"),
    ).length,
    "Học viên cần thi lại": students.filter((s) => s.status.includes("Thi lại"))
      .length,
  };

  const currentRecipientCount =
    recipientCounts[recipientFilter as keyof typeof recipientCounts] || 0;
  ...
  const educationTemplates = [
    {
      ...
      content:
        "Kính gửi học viên {ten}, Trung tâm xin thông báo học phí khóa học hạng {hang} của bạn hiện vẫn còn nợ {sotien}. {nhac_dong_phi} Để đảm bảo tiến độ học tập và dự thi đúng hạn, bạn vui lòng hoàn tất học phí trong tuần này. Trân trọng.",
    },
    {
      name: "Lịch thi",
      title: "THÔNG BÁO LỊCH THI SÁT HẠCH - {ten}",
      content:
        "Kính gửi học viên {ten}, Trung tâm xin thông báo lịch thi sát hạch hạng {hang} của bạn đã có vào ngày {ngaythi}. Bạn vui lòng có mặt đúng giờ và mang theo CCCD bản gốc để làm thủ tục dự thi. Chúc bạn thi tốt.",
    },
    {
      name: "Thi lại",
      title: "LỊCH THI LẠI & ÔN TẬP - {ten}",
      content:
        "Kính gửi học viên {ten}, Trung tâm đã sắp xếp lịch ôn tập và thi lại cho bạn khóa hạng {hang} tại trung tâm. Vui lòng liên hệ văn phòng để xác nhận lịch thi dự kiến kế tiếp. Cố gắng lên bạn nhé.",
    },
  ];
  const workerTemplates = [
    {
      ...
      content:
        "Kính gửi {ten}, công ty xin thông báo lịch phỏng vấn của bạn. Vui lòng kiểm tra thời gian, địa điểm trong nội dung được cung cấp và có mặt đúng giờ. Khi cần hỗ trợ, vui lòng liên hệ cán bộ phụ trách. Trân trọng.",
    },
    {
      name: "Bổ sung hồ sơ",
      title: "YÊU CẦU BỔ SUNG HỒ SƠ - {ten}",
      content:
        "Kính gửi {ten}, hồ sơ tuyển dụng của bạn hiện cần được bổ sung. Vui lòng liên hệ cán bộ phụ trách để hoàn thiện giấy tờ đúng thời hạn. Trân trọng.",
    },
    {
      name: "Nhận việc",
      title: "THÔNG BÁO NHẬN VIỆC - {ten}",
      content:
        "Kính gửi {ten}, công ty xin thông báo kế hoạch nhận việc của bạn. Vui lòng chuẩn bị đầy đủ giấy tờ và có mặt theo thời gian, địa điểm đã được hướng dẫn. Trân trọng.",
    },
  ];
  const customerTemplates = [
    {
      ...
      content:
        "Kính gửi khách hàng {ten}, chúng tôi xin thông báo lịch hẹn phục vụ của quý khách. Vui lòng kiểm tra thời gian và địa điểm trong nội dung được cung cấp. Khi cần hỗ trợ, quý khách vui lòng liên hệ nhân viên phụ trách. Trân trọng.",
    },
    {
      name: "Thông tin dịch vụ",
      title: "THÔNG TIN DỊCH VỤ - {ten}",
      content:
        "Kính gửi khách hàng {ten}, chúng tôi gửi đến quý khách thông tin cập nhật về dịch vụ đang sử dụng. Vui lòng liên hệ nhân viên phụ trách nếu quý khách cần tư vấn hoặc hỗ trợ thêm. Trân trọng.",
    },
    {
      name: "Chăm sóc khách hàng",
      title: "CHĂM SÓC KHÁCH HÀNG - {ten}",
      content:
        "Kính gửi khách hàng {ten}, cảm ơn quý khách đã sử dụng dịch vụ. Chúng tôi mong nhận được phản hồi để tiếp tục nâng cao chất lượng phục vụ. Trân trọng.",
    },
  ];
  ...
  const isDebtFilter = recipientFilter === "Học viên còn nợ học phí";
  const totalInstallmentPercent = installmentPlan.reduce(
    (s, p) => s + p.percent,
    0,
  );
  ...
  return (
    <div className="space-y-6">
              ...
              <div className="p-8 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-slate-500 text-xs font-bold mt-1">
                    Đang xử lý: {sendProgress.current}/{sendProgress.total}{" "}
                  ...
                  {currentInstallment && (
                    <div className="mt-2 flex items-center gap-2">
                      <span className="px-2 py-0.5 bg-violet-100 text-violet-700 rounded-lg text-[10px] font-black border border-violet-200">
                        ...
                        {currentInstallment.percent}% học phí gốc
                      </span>
                    </div>
                  )}
                </div>
                {!isSubmitting && (
                  <button
                    ...
                    className="p-2 bg-white text-slate-400 hover:text-slate-600 rounded-xl transition-colors"
                  >
                    <X className="w-5 h-5" />
                  </button>
                )}
                              ...
                              {res.status === "Thành công" &&
                                ...
                                !res.markedPaid && (
                                  <button
                                    type="button"
                                    onClick={() =>
                                      handleMarkInstallmentPaid(originalIdx)
                                    }
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold text-slate-900 tracking-tight text-left">
            BOT Thông báo
          </h1>
          <p className="text-slate-400 text-[11px] font-medium mt-0.5 text-left">
            {operationalCopy.notificationSubtitle}
          </p>
        </div>
      </div>
          ...
          >
            ...
            <form onSubmit={handleSend} className="p-4 space-y-4">
              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest text-left block">
                  Đối tượng nhận
                </label>
                          ...
                          {useInstallment && (
                            ...
                            >
                              <p className="text-[10px] font-black text-violet-500 uppercase tracking-widest">
                                Cấu hình đợt · Chọn đợt muốn gửi thông báo lần
                                này
                              </p>
                              <InstallmentPlanEditor
                              ...
                              {currentInstallment && (
                                <div className="flex items-center gap-2 p-3 rounded-xl bg-violet-100/60 border border-violet-200/50">
                                  ...
                                  <p className="text-xs font-black text-violet-700">
                                    Sẽ gửi thông báo cho{" "}
                                    <b>{currentInstallmentCount()}</b> học viên
                                    với QR số tiền ={" "}
                                    <b>{currentInstallment.percent}%</b> tổng
                                    học phí gốc ({currentInstallment.label})
                                  </p>
                                </div>
                              )}
                            </motion.div>
                        ...
                        !operationalCopy.isCustomer && (
                          <>
                            ...
                            <VariableTag
                              name="tiendot"
                              label="Tiền đợt"
                              highlight={useInstallment && isDebtFilter}
                            />
                            <VariableTag
                              name="nhac_dong_phi"
                              label="Gợi ý đóng phí"
                              highlight={useInstallment && isDebtFilter}
                            />
                          </>
                        )}
                ...
                <div className="text-xs font-bold text-left">
                  ...
                  vietqrConfig.accountNo ? (
                    ...
                    <div className="flex items-center gap-2 text-amber-700 bg-amber-50/50 border border-amber-100/50 p-3.5 rounded-2xl">
                      ...
                      />
                      <span>
                        ⚠️ Chưa cấu hình mã QR chuyển khoản hoặc tính năng này
                        đang tắt. Thư điện tử sẽ chỉ có văn bản thông thường.
                        Vào Cài đặt để kích hoạt.
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {channels.includes("Email") && apiStatus === "Ready" && (
                    <p className="text-[9px] text-slate-400 font-medium italic text-right">
                      * Dịch vụ gửi thư: Gửi thư điện tử không giới hạn qua tài
                      khoản của bạn.
                    </p>
                  )}
                </div>
                <div className="flex flex-wrap items-center gap-6">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[9px] font-bold text-amber-600 bg-amber-50 border border-amber-100/50 px-1.5 py-0.5 rounded-md uppercase tracking-wider">
                        Sắp có
                      </span>
                    </div>
                  </button>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-2 pt-3 border-t border-slate-50">
                <p className="text-[11px] font-bold text-cyan-600 bg-cyan-50 px-3 py-1.5 rounded-full">
                  ...
                  <span className="font-black underline underline-offset-4">
                    {currentRecipientCount} {entityLabel.singular}
                  </span>
                </p>
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  <button
                    ...
                    disabled={
                      ...
                      (useInstallment && totalInstallmentPercent > 100)
                    }
                    className={cn(
              ...
              <div className="space-y-2">
                {history.map((item) => (
                  <HistoryCard
                    key={item.id}
                    notification={item}
                    onDelete={canManage ? handleDeleteNotification : undefined}
                  />
                ))}
              </div>
            )}
