...
const DEFAULT_STANDARD_FIELDS: Record<ModuleKey, StandardFieldConfig[]> = {
  ...
  courses: [
    ...
    },
    {
      ...
      placeholder: "Ví dụ: Luyện thi TOEIC 650+ cam kết chuẩn đầu ra",
      type: "text",
      isRequired: true,
      isVisible: true,
      isArchived: false,
  ...
  batches: [
    ...
    },
    {
      key: "schedule",
      label: "Lịch học",
      placeholder: "Ví dụ: T2-T4-T6 (18:00-20:00)",
      type: "text",
      isRequired: true,
      isVisible: true,
      isArchived: false,
  ...
  resources: [
    ...
    {
      key: "capacity",
      label: "Sức chứa / Khả năng đáp ứng",
...
const PRESET_STANDARD_FIELD_OVERRIDES: Partial<
  Record<
    EntityPreset,
    Partial<Record<ModuleKey, Record<string, Partial<StandardFieldConfig>>>>
  >
> = {
  worker: {
    batches: {
      ...
      schedule: {
        ...
        placeholder: "Ví dụ: T2-T4-T6 (08:00-17:00)",
      },
...
function getDefaultStandardFields(
  moduleKey: ModuleKey,
  preset?: EntityPreset,
): StandardFieldConfig[] {
  const base = DEFAULT_STANDARD_FIELDS[moduleKey] || [];
  const overrides = preset
    ? PRESET_STANDARD_FIELD_OVERRIDES[preset]?.[moduleKey]
    : undefined;
...
export function getAdaptedFieldDefinition(
...
export function useStandardFields(moduleKey: ModuleKey, preset?: EntityPreset) {
    ...
    if (stored) {
      try {
        ...
        const merged = defaults.map((def) => {
          ...
          const keptLabel =
            base && matched.label === base.label ? def.label : matched.label;
          const keptPlaceholder =
            base && matched.placeholder === base.placeholder
              ? def.placeholder
              : matched.placeholder;
          return {
            ...def,
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      `standard-fields-changed:${moduleKey}`,
      handleLocalChange,
    );
    return () =>
  ...
  const updateField = (key: string, input: Partial<StandardFieldConfig>) => {
    const updated = fields.map((f) => {
      if (f.key === key) {
        const isVisible =
          input.isVisible !== undefined ? input.isVisible : f.isVisible;
