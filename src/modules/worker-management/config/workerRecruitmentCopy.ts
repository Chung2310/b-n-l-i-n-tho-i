...
const EDUCATION_COURSE_COPY: CoursePageCopy = {
  ...
  emptySubtitle:
    "Bấm 'Thêm khóa học mới' để khởi tạo chương trình đào tạo đầu tiên.",
  ...
  durationPlaceholder: "Ví dụ: 3 tháng / 8 tuần",
  ...
  deleteCategoryMessage: (name) =>
    `Bạn có chắc chắn muốn xóa phân loại "${name}" không? Hành động này không thể hoàn tác.`,
  ...
  tableCapacitySummary: (capacity, activeBatches) =>
    `${capacity} HV (${activeBatches} lớp)`,
  ...
  statusMessage: (code, status) =>
    `Khóa học ${code} đã chuyển sang "${status}".`,
...
const WORKER_PROJECT_COPY: CoursePageCopy = {
  ...
  deleteCategoryMessage: (name) =>
    `Bạn có chắc chắn muốn xóa nhóm dự án "${name}" không? Hành động này không thể hoàn tác.`,
  ...
  tableCapacitySummary: (capacity, activeBatches) =>
    `${capacity} lao động (${activeBatches} đợt tuyển)`,
...
const CUSTOMER_SERVICE_COPY: CoursePageCopy = {
  ...
  deleteCategoryMessage: (name) =>
    `Bạn có chắc chắn muốn xóa nhóm dịch vụ "${name}" không? Hành động này không thể hoàn tác.`,
  ...
  tableCapacitySummary: (capacity, activeBatches) =>
    `${capacity} khách hàng (${activeBatches} hợp đồng)`,
  ...
  statusMessage: (code, status) =>
    `Dịch vụ ${code} đã chuyển sang "${status}".`,
...
export function getBatchPageCopy(preset: EntityPreset): BatchPageCopy {
  ...
  return {
    ...
    emptySubtitle:
      "Bấm 'Mở lớp mới' để khai giảng lớp đầu tiên cho một khóa học.",
    codeLabel: "Mã lớp",
...
const WORKER_OPERATIONAL_COPY: WorkerOperationalCopy = {
  ...
  settingsMailDescription:
    "Kết nối tài khoản Gmail để tự động gửi thông báo tuyển dụng và lịch làm việc cho lao động, đối tác.",
};
...
const CUSTOMER_OPERATIONAL_COPY: WorkerOperationalCopy = {
  ...
  settingsMailDescription:
    "Kết nối tài khoản Gmail để tự động gửi thông báo dịch vụ và lịch hẹn cho khách hàng, đối tác.",
};

export function getWorkerOperationalCopy(
  preset: EntityPreset,
): WorkerOperationalCopy {
...
export function getOperationalStatusLabel(
  ...
  return (
    getWorkerOperationalCopy(preset).registrationStatusLabels[status] ?? status
  );
}
