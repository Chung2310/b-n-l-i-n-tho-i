...
const inputClass =
  "w-full px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm placeholder:text-slate-300 focus:outline-none focus:ring-4 focus:ring-cyan-600/5 focus:border-cyan-600 transition-all text-slate-800 disabled:bg-slate-50 disabled:text-slate-600 disabled:cursor-default";
...
function acceptsMime(file: File, allowed?: string[]): boolean {
  ...
  return allowed.some(
    (mime) =>
      mime === file.type ||
      (mime.endsWith("/*") && file.type.startsWith(mime.slice(0, -1))),
  );
}
...
export function CustomFieldRenderer({
  ...
  const upload = async (files: File[]) => {
    setUploadError(null);
    const allowedMimeTypes = validation?.allowedMimeTypes as
      | string[]
      | undefined;
    const maxSizeMb = Number(validation?.maxSizeMb ?? 100);
    const maxFiles = Number(
      validation?.maxFiles ??
        (fieldType === "multiImage" ? Number.POSITIVE_INFINITY : 1),
    );
    const currentFiles =
      fieldType === "multiImage" && Array.isArray(value) ? value.length : 0;
    ...
    const invalidMime = files.find(
      (file) => !acceptsMime(file, allowedMimeTypes),
    );
    if (invalidMime) {
      setPendingFiles(files);
      setUploadError(
        `Loại tệp ${invalidMime.type || invalidMime.name} không được cho phép.`,
      );
      return;
    }
    ...
    try {
      const metadata = await Promise.all(
        files.map(async (file): Promise<FileMetadata> => {
      ...
      setUploadError(
        caught instanceof Error ? caught.message : "Tải tệp thất bại.",
      );
    } finally {
      setUploading(false);
    }
  ...
  switch (fieldType) {
    ...
    case "switch":
      control = (
        <input
          {...common}
          className="h-4 w-4 rounded border-slate-200 text-cyan-600 focus:ring-cyan-600 focus:ring-opacity-20 cursor-pointer"
          role={fieldType === "switch" ? "switch" : undefined}
          type="checkbox"
          checked={value === true}
      ...
      control = (
        <div className="space-y-2">
          <input
            ...
            type="file"
            accept={
              (validation?.allowedMimeTypes as string[] | undefined)?.join(
                ",",
              ) ??
              (fieldType === "image" || fieldType === "multiImage"
                ? "image/*"
                : undefined)
            }
              ...
              <div className="flex-1 min-w-0">
                ...
                {typeof (value as FileMetadata).size === "number" && (
                  <p className="text-[10px] text-slate-400">
                    {(
                      Number((value as FileMetadata).size) /
                      1024 /
                      1024
                    ).toFixed(2)}{" "}
                    MB
          ...
          {showMultiImage && (value as FileMetadata[]).length > 0 && (
            <div className="flex flex-wrap gap-3 p-2 bg-slate-50 border border-slate-100 rounded-xl">
              {(value as FileMetadata[]).map((img, idx) => (
                <div
                  key={`${img.url}-${idx}`}
                  className="relative group/thumb w-16 h-16 border border-slate-200 rounded-lg overflow-hidden"
                >
                  <img
                    src={img.url}
                    alt={img.fileName}
                  ...
                  <button
                    ...
                    onClick={() => {
                      const next = (value as FileMetadata[]).filter(
                        (_, i) => i !== idx,
                      );
                      onChange(next);
                    }}
    ...
    default: {
      ...
      control = (
        <input
          {...common}
          className={inputClass}
          type={inputTypes[fieldType] ?? "text"}
          placeholder={field.placeholder}
          value={stringValue}
          min={dateMin as string | number | undefined}
          max={dateMax as string | number | undefined}
          step={
            isNumeric
              ? decimals > 0
                ? 10 ** -decimals
  ...
  return (
    <div className="space-y-1" data-custom-field={field.key}>
      <label
        className={`text-[10px] font-bold text-slate-800 uppercase tracking-wider ${checkboxLike ? "flex items-center gap-2" : "block"}`}
        htmlFor={inputId}
      >
      ...
      {error || uploadError ? (
        ...
        >
          ...
          {uploadError && pendingFiles ? (
            <button
              type="button"
              className="underline"
              onClick={() => void upload(pendingFiles)}
            >
              Thử lại
            </button>
          ) : null}
