...
const inputClass =
  "w-full px-4 py-2 mt-1 bg-white border border-slate-200 rounded-xl text-sm placeholder:text-slate-300 focus:outline-none focus:ring-4 focus:ring-cyan-600/5 focus:border-cyan-600 transition-all";
...
function initialState(field?: FieldDefinition | null): FormState {
  ...
  return {
    ...
    decimals: String(validation?.decimals ?? ""),
    minLength: String(validation?.minLength ?? ""),
    maxLength: String(validation?.maxLength ?? ""),
    minDateTime: String(validation?.minDateTime ?? ""),
    maxDateTime: String(validation?.maxDateTime ?? ""),
    maxSizeMb: String(validation?.maxSizeMb ?? "10"),
    maxFiles: String(validation?.maxFiles ?? "1"),
    allowedMimeTypes: Array.isArray(validation?.allowedMimeTypes)
      ? validation.allowedMimeTypes.join(", ")
      : "",
  };
}
...
export function CustomFieldEditorModal({
  ...
  useEffect(() => {
    if (open) {
      setForm(initialState(initialField));
      setErrors([]);
      setSubmitting(false);
    }
  }, [open, initialField]);
  ...
  const validate = () => {
    ...
    const min = numberOrUndefined(form.min);
    const max = numberOrUndefined(form.max);
    const minLength = numberOrUndefined(form.minLength);
    const maxLength = numberOrUndefined(form.maxLength);
    if (
      (min !== undefined && !Number.isFinite(min)) ||
      (max !== undefined && !Number.isFinite(max)) ||
      (min !== undefined && max !== undefined && min > max)
    )
      next.push("Khoảng số không hợp lệ.");
    if (
      (minLength !== undefined &&
      ...
      next.push("Khoảng độ dài không hợp lệ.");
    if (
      form.minDateTime &&
      form.maxDateTime &&
      form.minDateTime > form.maxDateTime
    )
      next.push("Khoảng ngày giờ không hợp lệ.");
    if (["file", "image"].includes(form.type)) {
      try {
        createMaxSizeMb(Number(form.maxSizeMb));
      } catch {
        next.push("Kích thước tối đa phải từ 1 đến 100 MB.");
      }
      const maxFiles = Number(form.maxFiles);
      if (!Number.isInteger(maxFiles) || maxFiles < 1)
        next.push("Số tệp tối đa phải là số nguyên từ 1.");
    }
    return next;
  };
  ...
  const submit = async (event?: FormEvent) => {
    ...
    if (form.type === "text") {
      validation = {
        minLength: numberOrUndefined(form.minLength),
        maxLength: numberOrUndefined(form.maxLength),
      };
    ...
    if (["percent", "currency"].includes(form.type))
      validation = {
        min: numberOrUndefined(form.min),
        max: numberOrUndefined(form.max),
        decimals: numberOrUndefined(form.decimals),
      };
    if (form.type === "dateTime")
      validation = {
        minDateTime: form.minDateTime || undefined,
        maxDateTime: form.maxDateTime || undefined,
      };
    if (["file", "image"].includes(form.type)) {
      const allowedMimeTypes =
        form.type === "image" ? ["image/*"] : DEFAULT_ALLOWED_FILE_MIME_TYPES;
      validation = {
        maxSizeMb: createMaxSizeMb(Number(form.maxSizeMb)),
    ...
    else if (["percent", "currency"].includes(form.type))
      defaultValue =
        form.defaultValue === "" ? undefined : Number(form.defaultValue);
    else
      defaultValue =
        typeof form.defaultValue === "string" && form.defaultValue !== ""
          ? form.defaultValue
          : undefined;
    setSubmitting(true);
    setErrors([]);
    try {
      await onSubmit({
        ...
        isRequired: form.isVisible && form.isRequired,
      });
      onClose();
    } catch (caught) {
      setErrors([
        caught instanceof Error
          ? caught.message
          : "Không thể lưu trường. Vui lòng thử lại.",
      ]);
    } finally {
      setSubmitting(false);
    }
    ...
    >
      ...
      >
        <div className="flex items-center justify-between">
          <h2
            id="custom-field-editor-title"
            className="text-sm font-bold text-slate-900 uppercase tracking-wider"
          >
            {initialField ? "Chỉnh sửa trường" : "Thêm trường"}
          </h2>
          <button
            type="button"
            aria-label="Đóng"
        ...
        <div className="mt-5 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            {fileType ? (
              <p className="self-end text-xs text-slate-500 py-2">
                Tệp không hỗ trợ giá trị mặc định.
              </p>
            ) : form.type === "checkbox" ? (
              <label className="flex items-center gap-2 self-end py-2 text-[10px] font-bold text-slate-800 uppercase tracking-wider cursor-pointer">
                <input
                  type="checkbox"
                  className="h-4 w-4 rounded border-slate-200 text-cyan-600 focus:ring-cyan-600 focus:ring-opacity-20"
                  checked={form.defaultValue === true}
                  onChange={(event) =>
                    set("defaultValue", event.target.checked)
                  }
                />
                Giá trị mặc định
              </label>
              ...
              <label className="block text-[10px] font-bold text-slate-800 uppercase tracking-wider">
                ...
                <input
                  type={
                    ["percent", "currency"].includes(form.type)
                      ? "number"
                      : form.type === "dateTime"
                        ? "datetime-local"
                        : "text"
                  ...
                  className={inputClass}
                  value={
                    typeof form.defaultValue === "string"
                      ? form.defaultValue
                      : ""
                  }
          ...
          <div className="flex gap-6">
            <label className="flex items-center gap-2 text-[10px] font-bold text-slate-800 uppercase tracking-wider cursor-pointer">
              <input
                type="checkbox"
                className="h-4 w-4 rounded border-slate-200 text-cyan-600 focus:ring-cyan-600 focus:ring-opacity-20"
            ...
            <div className="grid grid-cols-2 gap-4">
              <label className="block text-[10px] font-bold text-slate-800 uppercase tracking-wider">
                ...
                <input
                  type="datetime-local"
                  className={inputClass}
                  value={form.minDateTime}
                  onChange={(e) => set("minDateTime", e.target.value)}
                />
              </label>
              <label className="block text-[10px] font-bold text-slate-800 uppercase tracking-wider">
                Ngày giờ lớn nhất
                <input
                  type="datetime-local"
                  className={inputClass}
                  value={form.maxDateTime}
                  onChange={(e) => set("maxDateTime", e.target.value)}
                />
              </label>
            </div>
          ) : null}
