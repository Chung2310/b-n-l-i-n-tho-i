...
function validateFileSizeLimit(
  input: CreateFieldInput | UpdateFieldInput,
): void {
  const validation = input.validation as { maxSizeMb?: unknown } | undefined;
  if (validation?.maxSizeMb !== undefined)
    createMaxSizeMb(validation.maxSizeMb as number);
}
...
export async function createCustomField(
  ...
  const response = await apiFetch<ApiResponse<ApiFieldDefinition>>(
    `${basePath}/${moduleKey}`,
    {
      method: "POST",
...
export async function deleteCustomField(
  ...
  await apiFetch<ApiResponse<void>>(`${basePath}/${moduleKey}/${id}/delete`, {
    method: "POST",
    ...(tenantId ? { params: { tenantId } } : {}),
  });
}
...
export async function uploadCustomFieldFile(
  ...
  const response = await apiFetch<ApiResponse<FileMetadata>>(
    `${basePath}/${field.moduleKey}/${field.id}/upload`,
    {
      method: "POST",
