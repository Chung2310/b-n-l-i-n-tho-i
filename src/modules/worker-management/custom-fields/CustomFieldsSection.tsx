...
export function CustomFieldsSection({
  ...
  const {
    ...
    restoreField,
    deleteField,
  } = useCustomFields(moduleKey, tenantId);
  ...
  const activeFields = useMemo(
    () =>
      [...fields]
        .filter((field) => !field.isArchived && field.isVisible)
  ...
  const isEditing =
    isEditingFields !== undefined ? isEditingFields : localIsEditingFields;
  ...
  const archive = async (field: FieldDefinition) => {
    if (
      !window.confirm(
        `Lưu trữ trường “${field.label}”? Dữ liệu hiện có sẽ được giữ lại.`,
      )
    )
      return;
  ...
  const remove = async (field: FieldDefinition) => {
    if (
      !window.confirm(
        `Xóa vĩnh viễn trường “${field.label}”? Dữ liệu liên quan đến trường này trên tất cả hồ sơ cũng sẽ bị mất.`,
      )
    )
      return;
        ...
        {activeFields.map((field) => (
          <div key={field.id} className="relative space-y-1">
            {manageable && isEditing ? (
              <div className="absolute right-0 top-0 z-10 flex items-center gap-1.5 text-[10px] font-semibold text-slate-400 opacity-60 hover:opacity-100 transition-opacity">
                ...
                <button
                  type="button"
                  disabled={disabled}
                  aria-label={`Lưu trữ ${field.label}`}
                  title="Lưu trữ"
                  className="hover:text-cyan-600 transition-colors"
            ...
            <CustomFieldRenderer
              ...
              value={
                values[field.key] ??
                (mode === "create" ? field.defaultValue : undefined)
              }
