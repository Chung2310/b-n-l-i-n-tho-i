...
import type { RetailOrder, RetailPaymentInput, RetailScope } from "../types";
export const retailOrdersApi = {
  ...
  async confirm(scope: RetailScope, id: string, input: { expectedGrandTotal: number; payments: RetailPaymentInput[]; idempotencyKey: string }) { const response = await apiFetch<{ success: true; data: any }>(`/retail/orders/${id}/confirm`, { method: "POST", params: scope, body: JSON.stringify(input) }); return response.data; },
