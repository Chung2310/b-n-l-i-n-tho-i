...
export default function RetailReportTables({ report }: RetailReportTablesProps) {
  ...
  const productTable = (title: string, rows: RetailReport["products"]) => {
    const isTop = title.includes("bán chạy");
    const Icon = isTop ? TrendingUp : TrendingDown;
    const iconColor = isTop ? "text-cyan-600 bg-cyan-50" : "text-amber-600 bg-amber-50";
    ...
    return (
      <article className="overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-xs transition hover:border-slate-300">
        ...
        <div className="overflow-x-auto">
          <table aria-label={title} className="w-full min-w-[760px]">
              ...
              {rows.length > 0 ? (
                rows.map((row, index) => (
                  <tr key={row.productId} className="transition-colors hover:bg-slate-50/60">
                    ...
                    <td className={`${cellClass} font-semibold text-slate-900 max-w-[220px] truncate`}>
                      {row.productName}
                    </td>
