...
export default function RetailCustomersPage() {
  ...
  return (
    <section className="space-y-4">
      ...
      {detail && <aside className="fixed inset-y-0 right-0 z-50 w-full max-w-lg overflow-y-auto border-l bg-white p-6 shadow-2xl"><button aria-label="Đóng chi tiết" onClick={() => setDetail(null)} className="float-right"><X /></button><h2 className="text-xl font-bold">{detail.customer.name}</h2><p className="text-sm text-slate-500">{detail.customer.customerCode}</p><div className="mt-6 grid grid-cols-3 gap-3">{[["Doanh số", detail.summary.totalSales], ["Đã thu", detail.summary.totalCollected], ["Công nợ", detail.summary.currentDebt]].map(([label, value]) => <div key={String(label)} className="rounded-xl bg-slate-50 p-3"><div className="text-xs text-slate-500">{label}</div><div className="font-bold">{Number(value).toLocaleString("vi-VN")}đ</div></div>)}</div><p className="mt-6 text-sm text-slate-500">Lịch sử đơn hàng và thanh toán sẽ hiển thị từ dữ liệu giao dịch thực tế.</p></aside>}
    </section>
  );
}
