...
export function KanbanProjectSummary({ project, expanded = false }: { project: Project; expanded?: boolean }) {
  if (expanded) return <div className="min-w-0 flex-1 border-b border-slate-100 pb-3 text-xs">
  ...
  return <div className="min-w-0 flex-1 text-xs">
    ...
    <div role="progressbar" aria-valuenow={progress.percent} aria-valuemin={0} aria-valuemax={100} className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-200"><div className="h-full bg-indigo-600" style={{ width: `${progress.percent}%` }} /></div>
