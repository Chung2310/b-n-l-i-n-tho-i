...
export function PayrollPolicyManager({ canManage, onPoliciesChanged, runStatus, onRecalculate }: { canManage: boolean; onPoliciesChanged?: () => void | Promise<void>; runStatus?: string; onRecalculate?: () => Promise<void> }) {
  ...
  return <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-xs">
    <div className="mb-3 flex items-center justify-between gap-3">
      <h3 className="text-sm font-bold text-slate-800">Phiên bản công thức lương</h3>
      {canManage && <button onClick={() => setEditor({ mode: "create" })} className="inline-flex items-center gap-1 rounded-lg bg-indigo-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-indigo-700 cursor-pointer shadow-xs transition-colors"><Plus size={14}/>Tạo công thức</button>}
    </div>
