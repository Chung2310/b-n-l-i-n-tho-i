...
export function InventoryCountingSection({ warehouseId }: { warehouseId: string }) {
  ...
  return <section className="space-y-3 rounded-xl border border-slate-200 bg-white p-4" aria-label="Kiểm kê kho">
    ...
    {count && <><p className="text-sm">Phiếu <strong>{count.countCode}</strong> · {count.status} · Chênh lệch: <strong className={discrepancy === 0 ? "text-emerald-600" : "text-rose-600"}>{discrepancy > 0 ? "+" : ""}{discrepancy}</strong></p>
