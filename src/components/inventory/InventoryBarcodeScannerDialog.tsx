...
export default function InventoryBarcodeScannerDialog({ onScan, onClose }: { onScan: (value: string) => void; onClose: () => void }) {
  ...
  React.useEffect(() => {
    ...
    const start = async () => {
      ...
      if (!Detector) { setError("Trình duyệt chưa hỗ trợ quét barcode bằng camera. Hãy dùng Chrome Android hoặc đầu đọc."); return; }
      try {
        ...
        const scan = async () => {
          ...
          try {
            const value = String((await detector.detect(videoRef.current))[0]?.rawValue || "").trim();
            const now = Date.now();
            if (value && (lastScan.current.value !== value || now - lastScan.current.at > 1200)) {
              lastScan.current = { value, at: now }; feedback(); stopped = true; onScan(value); onClose(); return;
            }
