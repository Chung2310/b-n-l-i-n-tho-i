...
export default function CompanyWorkCalendarTab() {
  ...
  return <div className="space-y-4 text-left">
    ...
    {formOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"><form onSubmit={save} className="w-full max-w-md space-y-3 rounded-2xl bg-white p-5"><div className="flex justify-between"><h4 className="font-bold">{editingId ? "Sửa ngày công ty" : "Thêm ngày công ty"}</h4><button type="button" onClick={() => setFormOpen(false)}><X /></button></div><input aria-label="Ngày" type="date" required value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className="w-full rounded-lg border p-2"/><input aria-label="Tên ngày" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full rounded-lg border p-2"/><select aria-label="Loại ngày" value={form.dayType} onChange={(e) => setForm({ ...form, dayType: e.target.value as WorkCalendarDayType })} className="w-full rounded-lg border p-2">{Object.entries(TYPE_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><button className="w-full rounded-lg bg-indigo-600 p-2 font-bold text-white">Lưu</button></form></div>}
    {reasonTarget && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"><div className="w-full max-w-md space-y-3 rounded-2xl bg-white p-5"><h4 className="font-bold">Tắt áp dụng {reasonTarget.name}</h4><textarea aria-label="Lý do tắt áp dụng" value={reason} onChange={(e) => setReason(e.target.value)} className="w-full rounded-lg border p-2"/><div className="flex justify-end gap-2"><button onClick={() => setReasonTarget(null)} className="rounded-lg border px-3 py-2">Hủy</button><button onClick={() => void disable()} className="rounded-lg bg-red-600 px-3 py-2 text-white">Xác nhận</button></div></div></div>}
  </div>;
}
