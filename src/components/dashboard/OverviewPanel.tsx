...
export function OverviewPanel({
  ...
  return (
    <div className="space-y-3 pb-3">
      {actionItems && <ActionItemsWidget actionItems={actionItems} onGoToTasks={() => goToTab("NHÂN SỰ", "kanban")} onGoToApprovals={() => goToTab("NHÂN SỰ", "lich&view=requests")} onGoToInventory={() => { window.history.pushState(null, "", "/kho-san-pham?sub=nhap-hang"); window.dispatchEvent(new PopStateEvent("popstate")); }} onGoToContract={(alert) => goToContract(alert.employeeName)} />}
      {canSeeHr && (
