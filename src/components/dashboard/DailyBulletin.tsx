...
export function DailyBulletin({ data, error }: { data?: DashboardActionItems["bulletin"]; error: boolean }) {
  ...
  const iconTones: Record<string, string> = {
    "Cham cong hom nay": "bg-emerald-100 text-emerald-600",
    "Don treo": "bg-amber-100 text-amber-600",
    "KPI / doanh so thang": "bg-sky-100 text-sky-600",
    "May like-new ve": "bg-violet-100 text-violet-600",
    "Phieu sua do": "bg-sky-100 text-sky-600",
    "Lich hen tra may": "bg-violet-100 text-violet-600",
    "Thieu linh kien": "bg-orange-100 text-orange-600",
    "Doanh so hom nay": "bg-emerald-100 text-emerald-600",
    "Doanh so hom qua": "bg-sky-100 text-sky-600",
    "Nhan su hom nay": "bg-violet-100 text-violet-600",
    "Hang cham day an toan": "bg-orange-100 text-orange-600",
  };
    ...
    {error ? <p role="alert" className="text-sm font-semibold text-rose-800">Không tải được bản tin. Hệ thống sẽ tự thử lại sau 30 giây.</p> : !data ? <p role="status" className="text-sm text-slate-700">Đang tải việc cần làm của bạn…</p> : <>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {data.cards.map(card => { const key = card.title.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/\u0110/g, "D").replace(/\u0111/g, "d"); const Icon = cardIcons[key] || ClipboardList; return <article key={card.title} className="flex flex-col rounded-xl border border-slate-200 bg-white p-3">
          <div className="flex items-center justify-center gap-2 text-center"><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${iconTones[key] || "bg-cyan-100 text-cyan-600"}`}><Icon aria-hidden="true" className="h-5 w-5" /></span><h3 className="text-xs font-bold text-slate-700">{card.title}</h3></div>
          <p className="mt-2 text-center text-lg font-extrabold text-slate-900">{card.value}</p>
