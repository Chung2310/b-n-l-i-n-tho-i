...
describe("revenue combination chart", () => {
  it("aligns the line points with bar tops on the same scale", () => {
    ...
    linePoints.forEach((point, i) => expect(point[1]).toBe(Number(bars[i].getAttribute("y"))));
    expect(linePoints[0][1]).toBe(0);
    expect(linePoints[1][1]).toBe(130);
    expect(linePoints[2][1]).toBe(260);
  ...
  it("handles empty and all-zero series without invalid SVG coordinates", () => {
    ...
    expect(container.querySelector("polyline")?.getAttribute("points")).toBe("500,260");
