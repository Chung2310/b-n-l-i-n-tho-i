...
export interface BranchOption { _id: string; code: string; name: string; companyCode: string; isActive: boolean; }
...
export function BranchProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch("/api/v1/auth/branches", { headers: { Authorization: `Bearer ${localStorage.getItem("accessToken") || ""}` } })
      .then((res) => res.ok ? res.json() : Promise.reject(new Error("Không thể tải chi nhánh")))
      .then((json) => { const list = (json.data || []).filter((b: BranchOption) => b.isActive); setBranches(list); const key = `igen.activeBranch.${userProfile.companyCode}`; const saved = localStorage.getItem(key); setActiveBranchIdState(list.some((b: BranchOption) => b._id === saved) ? saved! : (list[0]?._id || "")); })
