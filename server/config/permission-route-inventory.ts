...
export const PUBLIC_ROUTE_EXCEPTIONS: readonly PublicRouteException[] = [
  ...
  { sourceFile: "server/router/webhook.router.ts", router: "webhookRouter", mount: "/webhook", method: "POST", path: "/sepay/retail-payment", reason: "HMAC/API-key authenticated SePay retail webhook" },
...
function permissionCodesFromMiddleware(middleware: string, constants: Readonly<Record<string, string[]>>): string[] {
  ...
  for (const reference of middleware.matchAll(/\b[A-Z][A-Z0-9_]*_PERMISSIONS\["([^"]+)"\]\.(read|manage)\b/g)) {    codes.push(`${reference[0].startsWith("STUDENT_AREA_PERMISSIONS") ? "people" : reference[1]}:${reference[2]}`);
  }
