...
companyEmailRouter.put("/smtp", async (req: any, res) => { const { error, value } = smtpSchema.validate(req.body); if (error) return res.status(400).json({ message: error.message }); try { return res.json({ data: await companyEmailService.saveSmtp(companyCode(req), value) }); } catch (e: any) { return res.status(400).json({ message: e.message }); } });
