...
export function createTenantRouter(deps: Dependencies = {}) {
  ...
  router.get("/tenants/:companyCode", async (req, res) => { try { const result = await service.get(req.params.companyCode); const audit = await AuditEventModel.find({ companyCode: result.code }, undefined, { sort: { occurredAt: -1 }, limit: 20 }); return res.json({ tenant: result, summary: { users: 0, usage: 0, storage: 0, financial: 0 }, audit }); } catch (error) { return res.status(404).json({ message: (error as Error).message, correlationId: (req as any).id }); } });
