...
import { LABOR_PARTNER_PERMISSION_CATALOG, PERMISSION_CATALOG } from "../config/permission-catalog";
...
superAdminUserAccessRouter.get("/permission-catalog", (_req, res) => res.json({ catalog: [...PERMISSION_CATALOG, ...LABOR_PARTNER_PERMISSION_CATALOG] }));
...
superAdminUserAccessRouter.get("/users/:userId/activity", async (req: any, res) => { try { return res.json(await userActivityService.list({ tenantId: companyCode(req), userId: req.params.userId, from: req.query.from, to: req.query.to, category: req.query.category, result: req.query.result, search: req.query.search, page: req.query.page, limit: req.query.limit })); } catch (e) { return res.status(400).json({ message: (e as Error).message }); } });
