...
walletRouter.post("/admin/balance", requireAuth, requireRole(["superadmin"]), requirePermission("finance:manage"), walletController.adminSetBalance);
walletRouter.get("/transactions", requireAuth, walletController.getTransactionHistory);
walletRouter.post("/deposit", requireAuth, requirePermission("finance:manage"), walletController.createDepositLink);
walletRouter.post("/webhook", walletController.handlePayOSWebhook);
walletRouter.post("/webhook-mock", requireAuth, requirePermission("finance:manage"), walletController.handleMockWebhook);
