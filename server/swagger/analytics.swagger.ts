export const analyticsSwagger = {
  paths: {
    "/api/v1/analytics/revenue": { get: { tags: ["Analytics"], summary: "Doanh thu đào tạo và bán hàng", security: [{ BearerAuth: [] }], parameters: ["from", "to", "granularity", "branchId", "courseId"].map((name) => ({ name, in: "query", required: name === "from" || name === "to", schema: { type: "string" } })), responses: { 200: { description: "Báo cáo doanh thu" } } } },
