...
export function clearModuleCache(companyCode?: string): void { if (companyCode) moduleCache.delete(companyCode.toUpperCase()); else moduleCache.clear(); }
export function resolveModuleAccess(user: { role?: string; companyCode?: string } | undefined, key: ModuleKey, enabledModules: string[] | undefined, companyExists = true, businessTypeInput?: unknown): boolean {
  if (user?.role === "superadmin") return true;
  if (!user?.companyCode || !companyExists) return false;
  return filterModulesForBusinessType(enabledModules, resolveBusinessType(businessTypeInput)).includes(key);
}
export async function getModuleStateForCompany(companyCode: string): Promise<CompanyModuleState> {
  ...
  if (cached && cached.expiresAt > Date.now()) return { exists: cached.exists, modules: cached.modules, businessType: cached.businessType };
  const company = await CompanyModel.findOne({ code }).select("enabledModules businessType").lean();
  const state = { exists: Boolean(company), modules: company?.enabledModules, businessType: resolveBusinessType(company?.businessType) };
  moduleCache.set(code, { ...state, expiresAt: Date.now() + CACHE_TTL_MS });
  return state;
}
export async function getEnabledModulesForCompany(companyCode: string): Promise<string[] | undefined> { return (await getModuleStateForCompany(companyCode)).modules; }
export function requireModule(key: ModuleKey) {
  const moduleAccessGuard = async (req: any, res: Response, next: NextFunction) => {
    try {
      const user = req.user;
      if (user?.role === "superadmin") return next();
      if (!user?.companyCode) return res.status(403).json({ status: "error", message: "Không xác định được doanh nghiệp." });
