...
import { refundRepairCommission } from "../partners/commission.service";
...
repairRouter.post("/tickets/:id/quote", quote, async (req, res, next) => { try { const data = await quoteRepairTicket(scope(req), req.params.id, Number(req.body?.amount), actor(req), req.body?.note, req.body?.laborFee === undefined ? undefined : Number(req.body.laborFee)); return res.json({ success: true, data }); } catch (error) { next(error); } });
