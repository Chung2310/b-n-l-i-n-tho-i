    ...
    if (!host || !portStr || !user || !pass) {
      if (process.env.DEPLOYMENT_ENV !== "production") {
        if (!this.EtherealTransporter) {
          logger.info("SMTP configuration is missing. Creating a temporary Ethereal Email test account for development/staging...");
          try {
            ...
            logger.error("Failed to create Ethereal Email test account: %o", err);
