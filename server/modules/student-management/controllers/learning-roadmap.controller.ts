...
export class LearningRoadmapController {
  ...
  static async create(req: AuthRequest, res: Response, next: NextFunction) { try { const ownerId = await resolveCreateOwnerId(req.user!, req.body.companyCode); const data = await LearningRoadmapService.createRoadmap(ownerId, req.user!.branchId, req.body); res.status(201).json({ success: true, data }); } catch (error) { next(error); } }
