...
const levelPriority: Record<LogLevel, number> = { error: 0, warn: 1, info: 2, http: 3, debug: 4 };
const ansiColors: Record<LogLevel, string> = {
  error: "\x1b[31m", warn: "\x1b[33m", info: "\x1b[32m", http: "\x1b[35m", debug: "\x1b[37m",
};
const resetColor = "\x1b[0m";
const currentLevel: LogLevel = process.env.NODE_ENV === "production" ? "info" : "debug";
const SENSITIVE_KEY = /^(authorization|cookie|password|passcode|token|accessToken|refreshToken|otp|secret|apiKey)$/i;
...
function serializeArg(arg: unknown): string {
  ...
  try {
    ...
    return util.inspect(redactLogData(arg), { depth: null, colors: false, breakLength: Infinity });
  }
}
