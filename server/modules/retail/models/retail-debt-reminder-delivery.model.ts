...
const RetailDebtReminderDeliverySchema = new Schema({
  ...
  status: { type: String, enum: ["queued", "sent", "failed", "duplicate"], default: "queued" }, attempt: { type: Number, default: 0 }, maxAttempts: { type: Number, required: true },
  payload: { type: Schema.Types.Mixed, required: true }, failureType: { type: String, enum: ["temporary", "permanent"] }, error: String, sentAt: Date, nextAttemptAt: Date,
}, { timestamps: true });
