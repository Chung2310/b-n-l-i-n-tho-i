...
export async function listWarrantyGapRisk(scope: RetailScope, at = new Date()) {
  ...
  const variantIds = [...new Set(units.map((unit) => unit.variantId).filter(Boolean))]; const variants: any[] = await ProductVariantModel.find({ companyCode: scope.companyCode, _id: { $in: variantIds } }).select("_id warrantyMonths").lean(); const monthsByVariant = new Map(variants.map((variant) => [String(variant._id), Number(variant.warrantyMonths || 0)]));
  return units.filter((unit) => { const months = monthsByVariant.get(String(unit.variantId)) || 0; return unit.supplierWarranty?.endAt && months > 0 && new Date(unit.supplierWarranty.endAt) < computeWarrantyEnd(at, months); }).map((unit) => { const months = monthsByVariant.get(String(unit.variantId)) || 0; return { ...unit, gapMonths: Math.max(0, Math.ceil((computeWarrantyEnd(at, months).getTime() - new Date(unit.supplierWarranty.endAt).getTime()) / (30 * 86_400_000))) }; });
}
