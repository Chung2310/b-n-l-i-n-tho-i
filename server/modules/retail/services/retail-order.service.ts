...
export const RetailOrderService = {
  ...
  async list(scope: RetailBranchScope, query: any) {
    ...
    const filter: any = { ...scope }; for (const key of ["status", "paymentStatus", "customerId", "shiftId"]) if (query[key]) filter[key] = query[key];
    const [items, total] = await Promise.all([RetailOrderModel.find(filter).sort({ createdAt: -1 }).skip((page - 1) * limit).limit(limit).lean(), RetailOrderModel.countDocuments(filter)]); return { items, total, page, limit };
  },
  async detail(scope: RetailBranchScope, id: string) { if (!Types.ObjectId.isValid(id)) throw new Error("Mã đơn không hợp lệ."); const order = await RetailOrderModel.findOne({ _id: id, ...scope }).lean(); if (!order) throw new Error("Không tìm thấy đơn hàng."); return order; },
  async createDraft(scope: RetailBranchScope, input: any, actor: any) {
    const { pricing } = await priceInput(scope, input); return RetailOrderModel.create({ ...scope, items: pricing.lines, subtotal: pricing.subtotal, orderDiscount: pricing.orderDiscount, taxRate: pricing.taxRate, taxAmount: pricing.taxAmount, shippingFee: pricing.shippingFee, grandTotal: pricing.grandTotal, totalCost: pricing.totalCost, payments: [], refunds: [], paidAmount: 0, refundedAmount: 0, dueAmount: pricing.grandTotal, paymentStatus: "unpaid", status: "draft", salespersonId: String(input.salespersonId || actorId(actor)), salespersonName: String(input.salespersonName || actorName(actor)), createdBy: actorId(actor), createdByName: actorName(actor), stockApplied: false, version: 0, customerId: input.customerId, dueDate: input.dueDate });
  },
  async updateDraft(scope: RetailBranchScope, id: string, input: any) {
    const { pricing } = await priceInput(scope, input); const order = await RetailOrderModel.findOneAndUpdate({ _id: id, ...scope, status: "draft" }, { $set: { items: pricing.lines, subtotal: pricing.subtotal, orderDiscount: pricing.orderDiscount, taxRate: pricing.taxRate, taxAmount: pricing.taxAmount, shippingFee: pricing.shippingFee, grandTotal: pricing.grandTotal, totalCost: pricing.totalCost, dueAmount: pricing.grandTotal, customerId: input.customerId, dueDate: input.dueDate }, $inc: { version: 1 } }, { new: true }); if (!order) throw Object.assign(new Error("Đơn hàng không thể chỉnh sửa."), { code: "ORDER_NOT_EDITABLE", status: 409 }); return order;
  },
  async confirm(scope: RetailBranchScope, id: string, input: any, actor: any, shift: any) {
  ...
  async cancel(scope: RetailBranchScope, id: string, input: any, actor: any, shift: any | undefined, canManage: boolean) {
    const reason = String(input.reason || "").trim(); if (!reason) throw new Error("Lý do hủy là bắt buộc."); const session = await mongoose.startSession(); let result: any; try { await session.withTransaction(async () => { const order: any = await RetailOrderModel.findOne({ _id: id, ...scope, status: { $in: ["draft", "confirmed", "completed"] } }).session(session); if (!order) throw new Error("Đơn không thể hủy."); if (order.status === "completed" && !canManage) throw Object.assign(new Error("Chỉ quản lý được hủy đơn hoàn tất."), { status: 403 }); if (order.paidAmount > order.refundedAmount && !shift) throw Object.assign(new Error("Bạn chưa mở ca bán hàng."), { code: "SHIFT_NOT_OPEN", status: 409 }); const remainingRefund = order.paidAmount - order.refundedAmount; const refunds = remainingRefund > 0 ? normalizePayments(input.refunds || [], remainingRefund) : { payments: [], total: 0 }; if (refunds.total !== remainingRefund) throw new Error("Phải ghi nhận đủ số tiền hoàn khi hủy đơn."); if (order.stockApplied && !order.stockRevertedAt) { await revertOrderStock(scope, String(order._id), order.orderCode, order.items, actorName(actor), session); order.stockRevertedAt = new Date(); } order.refunds.push(...refunds.payments.map((item: any) => ({ method: item.method, amount: item.amount, reference: item.reference, refundedAt: new Date(), refundedBy: actorId(actor), refundedByName: actorName(actor), shiftId: String(shift?._id || ""), businessDate: shift?.businessDate || order.businessDate, reason }))); order.refundedAmount += refunds.total; order.paymentStatus = paymentStatusFor(order.paidAmount, order.grandTotal, order.refundedAmount); order.status = "cancelled"; order.cancelReason = reason; order.cancelledAt = new Date(); order.version += 1; await order.save({ session }); result = order; }); } finally { await session.endSession(); } return result;
  },
};
