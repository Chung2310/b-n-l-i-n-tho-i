...
export const RetailOrderService = {
  ...
  async confirm(scope: RetailBranchScope, id: string, input: any, actor: any, shift: any, canManage = false) {
    ...
    try { await session.withTransaction(async () => {
      ...
      await claimSerialsForOrder(scope, draft.items as any, String(draft._id), String(draft.customerId), actorId(actor), session, actorName(actor)); await draft.save({ session });
