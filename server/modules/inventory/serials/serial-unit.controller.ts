...
export const serialUnitController = {
  list: async (req: Request, res: Response) => { try { return res.json({ status: "success", data: await listSerialUnits(scope(req), { serial: String(req.query.serial || ""), sku: String(req.query.sku || ""), productId: String(req.query.productId || ""), variantId: String(req.query.variantId || ""), trackingMode: req.query.trackingMode as any, status: req.query.status as any, page: Number(req.query.page), limit: Number(req.query.limit) }) }); } catch (e) { return sendError(res, e); } },
