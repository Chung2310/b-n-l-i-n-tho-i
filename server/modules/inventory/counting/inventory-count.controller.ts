...
export const inventoryCountController = {
  ...
  scan: async (req: Request, res: Response) => { try { return res.json({ status: "success", data: await scanCountUnit(scope(req), req.params.id, req.body?.code) }); } catch (error) { return sendError(res, error); } },
