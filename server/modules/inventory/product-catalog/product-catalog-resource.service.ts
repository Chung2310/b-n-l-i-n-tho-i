...
function normalizeResourceInput(kind: ProductCatalogResourceKind, input: any, actor: string): Record<string, unknown> {
  ...
  if (kind === "units") {
    ...
    if (conversionFactor !== undefined && (!Number.isFinite(conversionFactor) || conversionFactor <= 0)) throw new ProductCatalogValidationError("conversionFactor phải lớn hơn 0.");
...
export const ProductCatalogResourceService = {
  ...
  async update(companyCodeValue: unknown, kindValue: unknown, id: string, input: unknown, actorValue: unknown): Promise<ResourceDocument> {
    ...
    const document = await models[kindValue].findOneAndUpdate({ _id: id, companyCode }, { $set: normalized }, { new: true, runValidators: true });
