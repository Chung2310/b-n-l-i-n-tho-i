...
import {
  ...
  MAX_CUSTOM_FIELD_PATTERN_LENGTH,
...
const safePatternSchema = Joi.string()
  .max(MAX_CUSTOM_FIELD_PATTERN_LENGTH)
  .custom((value, helpers) =>
    isSafeCustomFieldPattern(value)
      ? value
      : helpers.error("string.pattern.unsafe"),
  );
...
function withTypeSpecificValidation(schema: Joi.ObjectSchema) {
  const typedSchema = schema
    .when(Joi.object({ type: Joi.valid(...textTypes) }).unknown(), {
      then: Joi.object({ validation: textValidationSchema }),
    })
    .when(Joi.object({ type: Joi.valid(...numericTypes) }).unknown(), {
      then: Joi.object({ validation: numericValidationSchema }),
    })
    .when(Joi.object({ type: Joi.valid(...dateTypes) }).unknown(), {
      then: Joi.object({ validation: dateValidationSchema }),
    })
    .when(Joi.object({ type: Joi.valid(...fileTypes) }).unknown(), {
      then: Joi.object({ validation: fileValidationSchema }),
    })
    .when(
      Joi.object({
        type: Joi.valid("singleSelect", "multiSelect", "checkbox", "switch"),
      }).unknown(),
      { then: Joi.object({ validation: emptyValidationSchema }) },
    );

  return typedSchema.custom((value, helpers) => {
    const validation = (helpers.original as { validation?: unknown })
      .validation;
    if (
      validation &&
      typeof validation === "object" &&
      ["__proto__", "prototype", "constructor"].some((key) =>
        Object.prototype.hasOwnProperty.call(validation, key),
      )
    ) {
...
function requireSelectOptions(schema: Joi.ObjectSchema) {
  ...
  return schema.when(
    Joi.object({ type: Joi.valid(...selectTypes) }).unknown(),
    {
...
export const createFieldSchema = requireSelectOptions(
  withTypeSpecificValidation(
    Joi.object({
      ...sharedFieldSchema,
...
export const updateFieldSchema = requireSelectOptions(
  withTypeSpecificValidation(
    Joi.object(sharedFieldSchema).min(1).unknown(false),
  ),
);
