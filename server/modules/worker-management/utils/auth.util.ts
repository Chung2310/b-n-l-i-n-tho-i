...
export function requireStudentBranch(user: StudentModuleUser): string {
  ...
  if (!branchId) {
    const error: Error & { status?: number } = new Error(
      "Vui lòng chọn chi nhánh trước khi thao tác.",
    );
    error.status = 400;
    throw error;
  }
...
async function getCompanyUserIds(companyCode?: string, branchId?: string) {
  ...
  const users = await User.find(
    buildCompanyUserFilter(companyCode, branchId),
  ).select("_id");
  return users.map((user) => user._id.toString());
}

async function getCompanyPrimaryOwnerId(
  companyCode?: string,
  branchId?: string,
) {
  ...
  const adminUser = await User.findOne({ ...companyFilter, role: "admin" })
    .sort({ createdAt: 1 })
    .select("_id");
  if (adminUser) {
  ...
  const managerUser = await User.findOne({ ...companyFilter, role: "manager" })
    .sort({ createdAt: 1 })
    .select("_id");
  if (managerUser) {
  ...
  const anyUser = await User.findOne(companyFilter)
    .sort({ createdAt: 1 })
    .select("_id");
...
export async function getAllowedOwnerIds(
  ...
  const companyUserIds = await getCompanyUserIds(
    user.companyCode || user.centerId,
    user.branchId,
  );
  const branchOwnerIds = user.branchId
    ? [...companyUserIds, user.branchId]
    : companyUserIds;
  if (user.role === "admin") {
...
export async function getCenterOwnerIds(
  user: StudentModuleUser,
): Promise<string[] | string> {
  if (user.role === "superadmin") {
    return "ALL";
...
export async function resolveCreateOwnerId(
  ...
  if (user.role === "superadmin") {
    const companyCode =
      requestedCompanyCode || user.companyCode || user.centerId;
    const companyOwnerId = await getCompanyPrimaryOwnerId(
      companyCode,
      user.branchId,
    );
    if (companyOwnerId) {
