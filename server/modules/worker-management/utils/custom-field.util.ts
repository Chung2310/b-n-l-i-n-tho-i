...
export async function resolveCustomFieldTenant(
  user: TenantUser,
  requestedTenant?: unknown,
): Promise<string> {
  const requested =
    typeof requestedTenant === "string" && requestedTenant.trim()
      ? requestedTenant.trim()
      : undefined;
  const ownTenant =
    user.companyCode ||
    (user.centerId && user.centerId !== "SYSTEM" ? user.centerId : undefined);

  if (user.role === "superadmin") {
    if (!requested)
      throw tenantError("Vui lòng chọn công ty cho trường tùy chỉnh.", 400);
  ...
  if (!ownTenant)
    throw tenantError("Không thể xác định công ty cho trường tùy chỉnh.", 400);
  if (requested) {
    const canonical = await canonicalTenant(requested);
    if (!canonical || canonical !== ownTenant)
      throw tenantError(
        "Bạn không có quyền truy cập trường tùy chỉnh của công ty này.",
        403,
      );
  }
  return ownTenant;
...
export async function resolveCustomFieldTenantForOwner(
  ownerId: unknown,
): Promise<string> {
