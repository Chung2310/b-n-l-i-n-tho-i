...
router.post(
  ...
  QRAttendanceController.checkin,
);
router.get(
  "/session-info",
  publicApiRateLimiter,
  QRAttendanceController.getSessionInfo,
);

// Các routes cần đăng nhập (Giảng viên / Admin)
router.use(authMiddleware as any);
router.use(
  requireAnyPermission([...STUDENT_AREA_PERMISSIONS.assignment.read]) as any,
);
router.post(
  "/session",
  requireAnyPermission([...STUDENT_AREA_PERMISSIONS.assignment.manage]) as any,
  QRAttendanceController.createSession,
);
...
router.post(
  ...
  requireAnyPermission([...STUDENT_AREA_PERMISSIONS.assignment.manage]) as any,
  QRAttendanceController.closeSession,
);
