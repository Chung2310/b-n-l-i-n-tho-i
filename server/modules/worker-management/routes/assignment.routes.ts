...
const requireManage = requireAnyPermission([
  ...STUDENT_AREA_PERMISSIONS.assignment.manage,
]) as any;
const requireRead = requireAnyPermission([
  ...STUDENT_AREA_PERMISSIONS.assignment.read,
]) as any;
...
router.post(
  "/public/upload",
  validate(uploadProofFileSchema),
  AssignmentController.uploadProofFile,
);
router.post(
  "/public/submit",
  validate(submitProofSchema),
  AssignmentController.submitProof,
);
router.post("/public/cancel", AssignmentController.cancelSubmission);

// Private routes for Teachers & Admins
router.use(authMiddleware);
router.use(requireModule("worker"));
