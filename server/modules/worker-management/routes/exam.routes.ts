...
const requireManage = requireAnyPermission([
  ...STUDENT_AREA_PERMISSIONS.exam.manage,
]) as any;
...
router.post(
  ...
  ExamController.unassign,
);
router.post(
  "/:id/students/:studentId/result",
  requireManage,
  validate(examStudentParamsSchema, "params"),
  validate(updateStudentResultSchema),
  ExamController.updateStudentResult,
);
router.post(
  "/:id/import-results",
  requireManage,
  validate(idParamSchema, "params"),
  validate(importResultsSchema),
  ExamController.importResults,
);
