...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const smtpSettings = user?.companyCode
        ? await companyEmailService.resolveLegacySettings(user.companyCode)
        : undefined;
      ...
      if (check) {
        ...
        if (!checkResult.success) {
          return res.status(400).json({
            success: false,
            error:
              checkResult.error === "SMTP_CONFIG_missing"
      ...
      if (!to || !subject || !html) {
        return res
          ...
          .json({
            success: false,
            error: "Thiếu thông tin (to, subject, html)",
          });
      }
      ...
      if (!result.success) {
        if (result.error === "SMTP_CONFIG_missing") {
          return res.status(400).json({
            success: false,
            error:
              "Hệ thống chưa được cấu hình máy chủ gửi thư SMTP. Vui lòng kiểm tra lại cấu hình.",
          });
        }
