...
const requireManage = requireAnyPermission([
  ...STUDENT_AREA_PERMISSIONS["custom-field"].manage,
]) as any;
const customFieldUploadRateLimiter = rateLimit({
  ...
  message: {
    success: false,
    error: "Quá nhiều yêu cầu tải tệp. Vui lòng thử lại sau.",
  },
});
...
router.post(
  "/:moduleKey/:id/archive",
  requireManage,
  validate(fieldParamsSchema, "params"),
  CustomFieldController.archive,
);
router.post(
  "/:moduleKey/:id/restore",
  requireManage,
  validate(fieldParamsSchema, "params"),
  CustomFieldController.restore,
);
router.post(
  "/:moduleKey/:id/upload",
  ...
  (req: Request, res: Response, next: NextFunction) =>
    customFieldUpload.single("file")(req, res, (error) => {
      if (error)
