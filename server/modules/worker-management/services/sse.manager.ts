...
export class SSEManager {
  ...
  addConnection(ownerId: string, res: Response) {
    ...
    logger.info(
      `[SSE] New client connected for ownerId: ${ownerId}. Active connections: ${this.connections.get(ownerId)!.length}`,
    );
  }
  ...
  removeConnection(ownerId: string, res: Response) {
    ...
    if (index !== -1) {
      ...
      logger.info(
        `[SSE] Client disconnected for ownerId: ${ownerId}. Active connections: ${clients.length}`,
      );
    }
    if (clients.length === 0) {
  ...
  broadcast(ownerId: string, event: string, data: unknown) {
    ...
    if (!clients || clients.length === 0) {
      logger.warn(
        `[SSE] No active clients for ownerId: ${ownerId} to broadcast event: ${event}`,
      );
      return;
    }
    ...
    logger.info(
      `[SSE] Broadcasted event "${event}" to ${clients.length} clients of ownerId: ${ownerId}`,
    );
  }
}
