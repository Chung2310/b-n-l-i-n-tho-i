...
export class CourseService {
  ...
  static async createCourse(
    ...
    logger.info(
      `[Course] Creating course for ownerId=${ownerId}, code=${data.code}`,
    );
    const writeData = await this.customFieldWrites.prepareCreate(context, data);
    const existing = await Course.findOne({
      ownerId,
      branchId: writeData.branchId,
    ...
    const course = new Course({
      ...writeData,
      fee: normalizeCourseFee(writeData.fee),
      ownerId,
    });
  ...
  static async getCourses(
    ...
    const activeCounts = await BatchService.countActiveByCourse(
  ...
  static async getCourseById(
    ...
    return {
      ...course.toObject(),
      activeBatches: activeCounts.get(String(course._id)) || 0,
    };
  }
  ...
  static async updateCourse(
    ...
    const targetContext =
      context.actorRole === "superadmin"
        ? {
            ...context,
            tenantId: await resolveCustomFieldTenantForOwner(existing.ownerId),
          }
        : context;
    ...
    const normalizedData = {
      ...writeData,
      ...(Object.prototype.hasOwnProperty.call(writeData, "fee")
        ? { fee: normalizeCourseFee(writeData.fee) }
        : {}),
    };
  ...
  static async deleteCourse(
    ...
    if ((activeCounts.get(id) || 0) > 0) {
      throw new Error(
        "Không thể xóa: khóa học đang có lớp hoạt động. Hãy kết thúc hoặc xóa các lớp trước.",
      );
    }
