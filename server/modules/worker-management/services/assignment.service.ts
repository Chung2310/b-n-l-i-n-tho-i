  ...
  try {
    ...
    if (!companyCode) {
      const owner = await User.findOne({
        $or: [{ companyCode: ownerId }, { uid: ownerId }],
      }).select("companyCode");
      companyCode = owner?.companyCode || ownerId;
    }
    return companyCode
      ? await companyEmailService.resolveLegacySettings(companyCode)
      : undefined;
  } catch (err) {
...
export class AssignmentService {
  ...
  static async notifyStudents(
    ...
    try {
      ...
      if (studentIds.length === 0) {
        logger.info(
          `[Assignment Email] Lớp ${batch.code} chưa có học viên nào để gửi email.`,
        );
        return;
      }
      ...
      logger.info(
        `[Assignment Email] Đã gửi ${sentCount} email cho học viên lớp ${batch.code} (${failedCount} thất bại).`,
      );
    } catch (error) {
  ...
  private static async validateSubmissionContext(
    decodedToken: any,
  ): Promise<any> {
  ...
  static async getPublicContext(decodedToken: any): Promise<any> {
    ...
    const [student, submission] = await Promise.all([
      Student.findOne({
        _id: decodedToken.studentId,
        ownerId: assignment.ownerId,
      }),
      SubmissionModel.findOne({
        assignmentId: decodedToken.assignmentId,
  ...
  static async uploadProofFile(
    ...
    return cloudinaryService.uploadMedia(
      fileBase64,
      "igen_erp/assignments/submissions",
    );
  }
