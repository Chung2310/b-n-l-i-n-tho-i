...
export async function verifyStudentAttendanceFace(
  input: GateInput,
  deps: GateDependencies,
) {
  const evidence = await deps.cloudinary.uploadPrivateImage(
    input.image,
    "student-attendance/evidence",
  );
  const verification = await deps.insightFace.verifyEmployee(
    input.insightFaceUserId,
    input.image,
    input.mimeType,
  );
  const accepted =
    ...
    verification.reasonCode === "verified";
  return {
    accepted,
    reasonCode: verification.reasonCode as FaceReasonCode,
    evidence,
    evidenceDeleteAfter: accepted
      ? undefined
