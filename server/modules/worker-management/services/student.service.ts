...
export function buildAssignableBranchQuery(
  branchId: string,
  companyCode: string,
) {
  return {
...
async function ensureUniqueFieldsInScope(
  ...
  const checks: Array<{
  ...
  }> = [
    {
      ...
      message:
        "Email này đã tồn tại trong trung tâm hiện tại. Vui lòng kiểm tra lại!",
    },
    {
      field: "phone",
      value: normalizePhone(String(data.phone || "")),
      message:
        "Số điện thoại này đã tồn tại trong trung tâm hiện tại. Vui lòng kiểm tra lại!",
    },
    {
      field: "idCard",
      value: normalizeIdCard(String(data.idCard || "")),
      message:
        "CCCD/CMND này đã tồn tại trong trung tâm hiện tại. Vui lòng kiểm tra lại!",
    },
  ];
...
export class StudentService {
  ...
  static async createStudent(
    ...
    logger.info(
      `[Student] Creating student for ownerId=${ownerId}, phone=${data.phone}`,
    );
    ...
    const normalizedPayload = {
      ...writeData,
      email:
        typeof writeData.email === "string"
          ? normalizeEmail(writeData.email)
          : writeData.email,
      phone: normalizePhone(String(writeData.phone ?? "")),
      idCard:
        typeof writeData.idCard === "string"
          ? normalizeIdCard(writeData.idCard)
          : writeData.idCard,
      fee: normalizeFee(writeData.fee),
      courseId:
        typeof writeData.courseId === "string"
          ? writeData.courseId.trim()
          : writeData.courseId,
    };

    const branchId =
      typeof writeData.branchId === "string" ? writeData.branchId : undefined;
    ...
    logger.info(
      `[Student] Student created successfully: id=${savedStudent._id}, phone=${savedStudent.phone}`,
    );
    return savedStudent;
  }
  ...
  static async getStudents(
    ...
    logger.info(
      `[Student] Fetching students list for ownerId=${ownerId} with filters: ${JSON.stringify(filters)}`,
    );
    ...
    const resolvedOwnerId = await resolveOwnerFilter(
      ownerId,
      filters.ownerFilter,
    );
    ...
    logger.info(
      `[Student] Fetched ${students.length} students (total=${total}) for ownerId=${ownerId}`,
    );
    return {
      students,
      total,
  ...
  static async assignUnassignedStudentBranch(
    ...
    const branch = await BranchModel.findOne(
      buildAssignableBranchQuery(branchId, companyCode),
    )
      .select("_id")
      .lean();
  ...
  static async getStudentById(
    ...
    logger.info(
      `[Student] Fetching student detail: id=${id}, ownerId=${ownerId}`,
    );
    const query: Record<string, unknown> = {
      _id: id,
      ...buildOwnerScopeQuery(ownerId),
  ...
  static async updateStudent(
    ...
    const targetContext =
      context.actorRole === "superadmin"
        ? {
            ...context,
            tenantId: await resolveCustomFieldTenantForOwner(
              existingStudent.ownerId,
            ),
          }
        : context;
    const writeData = await this.customFieldWrites.prepareUpdate(
      targetContext,
      existingStudent,
      data,
    );
    ...
    if (writeData.paymentHistory && Array.isArray(writeData.paymentHistory)) {
      ...
      writeData.paidAmount = history.reduce(
    ...
    if (writeData.paymentHistory && Array.isArray(writeData.paymentHistory)) {
      try {
        ...
        const newHistory = writeData.paymentHistory as Record<
          string,
          unknown
        >[];
        ...
        for (const oldItem of oldHistory) {
          const stillExists = newHistory.some(
            (newItem) => String(newItem.id) === String(oldItem.id),
          );
          if (!stillExists) {
        ...
        for (const newItem of newHistory) {
          const oldItem = oldHistory.find(
            (item) => String(item.id) === String(newItem.id),
          );
          if (!oldItem) continue;
          const amountChanged =
            Number(newItem.amount) !== Number(oldItem.amount);
          const dateChanged = String(newItem.date) !== String(oldItem.date);
          const noteChanged =
            String(newItem.note || "") !== String(oldItem.note || "");
        ...
        logger.error(
          `[Student] Failed to sync paymentHistory changes with Payment collection: %o`,
          err,
        );
      }
    }
  ...
  static async deleteStudent(
    ...
    if (deletedStudent) {
      ...
      try {
        ...
        logger.error(
          `[Student] Failed to clean up associated records for deleted student: %o`,
          err,
        );
      }
    } else {
      logger.warn(
        `[Student] Student delete failed/not found: id=${id}, ownerId=${ownerId}`,
      );
    }
    return deletedStudent;
  }

  static async bulkDeleteStudents(
    ownerId: string | string[],
    ids: string[],
    branchId?: string,
  ): Promise<number> {
    ...
    try {
      ...
      await Batch.updateMany(
        { learnerIds: { $in: resolvedIds } },
        { $pull: { learnerIds: { $in: resolvedIds } } },
      );
    } catch (err) {
      logger.error(
        `[Student] Failed to clean up associated records for bulk deleted students: %o`,
        err,
      );
    }
  ...
  static async bulkCreateStudents(
    ...
    logger.info(
      `[Student] Bulk importing ${studentsData.length} students: creatorId=${creatorId}, ownerId=${ownerId}, targetOwnerId=${targetOwnerId}`,
    );
    ...
    const existingStudents =
      await Student.find(query).select("phone email idCard");
    const existingPhones = new Set(
      existingStudents.map((s) => normalizePhone(s.phone)),
    );
    ...
    const existingIdCards = new Set(
      existingStudents
        .map((s) => normalizeIdCard(s.idCard || ""))
        .filter(Boolean),
    );
    ...
    for (let i = 0; i < studentsData.length; i++) {
      ...
      if (!fullName) {
        errors.push({
          ...
          name: fullName,
          phone,
          reason: "Họ và tên không được để trống.",
        });
        skippedCount++;
        continue;
      }
      ...
      if (seenPhonesInBatch.has(phone)) {
        errors.push({
          ...
          name: fullName,
          phone,
          reason: "Số điện thoại bị trùng lặp trong file import.",
        });
        skippedCount++;
        continue;
      }
      ...
      if (existingPhones.has(phone)) {
        errors.push({
          ...
          name: fullName,
          phone,
          reason: "Số điện thoại đã tồn tại trong trung tâm hiện tại.",
        });
        skippedCount++;
        continue;
      }
      ...
      const registrationDate = String(
        data.registrationDate || new Date().toLocaleDateString("vi-VN"),
      ).trim();
      ...
      if (email && existingEmails.has(email)) {
        errors.push({
          ...
          name: fullName,
          phone,
          reason: "Email đã tồn tại trong trung tâm hiện tại.",
        });
        skippedCount++;
        continue;
      }
      ...
      if (idCard && existingIdCards.has(idCard)) {
        errors.push({
          ...
          name: fullName,
          phone,
          reason: "CCCD/CMND đã tồn tại trong trung tâm hiện tại.",
        });
        skippedCount++;
        continue;
      }
      ...
      const paidAmount =
        parseInt(String(data.paidAmount || "0").replace(/\D/g, ""), 10) || 0;
    ...
    if (validStudents.length > 0) {
      ...
      logger.info(
        `[Student] Bulk import complete: successfully imported ${importedCount} students, skipped ${skippedCount} students`,
      );
    } else {
      logger.info(
        `[Student] Bulk import complete: no valid students to import. Skipped ${skippedCount} students`,
      );
    }
  ...
  static async markInstallmentPaid(
    ...
    logger.info(
      `[Student] Mark installment paid: studentId=${studentId}, installmentNo=${installmentNo}, ownerId=${ownerId}`,
    );
    ...
    if (!student) {
      logger.warn(
        `[Student] markInstallmentPaid: student not found, id=${studentId}`,
      );
    ...
    if (!student.installmentStatus || student.installmentStatus.length === 0) {
      return {
        success: false,
        error: "Học viên này chưa có lịch sử đợt thu học phí.",
      };
    }
    ...
    if (idx < 0) {
      return {
        success: false,
        error: `Không tìm thấy đợt ${installmentNo} cho học viên này.`,
      };
    }
    ...
    logger.info(
      `[Student] Installment ${installmentNo} marked as paid for student ${studentId}`,
    );
    return { success: true };
  }
