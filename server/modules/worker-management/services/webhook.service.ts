...
export class WebhookService {
   ...
   */
  static async matchStudentByDescription(
    description: string,
    ownerId: string,
    branchId?: string,
  ) {
    logger.info(
      `[Webhook] Matching student for ownerId: ${ownerId}, description: "${description}"`,
    );
    ...
    const companyUsers = companyCode
      ? await User.find({ companyCode }).select("_id")
      : [];
    const allowedOwnerIds = [
      ...new Set([ownerId, ...companyUsers.map((u) => u._id.toString())]),
    ];
    const branchScope = branchId ? { branchId } : {};
    logger.info(
      `[Webhook] adminUser companyCode: ${companyCode}, allowedOwnerIds: ${JSON.stringify(allowedOwnerIds)}`,
    );
    ...
    if (objectIdMatch) {
      ...
      if (student) {
        logger.info(
          `[Webhook] Matched student by ID: ${student.fullName} (${student._id})`,
        );
        return student;
      }
      ...
      if (studentAnyOwner) {
        logger.warn(
          `[Webhook] Found studentId ${studentId} (${studentAnyOwner.fullName}) but its ownerId "${studentAnyOwner.ownerId}" is outside allowedOwnerIds for this center.`,
        );
      } else {
        logger.warn(
          `[Webhook] Extracted studentId ${studentId} from description but no student exists with this ID at all.`,
        );
      }
    }
      ...
      for (const num of digitMatches) {
        ...
        for (const student of students) {
          ...
          if (
            ...
            logger.info(
              `[Webhook] Matched student by Phone: ${student.fullName} (${student._id}) via SĐT match: ${num}`,
            );
            return student;
          }
        }
    ...
    logger.warn(
      `[Webhook] No student matched for description: "${description}"`,
    );
    return null;
  }
  ...
  static async processIncomingTransaction(payload: {
    ...
    logger.info(
      `[Webhook] Processing incoming transaction payload: ${JSON.stringify(payload)}`,
    );
    ...
    const dateStr =
      payload.when || payload.transactionDate || new Date().toISOString();
    const date = dateStr.substring(0, 10); // Lấy YYYY-MM-DD
    const bankAccountNo = (payload.subAccount || payload.accountNumber || "")
      .toString()
      .trim();
    ...
    if (!user) {
      logger.error(
        `[Webhook] No user found with bankAccountNo: "${bankAccountNo}"`,
      );
      throw new Error(
        `Không tìm thấy admin nào có tài khoản ngân hàng: ${bankAccountNo}`,
      );
    }
    ...
    const student = await this.matchStudentByDescription(
      description,
      ownerId,
      branchId,
    );
    if (!student) {
      return {
        success: false,
        message: "Không khớp được học viên từ nội dung chuyển khoản.",
      };
    }
    ...
    if (remaining <= 0) {
      logger.info(
        `[Webhook] Student ${student.fullName} has already paid in full. No payment recorded.`,
      );
      return {
        success: true,
        message: "Học viên đã hoàn thành học phí từ trước.",
