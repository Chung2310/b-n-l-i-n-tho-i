...
export function buildInstructorAssignmentQuery(
  actor: BatchActor,
  instructorId: unknown,
): Record<string, unknown> {
  const companyCode = actor.companyCode || actor.centerId;
  if (!companyCode)
    throw new Error("Không xác định được công ty khi gán người phụ trách.");
  if (!actor.branchId)
    throw new Error("Vui lòng chọn chi nhánh trước khi gán người phụ trách.");
  return {
    _id: instructorId,
    companyCode,
...
async function assertInstructorAssignable(
  actor: BatchActor,
  instructorId: unknown,
) {
  if (!instructorId) return;
  const instructor = await User.findOne(
    buildInstructorAssignmentQuery(actor, instructorId),
  );
  if (!instructor) {
    throw new Error(
      "Không tìm thấy tài khoản đang hoạt động trong chi nhánh được chọn.",
    );
  }
}
...
function normalizeInstructorFields(data: BatchData): BatchData {
  ...
  if (hasText)
    return {
      ...data,
      instructorId: "",
      instructorText: String(data.instructorText ?? "").trim(),
    };
...
function assertScheduleValid(data: BatchData) {
  ...
  if (
    data.startDate &&
    data.endDate &&
    String(data.startDate) > String(data.endDate)
  ) {
...
async function enrichBatches(batches: IBatch[]): Promise<EnrichedBatch[]> {
  ...
  const instructorIds = [
    ...new Set(batches.map((b) => b.instructorId).filter(Boolean)),
  ];
  ...
  return batches.map((b) => {
    ...
    const instructor = b.instructorId
      ? instructorMap.get(b.instructorId)
      : undefined;
    return {
      ...b.toObject(),
 ...
 */
async function resolveSmtpForOwner(
  ownerId: string,
): Promise<SmtpSettings | undefined> {
...
function buildInstructorAssignmentHtml(
  instructorName: string,
  batch: EnrichedBatch,
): string {
 ...
 */
async function notifyInstructorAssigned(
  ownerId: string,
  instructorId: string,
  batch: EnrichedBatch,
): Promise<void> {
  try {
    const instructor =
      await User.findById(instructorId).select("email displayName");
    if (!instructor || !instructor.email) {
      logger.warn(
        `[Batch] Skip instructor email: no email for instructorId=${instructorId}`,
      );
      return;
    }
    ...
    const result = await EmailService.sendMail(
      {
        ...
        html: buildInstructorAssignmentHtml(
    ...
    if (result.success) {
      logger.info(
        `[Batch] Instructor assignment email sent to ${instructor.email} for batch=${batch.code}`,
      );
    } else {
      logger.warn(
        `[Batch] Instructor assignment email failed (${result.error}) for batch=${batch.code}`,
      );
    }
  } catch (error: unknown) {
...
export class BatchService {
  ...
  static async createBatch(
    ...
    logger.info(
      `[Batch] Creating batch for ownerId=${ownerId}, code=${data.code}`,
    );
    ...
    const existing = await Batch.findOne({
      ownerId,
      branchId: actor.branchId,
      code: String(writeData.code || "").toUpperCase(),
    });
    if (existing) {
    ...
    const batch = new Batch({
      ...normalizeInstructorFields(writeData),
      ownerId,
      branchId: actor.branchId,
    });
  ...
  static async getBatches(
    ...
    const query: Record<string, unknown> = {
      ...buildOwnerQuery(resolvedOwnerId),
    ...
    return {
      batches: await enrichBatches(batches),
      total,
      page,
      limit,
  ...
  static async updateBatch(
    ...
    const targetContext =
      context?.actorRole === "superadmin"
        ? {
            ...context,
            tenantId: await resolveCustomFieldTenantForOwner(batch.ownerId),
          }
        : context;
    const writeData = targetContext
      ? await this.customFieldWrites.prepareUpdate(targetContext, batch, data)
      : data;
    ...
    if (
      writeData.instructorId &&
      writeData.instructorId !== batch.instructorId
    ) {
      await assertInstructorAssignable(actor, writeData.instructorId);
    }
  ...
  static async deleteBatch(
    ...
    return await Batch.findOneAndDelete({
      _id: id,
      ...buildOwnerQuery(ownerId),
      ...buildBranchScopeQuery(branchId),
    });
  ...
  static async addLearner(
    ...
    if (
      course &&
      course.maxLearners > 0 &&
      batch.learnerIds.length >= course.maxLearners
    ) {
      throw new Error(
        `Lớp đã đạt sĩ số tối đa (${course.maxLearners} học viên).`,
      );
    }
  ...
  static async removeLearner(
    ...
    const saved = await batch.save();
    logger.info(
      `[Batch] Learner removed: batchId=${id}, studentId=${studentId}`,
    );
    return (await enrichBatches([saved]))[0];
  }
  ...
  static async countActiveByInstructor(
  ...
  static async getClassEventsInRange(
    ownerId: string | string[],
    from?: string,
    to?: string,
    branchId?: string,
    ...
    for (const b of enriched) {
      ...
      const learnerCount = Array.isArray(b.learnerIds)
        ? b.learnerIds.length
        : 0;
      const detailParts = [
  ...
  static async saveAttendanceSession(
    ...
    if (!session) {
      ...
      batch.attendanceSessions.push(
        newSessionObj as unknown as IAttendanceSession,
      );
  ...
  static async deleteAttendanceSessionByDate(
    ...
    const dateStr = date.trim();
    batch.attendanceSessions = batch.attendanceSessions.filter(
      (s) => s.date !== dateStr,
