...
export async function setWorkerStandardFields(scope: WorkerScope, moduleKey: string, fields: unknown[]) { return (WorkerStandardFieldModel as any).findOneAndUpdate({ ...scoped(scope), moduleKey }, { ...scoped(scope), moduleKey, fields }, { upsert: true, returnDocument: 'after', setDefaultsOnInsert: true }).lean(); }
