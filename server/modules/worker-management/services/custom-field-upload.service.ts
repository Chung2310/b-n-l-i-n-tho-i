...
const defaultUploadAsset: UploadAsset = (buffer, folder) =>
  new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { folder, resource_type: "auto" },
      (error, result) => {
        if (error || !result)
          reject(error ?? new Error("Không nhận được kết quả tải tệp."));
        else
          resolve({
...
export class CustomFieldUploadService {
  ...
  async upload(
    ...
    if (!field)
      throw Object.assign(new Error("Không tìm thấy trường tải tệp hợp lệ."), {
        status: 404,
      });
    if (!file?.buffer?.length)
      throw Object.assign(new Error("Tệp tải lên không hợp lệ."), {
        status: 400,
      });
    ...
    if (actualMime === "application/octet-stream")
      throw Object.assign(
        new Error("Không thể xác định định dạng thật của tệp."),
        { status: 400 },
      );
    if (
      file.mimetype &&
      file.mimetype !== "application/octet-stream" &&
      !mimeMatches(file.mimetype, actualMime)
    ) {
      throw Object.assign(
        new Error("Định dạng khai báo không khớp nội dung thật của tệp."),
        { status: 400 },
      );
    }
    if (
      ((field.type as string) === "image" ||
        (field.type as string) === "multiImage") &&
      !actualMime.startsWith("image/")
    ) {
      throw Object.assign(new Error("Trường này chỉ chấp nhận tệp ảnh."), {
        status: 400,
      });
    }
    const validation = field.validation ?? {};
    const maxSizeMb =
      typeof validation.maxSizeMb === "number" ? validation.maxSizeMb : 100;
    if (size > maxSizeMb * 1024 * 1024)
      throw Object.assign(
        new Error(`Dung lượng tệp không được vượt quá ${maxSizeMb} MB.`),
        { status: 400 },
      );
    const allowed = Array.isArray(validation.allowedMimeTypes)
      ? validation.allowedMimeTypes.filter(
          (item): item is string => typeof item === "string",
        )
      : [];
    if (
      allowed.length &&
      !allowed.some((mime) => mimeMatches(mime, actualMime))
    ) {
      throw Object.assign(
        new Error("Định dạng thật của tệp không được phép."),
        { status: 400 },
      );
    }
    ...
    const asset = await this.uploadAsset(
      file.buffer,
      `student_management/custom-fields/${safeTenant}/${moduleKey}/${field.key}`,
    );
    return {
