...
export class AuthService {
  ...
  static async register(data: RegisterData): Promise<IUser> {
    ...
    if (existingUser) {
      logger.warn(
        `[Auth] Registration failed - Email already exists: ${data.email}`,
      );
    ...
    logger.info(
      `[Auth] User registered successfully: email=${savedUser.email}, uid=${savedUser._id}`,
    );
    return savedUser;
  }
  ...
  static async login(data: LoginData) {
    ...
    if (!isPasswordValid) {
      logger.warn(
        `[Auth] Login failed - Invalid password for email: ${data.email}`,
      );
      throw new Error(
        "Email hoặc mật khẩu không chính xác. Vui lòng kiểm tra lại.",
      );
    ...
    logger.info(
      `[Auth] User logged in successfully: email=${user.email}, uid=${user._id}`,
    );
  ...
  static async verifyRefreshToken(token: string) {
    try {
      ...
      if (!user) {
        logger.warn(
          `[Auth] Refresh token verification failed - User not found for uid: ${decoded.uid}`,
        );
      ...
      logger.info(
        `[Auth] Refresh token verified successfully for email: ${user.email}`,
      );
      ...
      logger.error(
        `[Auth] Refresh token verification failed: ${error instanceof Error ? error.message : error}`,
      );
    ...
    return await User.findByIdAndUpdate(
      uid,
      {
        $set: {
          ...
          bankQrEnabled:
            data.bankQrEnabled !== undefined ? data.bankQrEnabled : true,
        },
      },
      { new: true },
    );
  ...
  static async updateBusinessSettings(
    uid: string,
    data: { businessType: "driving" | "language" | "general" },
  ): Promise<IUser | null> {
    logger.info(
      `[Auth] Updating business settings for uid: ${uid} to ${data.businessType}`,
    );
    return await User.findByIdAndUpdate(
      uid,
      {
  ...
  static async listUsers(requester: {
    uid: string;
    role: string;
    centerId: string;
  }) {
    ...
    const query =
      requester.role === "superadmin" ? {} : { centerId: requester.centerId };
  ...
  static async createManagedUser(
    ...
    if (requester.role === "admin") {
      ...
      const currentUserCount = await User.countDocuments({
        centerId: centerIdToCheck,
        role: "user",
      });
      if (currentUserCount >= limit) {
        throw new Error(
          `Trung tâm của bạn đã đạt giới hạn tối đa ${limit} tài khoản nhân viên.`,
        );
      }
    }
    ...
    const newUser = new User({
      ...
      centerId:
        requester.role === "admin"
          ? requester.centerId || requester.uid
          : data.centerId || "",
  ...
  static async updateManagedUser(
    ...
    if (requester.role === "admin") {
      ...
      if (userToEdit.role !== "user") {
        throw new Error(
          "Bạn chỉ có quyền sửa nhân viên thuộc trung tâm của mình.",
        );
      }
    }
    ...
    if (data.bankAccountNo !== undefined)
      updates.bankAccountNo = data.bankAccountNo;
    ...
    if (data.permissions !== undefined) {
      if (
        ...
        (requester.role === "admin" && userToEdit.role === "user")
      ) {
  ...
  static async deleteManagedUser(
    ...
    if (requester.role === "admin") {
      ...
      if (userToDelete.role !== "user") {
        throw new Error(
          "Bạn chỉ có quyền xóa nhân viên thuộc trung tâm của mình.",
        );
      }
    }
  ...
  static async seedAdmin() {
    ...
    const superadminEmail =
      process.env.SUPERADMIN_EMAIL || "superadmin@studentmanagement.com";
    const superadminPassword =
      process.env.SUPERADMIN_PASSWORD || "SuperAdminPass123";
    ...
    try {
      ...
      if (!existingSuper) {
        ...
        logger.info(
          `>>> Seeded superadmin account successfully: ${superadminEmail}`,
        );
      } else {
        if (
        ...
        logger.info(
          `>>> Superadmin account already exists: ${superadminEmail}`,
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (!existingAdmin) {
        ...
        if (!existingAdmin.centerId || existingAdmin.centerId === "undefined") {
          ...
          logger.info(
            `>>> Updated existing admin account centerId to 'system_admin'`,
          );
        } else {
    ...
    try {
      ...
      for (const admin of adminsToFix) {
        ...
        await admin.save();
        logger.info(
          `>>> Migrated admin user ${admin.email} centerId to ${admin.centerId}`,
        );
      }
      ...
      for (const sa of superadminsToFix) {
        ...
        await sa.save();
        logger.info(
          `>>> Migrated superadmin user ${sa.email} centerId to superadmin`,
        );
      }
      ...
      for (const staff of staffUsers) {
        ...
        if (targetCenterId === "tuna") {
          const tunaAdmin = (await User.findOne({
        ...
        if (staff.centerId !== targetCenterId) {
          logger.info(
            `>>> Migrated staff user ${staff.email} centerId from '${staff.centerId}' to '${targetCenterId}'`,
          );
