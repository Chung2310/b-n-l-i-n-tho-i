...
async function enrichPartners(
  ...
  return partners.map((p) => {
    ...
    const totalReferredTuition = referredStudents.reduce(
      (sum, s) => sum + parseCurrency(s.fee),
      0,
    );
    ...
    const activeLevels =
      centerLevels.length > 0 ? centerLevels : DEFAULT_LEVELS;
    ...
    const totalPaid = (p.payoutHistory || []).reduce(
...
export class PartnerService {
  ...
  static async createPartner(
    ...
    logger.info(
      `[Partner] Creating partner name=${data.name}, phone=${data.phone} for ownerId=${ownerId}`,
    );
    ...
    if (existing) {
      throw new ConflictError(
        ...
        "Số điện thoại đã tồn tại cho đối tác của trung tâm.",
        { field: "phone" },
      );
    }
  ...
  static async bulkCreatePartners(
    ...
    const existingPhones = new Set(
      existingPartners.map((p) => normalizePhone(p.phone)),
    );
    ...
    for (let i = 0; i < partnersData.length; i++) {
      ...
      if (!name) {
        errors.push({
          ...
          name,
          phone,
          reason: "Tên đối tác không được để trống.",
        });
        skippedCount++;
        continue;
      }
      ...
      if (existingPhones.has(phone)) {
        errors.push({
          ...
          name,
          phone,
          reason: "Số điện thoại đối tác đã tồn tại trong trung tâm.",
        });
        skippedCount++;
        continue;
      }
    ...
    if (validPartners.length > 0) {
      ...
      logger.info(
        `[Partner] Bulk import complete: imported=${importedCount}, skipped=${skippedCount}`,
      );
    }
  ...
  static async getPartnerById(
    ...
    (EnrichedPartner & { referredStudents: ReferredStudentItem[] }) | null
  > {
    ...
    const students = await Student.find({ partnerId: id }).sort({
      createdAt: -1,
    });
    const studentsList = students.map((student) => {
      const commission =
        partner.commissionType === "fixed"
          ...
          : Math.round(
              (parseCurrency(student.fee) * partner.commissionValue) / 100,
            );
  ...
  static async updatePartner(
    ...
    const targetContext =
      context.actorRole === "superadmin"
        ? {
            ...context,
            tenantId: await resolveCustomFieldTenantForOwner(partner.ownerId),
          }
        : context;
    ...
    if (writeData.phone && writeData.phone !== partner.phone) {
      ...
      if (dup) {
        throw new ConflictError(
          "PARTNER_PHONE_ALREADY_EXISTS",
          "Số điện thoại đã tồn tại cho một đối tác khác.",
          { field: "phone" },
        );
      }
    }
  ...
  static async deletePartner(
    ...
    if (studentCount > 0) {
      throw new ConflictError(
        "PARTNER_HAS_REFERRED_STUDENTS",
        `Không thể xóa đối tác vì đã giới thiệu ${studentCount} học viên. Vui lòng vô hiệu hóa thay vì xóa.`,
        { studentCount },
      );
    }

    return await Partner.findOneAndDelete({
      _id: id,
      ...buildOwnerQuery(ownerId),
      ...buildBranchScopeQuery(branchId),
    });
  ...
  static async addPayout(
    ...
    logger.info(
      `[Partner] Adding payout for partner: id=${id}, amount=${payoutData.amount}`,
    );
  ...
  static async getCommissionLevels(
    ...
    logger.info(
      `[PartnerService] Fetching commission levels for ownerId: ${ownerId}`,
    );
    return await CommissionLevel.find({
      ...buildOwnerQuery(ownerId),
      ...buildBranchScopeQuery(branchId),
    }).sort({ minTuition: 1 });
  }
  ...
  static async createCommissionLevel(
    ...
    logger.info(
      `[PartnerService] Creating commission level for ownerId: ${ownerId}, name: ${data.name}`,
    );

    // Check unique name per ownerId
    const existing = await CommissionLevel.findOne({
      name: data.name,
      ownerId,
    });
    if (existing) {
      throw new ConflictError(
        "COMMISSION_LEVEL_ALREADY_EXISTS",
        "Cấp bậc hoa hồng đã tồn tại.",
        { field: "name" },
      );
    }
