...
export class ExamService {
  ...
  static async createExam(
    ...
    logger.info(
      `[Exam] Creating exam for ownerId=${ownerId}, data=${JSON.stringify(data)}`,
    );
    ...
    logger.info(
      `[Exam] Exam created successfully: id=${savedExam._id}, name=${savedExam.name}`,
    );
    return savedExam;
  }
  ...
  static async getExams(
    ...
    logger.info(
      `[Exam] Fetching exams for ownerId=${ownerId} with filters: ${JSON.stringify(filters)}`,
    );
    ...
    if (resolvedOwnerId !== "ALL") {
      query.ownerId = Array.isArray(resolvedOwnerId)
        ? { $in: resolvedOwnerId }
        : resolvedOwnerId;
    }
    ...
    logger.info(
      `[Exam] Fetched ${exams.length} exams (total=${total}) for ownerId=${ownerId}`,
    );
    return {
      exams,
      total,
  ...
  static async deleteExam(
    ...
    if (deletedExam) {
      ...
      logger.warn(
        `[Exam] Exam delete failed/not found: id=${id}, ownerId=${ownerId}`,
      );
    }
    return deletedExam;
  }
  ...
  static async assignStudents(
    ...
    logger.info(
      `[Exam] Assigning ${studentIds.length} students to examId=${examId}, ownerId=${ownerId}`,
    );
    ...
    if (!exam) {
      logger.warn(
        `[Exam] Assign students failed - Exam not found: id=${examId}, ownerId=${ownerId}`,
      );
    ...
    if (ownerId !== "ALL") {
      studentQuery.ownerId = Array.isArray(ownerId)
        ? { $in: ownerId }
        : ownerId;
    }
    ...
    const updateResult = await Student.updateMany(studentQuery, {
      $set: {
    ...
    logger.info(
      `[Exam] Assigned students updated database: matchedCount=${updateResult.matchedCount}, modifiedCount=${updateResult.modifiedCount}`,
    );
    ...
    await exam.save();
    logger.info(
      `[Exam] Updated studentCount for examId=${examId} to: ${exam.studentCount}`,
    );
  ...
  static async unassignStudent(
    ...
    logger.info(
      `[Exam] Unassigning studentId=${studentId} from examId=${examId}, ownerId=${ownerId}`,
    );
    ...
    if (!exam) {
      logger.warn(
        `[Exam] Unassign student failed - Exam not found: id=${examId}, ownerId=${ownerId}`,
      );
      throw new Error("Kỳ thi không tồn tại.");
    }
    ...
    if (!student) {
      logger.warn(
        `[Exam] Unassign student failed - Student not found or not assigned: studentId=${studentId}, examId=${examId}`,
      );
    ...
    logger.info(
      `[Exam] Unassigned student successfully. Updated exam stats: studentCount=${studentCount}, passCount=${passCount}, failCount=${failCount}`,
    );
    return { success: true };
  }
  ...
  static async updateStudentResult(
    ...
    logger.info(
      `[Exam] Updating student result: studentId=${studentId}, examId=${examId}, overallResult=${overallResult}`,
    );
    ...
    if (!exam) {
      logger.warn(
        `[Exam] Update student result failed - Exam not found: id=${examId}, ownerId=${ownerId}`,
      );
      throw new Error("Kỳ thi không tồn tại.");
    }
    ...
    if (!student) {
      logger.warn(
        `[Exam] Update student result failed - Student not found or not assigned: studentId=${studentId}, examId=${examId}`,
      );
      throw new Error("Học viên không thuộc kỳ thi này hoặc không tồn tại.");
    }
    ...
    logger.info(
      `[Exam] Updated student result successfully. Exam stats: passCount=${passCount}, failCount=${failCount}`,
    );
    return { success: true };
  }
  ...
  static async importResults(
    ...
    logger.info(
      `[Exam] Importing ${results.length} results for examId=${examId}, ownerId=${ownerId}, preview=${preview}`,
    );
    ...
    if (!exam) {
      logger.warn(
        `[Exam] Import results failed - Exam not found: id=${examId}, ownerId=${ownerId}`,
      );
      throw new Error("Kỳ thi không tồn tại.");
    }
    ...
    for (const item of results) {
      try {
        ...
        if (!student) {
          ...
          invalidList.push({
            phone,
            fullName: item.fullName || "Chưa rõ",
            reason,
          });
        ...
        const totalFee =
          parseInt(String(student.fee || "0").replace(/\D/g, ""), 10) || 0;
        ...
        const eligibleRank = isStudentEligibleForExamRank(
          businessType,
          exam.rank,
          student.rank,
        );
    ...
    logger.info(
      `[Exam] Finished importing results. Success: ${successCount}, Failed: ${failedCount}, studentCount=${studentCount}, passCount=${passCount}, failCount=${failCount}`,
    );
