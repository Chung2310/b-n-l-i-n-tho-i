...
export class ResourceCategoryService {
  static async getCategories(
    ...
    branchId?: string,
  ): Promise<IResourceCategory[]> {
    logger.info(
      `[ResourceCategory] Fetching categories for ownerId: ${ownerId}`,
    );
  ...
  static async createCategory(
    ...
    logger.info(
      `[ResourceCategory] Creating category "${trimmedName}" for ownerId: ${ownerId}`,
    );
  ...
  static async deleteCategory(
    ...
    id: string,
    branchId?: string,
  ): Promise<IResourceCategory | null> {
    ...
    if (resourcesInUse) {
      throw new Error(
        `Không thể xóa phân loại "${category.name}" vì đang có tài nguyên sử dụng.`,
      );
    }
