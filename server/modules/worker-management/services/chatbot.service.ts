...
export class ChatbotService {
  static async getResponse(
    ...
    if (!apiKey) {
      throw new Error(
        "Cấu hình khóa API (OPENROUTER_API_KEY) chưa được thiết lập trên Server.",
      );
    }
    ...
    const studentList = students
      .slice(0, limit)
      .map((s) => {
        const datProgress = s.progress?.dat
          ? `${s.progress.dat.kmDone}/${s.progress.dat.totalKm} km`
          : "0 km";
        const theoryProgress = s.progress?.theory?.completed
          ? "Lý thuyết Đạt"
          : "Lý thuyết Chưa đạt";
        const statusStr = Array.isArray(s.status)
          ? s.status.join(", ")
          : s.status;
        const drivingInfo = s.rank
          ? `, hạng ${s.rank}, DAT: ${datProgress}, ${theoryProgress}`
          : "";
...
- Nếu được hỏi về kiến thức chung của ngành đào tạo (quy chế học/thi sát hạch lái xe, chứng chỉ ngoại ngữ, chương trình kỹ năng...), hãy trả lời theo kiến thức chuyên môn của bạn về lĩnh vực tương ứng.`,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Lỗi kết nối dịch vụ OpenRouter AI (Mã lỗi: ${response.status} ${response.statusText}).`,
        );
      }
      ...
      if (!content) {
        throw new Error(
          "Không nhận được nội dung phản hồi hợp lệ từ mô hình AI.",
        );
      }
