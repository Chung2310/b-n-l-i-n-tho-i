...
export class OnlineCheckinError extends Error {
  constructor(
    public readonly reasonCode: OnlineCheckinReasonCode,
    message: string,
  ) {
    super(message);
...
export class StudentOnlineAttendanceService {
  ...
  static async createSessions(
    ...
    if (!batch) {
      throw new OnlineCheckinError(
        "batch_not_found",
        "Không tìm thấy lớp học hoặc bạn không có quyền.",
      );
    }

    const students = await Student.find({
      _id: { $in: batch.learnerIds },
      ownerId,
    });
...
<p>Mã có hiệu lực trong 5 phút và chỉ dùng được một lần.</p>`,
  ...
  static async checkin(
    ...
    if (
      ...
      throw new OnlineCheckinError(
        "code_expired",
        "Mã xác thực đã hết hạn. Vui lòng yêu cầu giáo viên gửi mã mới.",
      );
    }
