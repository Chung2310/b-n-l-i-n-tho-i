...
export class PaymentService {
  static async createPayment(
    ...
    logger.info(
      `[Payment] Creating payment: studentId=${data.studentId}, ownerId=${ownerId}, amount=${data.amount}`,
    );
    ...
    if (!student) {
      logger.warn(
        `[Payment] Create payment failed - Student ${data.studentId} not found for ownerId=${ownerId}`,
      );
    ...
    if (payAmount > remaining) {
      logger.warn(
        `[Payment] Create payment failed - Amount ${payAmount} exceeds remaining debt ${remaining} for student ${data.studentId}`,
      );
      throw new Error(
    ...
    logger.info(
      `[Payment] Giao dịch thanh toán đã tạo: id=${savedPayment._id}, studentId=${savedPayment.studentId}`,
    );
    ...
    if (student.installmentStatus && student.installmentStatus.length > 0) {
      ...
      const exactMatch = student.installmentStatus.find(
        (inst) =>
          inst.status !== "Đã thu" &&
          Math.abs(inst.amountDue - allocated) <= 1000,
      );
      ...
      if (exactMatch) {
        ...
        logger.info(
          `[Payment] Khớp chính xác đợt ${exactMatch.installmentNo} với số tiền ${payAmount}`,
        );
      } else {
    ...
    logger.info(
      `[Payment] Cập nhật thông tin học phí thành công cho học viên: id=${student._id}, đã đóng=${student.paidAmount}`,
    );
  ...
  static async getPayments(
    ...
    logger.info(
      `[Payment] Fetching payments for ownerId=${ownerId} with filters: ${JSON.stringify(filters)}`,
    );
    ...
    logger.info(
      `[Payment] Fetched ${payments.length} payments (total=${total}) for ownerId=${ownerId}`,
    );
    return {
      payments,
      total,
  ...
  static async deletePayment(
    ...
    if (ownerId !== "ALL") {
      paymentQuery.ownerId = Array.isArray(ownerId)
        ? { $in: ownerId }
        : ownerId;
    }
    ...
    if (!payment) {
      logger.warn(
        `[Payment] Delete payment failed - Payment not found: id=${id}, ownerId=${ownerId}`,
      );
    ...
    if (student) {
      student.paidAmount = Math.max(
        0,
        (student.paidAmount || 0) - payment.amount,
      );
      if (student.paymentHistory) {
        student.paymentHistory = (
          student.paymentHistory as PaymentHistoryEntry[]
        ).filter((p) => p.id !== payment._id.toString());
      }
      await Student.updateOne(
        studentQuery,
        {
      ...
      logger.info(
        `[Payment] Cập nhật hoàn tiền học phí thành công cho học viên: id=${student._id}`,
      );
    }
