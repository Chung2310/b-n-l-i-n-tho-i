...
export type CustomFieldValueRepository = {
  find(filter: {
    tenantId: string;
    moduleKey: ModuleKey;
  }): CustomFieldValueDefinition[] | PromiseLike<CustomFieldValueDefinition[]>;
};
...
function enforceTextConstraints(
  ...
  if (typeof validation.pattern === "string") {
    ...
    if (!pattern.test(value))
      fail(definition.label, "giá trị không đúng định dạng yêu cầu.");
  }
}
...
function numericValue(
  ...
  if (
    typeof validation.decimals === "number" &&
    decimalPlaces(value) > validation.decimals
  ) {
    fail(
      definition.label,
      `giá trị chỉ được có tối đa ${validation.decimals} chữ số thập phân.`,
    );
  }
  return value;
}
...
function isValidTime(value: string): boolean {
  return /^(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d(?:\.\d{1,9})?)?(?:Z|[+-](?:[01]\d|2[0-3]):[0-5]\d)?$/.test(
    value,
  );
}
...
function dateValue(
  ...
  if ((definition.type as string) === "date") {
    if (!isValidDate(normalized))
      fail(definition.label, "giá trị ngày không hợp lệ.");
    if (
      typeof validation.minDate === "string" &&
      normalized < validation.minDate
    ) {
      fail(definition.label, `ngày không được trước ${validation.minDate}.`);
    }
    if (
      typeof validation.maxDate === "string" &&
      normalized > validation.maxDate
    ) {
    ...
    if (!isValidTime(normalized))
      fail(definition.label, "giá trị giờ không hợp lệ.");
    const comparable = normalized.slice(0, 5);
    if (
      typeof validation.minTime === "string" &&
      comparable < validation.minTime
    ) {
      fail(definition.label, `giờ không được trước ${validation.minTime}.`);
    }
    if (
      typeof validation.maxTime === "string" &&
      comparable > validation.maxTime
    ) {
      fail(definition.label, `giờ không được sau ${validation.maxTime}.`);
    }
  } else {
    if (!isValidDateTime(normalized))
      fail(definition.label, "giá trị ngày giờ không hợp lệ.");
    const timestamp = Date.parse(normalized);
    if (
      typeof validation.minDateTime === "string" &&
      timestamp < Date.parse(validation.minDateTime)
    ) {
      fail(
        definition.label,
        `ngày giờ không được trước ${validation.minDateTime}.`,
      );
    }
    if (
      typeof validation.maxDateTime === "string" &&
      timestamp > Date.parse(validation.maxDateTime)
    ) {
      fail(
        definition.label,
        `ngày giờ không được sau ${validation.maxDateTime}.`,
      );
    }
  }
  return normalized;
...
function mimeMatches(allowed: string, actual: string): boolean {
  ...
  if (allowed.endsWith("/*") && actual.startsWith(allowed.slice(0, -1)))
    return true;
  if (
    allowed ===
  ...
  if (
    ...
    actual === "application/x-ole-storage"
  )
    return true;
  return false;
}
...
function fileValue(
  ...
  if (
    ...
    Object.keys(value).some(
      (key) => UNSAFE_KEYS.has(key) || !FILE_KEYS.has(key),
    ) ||
  ...
  if (typeof value.reference === "string")
    normalized.reference = value.reference.trim();
  ...
  if (
    ...
    (!normalized.mimeType ||
      !allowedMimeTypes.some((mime) => mimeMatches(mime, normalized.mimeType!)))
  ) {
  ...
  if (
    ...
    fail(
      definition.label,
      `dung lượng tệp không được vượt quá ${validation.maxSizeMb} MB.`,
    );
  }
  return normalized;
}
  ...
  switch (definition.type as string) {
    ...
    case "email": {
      ...
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalized))
        fail(definition.label, "địa chỉ email không hợp lệ.");
      enforceTextConstraints(normalized, definition);
      return normalized;
    }
    case "phone": {
      ...
      if (!/^\+?[0-9][0-9 ()-]*$/.test(normalized))
        fail(definition.label, "số điện thoại không hợp lệ.");
      enforceTextConstraints(normalized, definition);
      return normalized;
    }
    case "url": {
    ...
    case "switch":
      if (typeof value !== "boolean")
        fail(definition.label, "giá trị phải là boolean.");
      return value;
    case "file":
    case "image":
    ...
    case "multiImage": {
      if (!Array.isArray(value))
        fail(definition.label, "giá trị phải là danh sách ảnh.");
...
export function createCustomFieldValueValidator(
  repository: CustomFieldValueRepository,
) {
  return async function validateCustomFieldValues(
    ...
    if (
      input.existingValues !== undefined &&
      !isPlainObject(input.existingValues)
    ) {
      throw new Error(
        "Giá trị trường tùy chỉnh hiện có phải là một đối tượng thuần.",
      );
    }
    ...
    for (const definition of definitions) {
      if (definition.isArchived || !definition.isVisible) {
        if (
          ...
          result[definition.key] = input.existingValues[
            definition.key
          ] as CustomFieldValue;
        }
        continue;
      ...
      if (
        !supplied &&
        input.mode === "create" &&
        !isMissing(definition.defaultValue)
      ) {
        value = definition.defaultValue;
      }
      if (isMissing(value)) {
        if (definition.isRequired)
          fail(definition.label, "trường này là bắt buộc.");
        continue;
      }
...
const defaultRepository =
  CustomFieldDefinition as unknown as CustomFieldValueRepository;

export const validateCustomFieldValues =
  createCustomFieldValueValidator(defaultRepository);
