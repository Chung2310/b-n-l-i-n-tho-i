...
export class QrCheckinError extends Error {
  constructor(
    public readonly reasonCode: QrCheckinReasonCode,
    message: string,
  ) {
    super(message);
...
export class QRAttendanceService {
  ...
  static async createSession(
    ...
    logger.info(
      `[QR-Attendance] Creating session for batchId=${batchId}, date=${date}, ownerId=${ownerId}`,
    );
    ...
    for (const [id, session] of sessions.entries()) {
      if (
        ...
        logger.info(
          `[QR-Attendance] Closing previous active session ${id} for batchId=${batchId}`,
        );
        session.closed = true;
        sessions.delete(id);
      }
    ...
    const { token, expiresAt: tokenExpiresAt } = generateToken(
      sessionId,
      batchId,
    );
  ...
  static async checkin(
    ...
    try {
      ...
      throw new QrCheckinError(
        "session_invalid",
        "Mã QR không hợp lệ hoặc đã hết hạn. Vui lòng quét lại.",
      );
    }
    ...
    if (!session || session.closed || Date.now() > session.expiresAt) {
      throw new QrCheckinError(
        "session_invalid",
        "Phiên điểm danh đã kết thúc hoặc không tồn tại.",
      );
    }
    ...
    if (session.usedNonces.has(nonce)) {
      throw new QrCheckinError(
        "replay",
        "Mã QR này đã được quét và sử dụng rồi.",
      );
    }
    ...
    if (fingerprint) {
      ...
      if (registeredPhone && registeredPhone !== cleanPhone) {
        throw new QrCheckinError(
          "device_conflict",
          "Thiết bị này đã được sử dụng để điểm danh cho học viên khác.",
        );
      }
    }
    ...
    if (!student) {
      throw new QrCheckinError(
        "student_not_found",
        "Số điện thoại không có trong hệ thống hoặc không đúng cơ sở.",
      );
    }
    ...
    if (!batch.learnerIds.includes(student._id.toString())) {
      throw new QrCheckinError(
        "not_in_batch",
        "Học viên không nằm trong danh sách lớp học này.",
      );
    }
    ...
    if (session.checkins.has(student._id.toString())) {
      throw new QrCheckinError(
        "already_checked_in",
        "Bạn đã điểm danh thành công trước đó rồi.",
      );
    }
    ...
    if (
      ...
      batch.geoLocation?.longitude != null
    ) {
      if (latitude == null || longitude == null) {
        ...
        throw new QrCheckinError(
          "missing_image",
          "Không lấy được vị trí GPS của bạn. Vui lòng cấp quyền định vị và thử lại.",
        );
      }
      distanceMeters = calculateHaversineDistanceMeters(
        latitude,
        longitude,
        batch.geoLocation.latitude,
        batch.geoLocation.longitude,
      );
    ...
    if (
      !student.faceEnrollment?.registered ||
      ...
      throw new QrCheckinError(
        "not_registered",
        "Học viên chưa đăng ký khuôn mặt. Vui lòng liên hệ giáo viên/admin.",
      );
    }
    ...
    if (!image || image.length === 0) {
      ...
      throw new QrCheckinError(
        "missing_image",
        "Vui lòng chụp ảnh khuôn mặt để điểm danh.",
      );
    }

    const gateResult = await verifyStudentAttendanceFace(
      {
        insightFaceUserId: student.faceEnrollment.insightFaceUserId,
        image,
        mimeType,
      },
    ...
    if (!gateResult.accepted) {
      ...
      throw new QrCheckinError(
        ...
        "Xác thực khuôn mặt không thành công. Vui lòng thử lại.",
      );
    }
    ...
    logger.info(
      `[QR-Attendance] Student checked in successfully: name=${student.fullName}, session=${sid}`,
    );
  ...
  static async closeSession(sessionId: string): Promise<void> {
    ...
    const records = batch.learnerIds.map((studentId) => {
      ...
      return {
        studentId,
        status: (checkedIn ? "present" : "absent") as
          | "present"
          | "absent"
          | "excused",
      };
    ...
    sessions.delete(sessionId);
    logger.info(
      `[QR-Attendance] Saved & Closed session ${sessionId} for batchId=${session.batchId}`,
    );
  }
}
