...
export class CourseCategoryService {
  static async getCategories(
    ...
    logger.info(
      `[CourseCategory] Fetching categories for ownerId: ${ownerId}, ownerFilter: ${filters.ownerFilter}`,
    );
    ...
    if (resolvedOwnerId !== "ALL") {
      query = {
        ownerId: Array.isArray(resolvedOwnerId)
          ? { $in: resolvedOwnerId }
          : resolvedOwnerId,
      };
    }
  ...
  static async createCategory(
    ...
    logger.info(
      `[CourseCategory] Creating category "${trimmedName}" for ownerId: ${ownerId}`,
    );
  ...
  static async deleteCategory(
    ...
    if (coursesInUse) {
      throw new Error(
        `Không thể xóa phân loại "${category.name}" vì đang có khóa học sử dụng.`,
      );
    }
