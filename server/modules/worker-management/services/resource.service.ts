...
export class ResourceService {
  ...
  static async createResource(
    ...
    logger.info(
      `[Resource] Creating resource for ownerId=${ownerId}, name=${data.name}`,
    );
    ...
    logger.info(
      `[Resource] Resource created: id=${saved._id}, name=${saved.name}`,
    );
    return saved;
  }
  ...
  static async bookResource(
    ...
    if (
      booking.date === today &&
      booking.startTime <= hhmm &&
      booking.endTime > hhmm
    ) {
    ...
    const saved = await resource.save();
    logger.info(
      `[Resource] Booking added: resourceId=${id}, date=${booking.date} ${booking.startTime}-${booking.endTime}`,
    );
    return saved;
  }
  ...
  static async cancelBooking(
    ...
    const saved = await resource.save();
    logger.info(
      `[Resource] Booking cancelled: resourceId=${id}, bookingId=${bookingId}`,
    );
    return saved;
  }
