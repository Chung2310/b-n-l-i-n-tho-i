...
export type UpdateFieldInput = Omit<Partial<CreateFieldInput>, "moduleKey"> & {
  order?: number;
};

type QueryableModel = {
  find(filter: Record<string, unknown>): {
    sort(sort: Record<string, 1 | -1>): Promise<IFieldDefinition[]>;
  };
  findOne(
    filter: Record<string, unknown>,
  ): PromiseLike<IFieldDefinition | null> & {
    sort(sort: Record<string, 1 | -1>): Promise<IFieldDefinition | null>;
  };
...
type UserModel = {
  find(filter: Record<string, unknown>): {
    select(fields: string): Promise<Array<{ _id: unknown }>>;
  };
};
type EntityModel = {
...
function validateKey(label: string): string {
  ...
  if (RESERVED_KEYS.has(key) || RESERVED_KEYS.has(raw))
    throw new Error("Tên trường này dành riêng cho hệ thống.");
  return key;
}
...
function normalizeOptions(
  ...
  if (
    !normalized.length ||
    normalized.some((option) => !option.label || !option.value)
  ) {
...
const VALIDATION_KEYS: Record<DynamicFieldType, Set<string>> &
  ...
  email: new Set(["minLength", "maxLength", "pattern"]),
  phone: new Set(["minLength", "maxLength", "pattern"]),
  url: new Set(["minLength", "maxLength", "pattern"]),
  number: new Set(["min", "max", "decimals"]),
  percent: new Set(["min", "max", "decimals"]),
  currency: new Set(["min", "max", "decimals"]),
  date: new Set(["minDate", "maxDate"]),
  time: new Set(["minTime", "maxTime"]),
  dateTime: new Set(["minDateTime", "maxDateTime"]),
  singleSelect: new Set(),
  multiSelect: new Set(),
  checkbox: new Set(),
  switch: new Set(),
  file: new Set(["maxSizeMb", "allowedMimeTypes"]),
  image: new Set(["maxSizeMb", "allowedMimeTypes"]),
  multiImage: new Set(["maxSizeMb", "maxFiles", "allowedMimeTypes"]),
};

function validateMergedConfiguration(
  ...
  const validation = (input.validation ??
    (input.type && input.type !== field.type ? {} : field.validation) ??
    {}) as Record<string, unknown>;
  const invalidKey = Object.keys(validation).find(
    (key) => !VALIDATION_KEYS[targetType].has(key),
  );
  if (invalidKey)
    throw new Error(
      `Cấu hình kiểm tra "${invalidKey}" không hợp lệ cho loại trường ${targetType}.`,
    );
...
export class CustomFieldService {
  constructor(
    private readonly dependencies: CustomFieldServiceDependencies = defaultDependencies,
  ) {}
  ...
  async create(
    ...
    const existing = await this.dependencies.fieldDefinitions
      .find({ tenantId: context.tenantId, moduleKey: input.moduleKey })
      .sort({ order: -1 });
    ...
    const order = existing.length
      ? Math.max(...existing.map((field) => field.order ?? 0)) + 1
      : 1;
    const document: Record<string, unknown> = {
      ...
      isRequired:
        input.isVisible === false ? false : (input.isRequired ?? false),
      isArchived: false,
      order,
      createdBy: context.actorId,
      updatedBy: context.actorId,
    };
    for (const key of ["placeholder", "defaultValue", "validation"] as const)
      if (input[key] !== undefined) document[key] = input[key];
  ...
  async update(
    ...
    const field = await this.dependencies.fieldDefinitions.findOne({
      _id: id,
      tenantId: context.tenantId,
      moduleKey,
    });
    ...
    if (
      ...
      (await this.hasStoredValues(context.tenantId, field.moduleKey, field.key))
    ) {
      throw new Error(
        "Không thể thay đổi loại trường khi đã có dữ liệu lưu trữ.",
      );
    }
    ...
    const operation: Record<string, unknown> = {
      $set: { ...update, updatedBy: context.actorId },
    };
  ...
  async delete(
    ...
    const deleted = await this.dependencies.fieldDefinitions.findOneAndDelete({
      _id: id,
      tenantId: context.tenantId,
      moduleKey,
    });
  ...
  async hasStoredValues(
    ...
    const users = await this.dependencies.users
      .find({ companyCode: tenantId })
      .select("_id");
