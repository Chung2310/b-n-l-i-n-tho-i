...
function authenticateStudentRequest(
  ...
  requireAuth(req as unknown as AuthenticatedRequest, res, () => {
    ...
    if (options.unassignedAdminOnly) {
      if (req.user.role !== "admin") {
        return res
          ...
          .json({
            success: false,
            error: "Chỉ quản trị viên được xử lý dữ liệu chưa gán chi nhánh.",
          });
      }
      return next();
    }
    ...
    if (
      ["admin", "manager", "branch_owner"].includes(req.user.role) &&
      !req.user.branchId
    ) {
      try {
...
export function adminUnassignedAuthMiddleware(
  req: AuthRequest,
  res: Response,
  next: NextFunction,
) {
  return authenticateStudentRequest(req, res, next, {
    unassignedAdminOnly: true,
  });
}
export function requireRoles(...roles: StudentManagementRole[]) {
  return (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    if (!roles.includes(req.user.role)) {
      return res
        .status(403)
        .json({ success: false, error: "Bạn không có quyền truy cập." });
    }
