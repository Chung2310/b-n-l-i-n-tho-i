...
export class WebhookController {
  static async handlePaymentWebhook(
    ...
    try {
      ...
      if (!expectedSecret) {
        logger.error(
          "[WebhookController] WEBHOOK_SECRET is not configured on server.",
        );
        return res.status(500).json({
          success: false,
          error:
            "Cấu hình hệ thống lỗi: Chưa thiết lập WEBHOOK_SECRET trên máy chủ.",
        });
      }

      if (secret !== expectedSecret) {
        logger.warn(
          `[WebhookController] Unauthorized attempt. Received secret: "${secret}"`,
        );
      ...
      if (payload && Array.isArray(payload.data)) {
        logger.info(
          `[WebhookController] Casso format detected. Processing ${payload.data.length} items.`,
        );
        for (const transaction of payload.data) {
          const result =
            await WebhookService.processIncomingTransaction(transaction);
          results.push(result);
        }
      } else {
        logger.info(
          "[WebhookController] SePay or flat format detected. Processing single item.",
        );
      ...
      logger.error(
        `[WebhookController] Error processing webhook: ${error instanceof Error ? error.message : error}`,
      );
      res.status(500).json({
        success: false,
        error:
          error instanceof Error
            ? error.message
            : "Đã xảy ra lỗi máy chủ khi xử lý giao dịch.",
      });
    }
  }
}
