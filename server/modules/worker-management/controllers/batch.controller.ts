...
export class BatchController {
  ...
  static async getDetail(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (!batch) {
        return res
          .status(404)
          .json({ success: false, error: "Không tìm thấy lớp học." });
      }
    ...
    try {
      ...
      if (!batch) {
        return res
          ...
          .json({
            success: false,
            error: "Không tìm thấy lớp học để cập nhật.",
          });
      }
      res.json({ success: true, data: batch });
    } catch (error: unknown) {
  ...
  static async delete(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const ownerId = await getAllowedOwnerIds(req.user!);
      const batch = await BatchService.deleteBatch(
        ownerId,
        req.params.id,
        req.user!.branchId,
      );
      if (!batch) {
        return res
          .status(404)
          .json({ success: false, error: "Không tìm thấy lớp học để xóa." });
      }
      res.json({ success: true, data: batch });
    } catch (error: unknown) {
      next(error);
  ...
  static async addLearner(req: AuthRequest, res: Response) {
    try {
      ...
      const batch = await BatchService.addLearner(
        ownerId,
        req.params.id,
        req.body.studentId,
        businessType,
  ...
  static async removeLearner(req: AuthRequest, res: Response) {
    try {
      const ownerId = await getAllowedOwnerIds(req.user!);
      const batch = await BatchService.removeLearner(
        ownerId,
        req.params.id,
  ...
  static async saveAttendance(req: AuthRequest, res: Response) {
    try {
      ...
      if (!date) {
        return res
          .status(400)
          .json({ success: false, error: "Vui lòng truyền ngày điểm danh." });
      }
      const batch = await BatchService.saveAttendanceSession(
        ownerId,
        req.params.id,
        date,
        records,
    ...
    try {
      ...
      if (!date) {
        return res
          ...
          .json({
            success: false,
            error: "Vui lòng cung cấp ngày cần xóa điểm danh.",
          });
      }
      const batch = await BatchService.deleteAttendanceSessionByDate(
        ownerId,
        req.params.id,
        String(date),
        req.user!.branchId,
