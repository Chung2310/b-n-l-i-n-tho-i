...
export class ExamController {
    ...
    try {
      ...
      if (!exam) {
        return res
          ...
          .json({
            success: false,
            error: "Không tìm thấy kỳ thi để cập nhật.",
          });
      }
      res.json({ success: true, data: exam });
    } catch (error: unknown) {
  ...
  static async delete(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (!exam) {
        return res
          .status(404)
          .json({ success: false, error: "Không tìm thấy kỳ thi để xóa." });
      }
      res.json({ success: true, data: exam });
    } catch (error: unknown) {
      next(error);
  ...
  static async assign(req: AuthRequest, res: Response) {
    try {
      ...
      if (idsToAssign.length === 0) {
        return res
          .status(400)
          .json({ success: false, error: "Thiếu ID học viên." });
      }

      await ExamService.assignStudents(
        ownerId,
        examId,
        idsToAssign,
        req.user!.branchId,
      ...
      res.json({
        success: true,
        message: "Đã thêm học viên vào kỳ thi thành công.",
      });
    } catch (error: unknown) {
      const msg =
        error instanceof Error ? error.message : "Lỗi không xác định.";
  ...
  static async unassign(req: AuthRequest, res: Response) {
    try {
      ...
      await ExamService.unassignStudent(
        ownerId,
        examId,
        studentId,
        req.user!.branchId,
      ...
      res.json({
        success: true,
        message: "Đã xóa học viên khỏi kỳ thi thành công.",
      });
    } catch (error: unknown) {
      const msg =
        error instanceof Error ? error.message : "Lỗi không xác định.";
  ...
  static async updateStudentResult(req: AuthRequest, res: Response) {
    try {
      ...
      if (!overallResult) {
        return res
          .status(400)
          .json({ success: false, error: "Thiếu kết quả thi." });
      }

      await ExamService.updateStudentResult(
        ownerId,
        examId,
        studentId,
        overallResult,
      ...
      res.json({
        success: true,
        message: "Cập nhật kết quả thi học viên thành công.",
      });
    } catch (error: unknown) {
      const msg =
        error instanceof Error ? error.message : "Lỗi không xác định.";
  ...
  static async importResults(req: AuthRequest, res: Response) {
    try {
      ...
      if (!results || !Array.isArray(results)) {
        return res
          .status(400)
          .json({ success: false, error: "Dữ liệu kết quả không hợp lệ." });
      }

      const outcome = await ExamService.importResults(
        ownerId,
        examId,
        results,
        !!preview,
      ...
      res.json({
        success: true,
        message: preview
          ? "Xem trước kết quả nhập thành công."
          : "Nhập kết quả thi hàng loạt thành công.",
        ...outcome,
      });
    } catch (error: unknown) {
      const msg =
