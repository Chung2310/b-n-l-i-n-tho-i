    ...
    try {
      ...
      if (!student) {
        return res
          ...
          .json({
            success: false,
            error: "Không tìm thấy học viên hoặc bạn không có quyền.",
          });
      }
  ...
  static async register(req: UploadRequest, res: Response, next: NextFunction) {
    try {
      if (!req.file) {
        return res
          .status(400)
          .json({ success: false, reasonCode: "invalid_image" });
      }
      const student = await findAllowedStudent(req);
      if (!student) {
        return res
      ...
      if (error instanceof InsightFaceBusinessError) {
        return res
          .status(400)
          .json({ success: false, reasonCode: error.reasonCode });
      }
      if (error instanceof InsightFaceUnavailableError) {
        return res
          .status(503)
          .json({ success: false, reasonCode: "model_unavailable" });
      }
      next(error);
    }
  }
