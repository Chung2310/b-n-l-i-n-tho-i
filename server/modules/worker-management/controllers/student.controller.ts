...
export class StudentController {
    ...
    try {
      ...
      if (req.user!.role === "superadmin") {
        ...
        if (!companyCode || typeof companyCode !== "string") {
          return res
            .status(400)
            .json({ success: false, error: "Vui long chon cong ty." });
        }
        ownerId = await resolveCreateOwnerId(req.user!, companyCode);
        centerOwnerIds = await getCenterOwnerIds({
          uid: companyCode,
          role: "admin",
          centerId: companyCode,
          companyCode,
        });
      } else {
        ...
        { ...req.body, branchId: req.user!.branchId },
        {
          tenantId:
            req.user!.role === "superadmin"
              ? await resolveCustomFieldTenantForOwner(ownerId)
              : req.user!.companyCode || req.user!.centerId,
          moduleKey: "students",
  ...
  static async getUnassignedList(
    ...
    try {
      const ownerId = await getAllowedOwnerIds({
        ...req.user!,
        branchId: undefined,
      });
    ...
    try {
      ...
      if (!student) {
        return res
          ...
          .json({
            success: false,
            error: "Không tìm thấy dữ liệu chưa gán hoặc chi nhánh hợp lệ.",
          });
      }
  ...
  static async getDetail(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (!student) {
        return res
          .status(404)
          .json({ success: false, error: "Khong tim thay hoc vien." });
      }
      res.json({ success: true, data: student });
    } catch (error: unknown) {
      next(error);
    ...
    try {
      ...
      if (!student) {
        return res
          ...
          .json({
            success: false,
            error: "Khong tim thay hoc vien de cap nhat.",
          });
      }
      res.json({ success: true, data: student });
    } catch (error: unknown) {
  ...
  static async delete(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (!student) {
        return res
          .status(404)
          .json({ success: false, error: "Khong tim thay hoc vien de xoa." });
      }
      res.json({ success: true, data: student });
    } catch (error: unknown) {
      next(error);
    ...
    try {
      ...
      if (!Array.isArray(ids) || ids.length === 0) {
        return res
          ...
          .json({
            success: false,
            error: "Vui long chon it nhat mot hoc vien de xoa.",
          });
      }
      const deletedCount = await StudentService.bulkDeleteStudents(
        ownerId,
        ids,
        req.user!.branchId,
      );
      res.json({
        success: true,
        message: `Da xoa thanh cong ${deletedCount} hoc vien.`,
        deletedCount,
      });
    } catch (error: unknown) {
      next(error);
  ...
  static async bulkCreate(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (req.user!.role === "superadmin") {
        ...
        ownerId = await getCenterOwnerIds({
          uid: companyCode,
          role: "admin",
          centerId: companyCode,
          companyCode,
      ...
      if (!Array.isArray(students)) {
        return res
          .status(400)
          .json({ success: false, error: "Du lieu hoc vien khong hop le." });
      }
    ...
    try {
      ...
      if (!teacher || teacher.isActive === false) {
        return res
          ...
          .json({
            success: false,
            error: "Giao vien khong hop le hoac da bi khoa.",
          });
      }
  ...
  static async markInstallmentPaid(req: AuthRequest, res: Response) {
    try {
      ...
      if (isNaN(installmentNo) || installmentNo < 1) {
        return res
          .status(400)
          .json({ success: false, error: "So dot khong hop le." });
      }

      const result = await StudentService.markInstallmentPaid(
        ownerId,
        id,
        installmentNo,
        req.user!.branchId,
      ...
      res.json({
        success: true,
        message: `Da danh dau da thu dot ${installmentNo}.`,
      });
    } catch (error: unknown) {
      const msg =
        error instanceof Error ? error.message : "Loi khong xac dinh.";
  ...
  static async publicLookup(req: Request, res: Response) {
    try {
      ...
      if (!idCard || typeof idCard !== "string") {
        return res
          .status(400)
          .json({ success: false, error: "Vui long nhap so CCCD." });
      }
      ...
      if (!student) {
        return res
          ...
          .json({
            success: false,
            error: "Khong tim thay thong tin hoc vien.",
          });
      }
