...
export class AssignmentController {
  ...
  static async getList(req: AuthRequest, res: Response) {
    try {
      ...
      if (!batchId) {
        return res
          .status(400)
          .json({ success: false, error: "Thiếu mã lớp học batchId." });
      }
      const ownerScope = await getAllowedOwnerIds(req.user!);
      ...
      const msg =
        error instanceof Error ? error.message : "Lỗi lấy danh sách bài tập.";
      res.status(400).json({ success: false, error: msg });
    }
  }
  ...
  static async getSubmissions(req: AuthRequest, res: Response) {
    try {
      ...
      const msg =
        error instanceof Error ? error.message : "Lỗi lấy danh sách bài nộp.";
      res.status(400).json({ success: false, error: msg });
    }
  }
  ...
  static async grade(req: AuthRequest, res: Response) {
    try {
      ...
      const msg =
        error instanceof Error ? error.message : "Lỗi chấm điểm bài nộp.";
      res.status(400).json({ success: false, error: msg });
    }
  }
  ...
  static async getPublicDetail(req: AuthRequest, res: Response) {
    try {
      ...
      if (!token) {
        return res
          .status(400)
          .json({ success: false, error: "Thiếu liên kết mã hóa bài tập." });
      }
      const decoded = await AssignmentService.verifySubmissionToken(token);
      const { assignment, student, submission } =
        await AssignmentService.getPublicContext(decoded);
      ...
      const msg =
        error instanceof Error
          ? error.message
          : "Liên kết nộp bài tập không hợp lệ.";
      res.status(400).json({ success: false, error: msg });
    }
  }
  ...
  static async uploadProofFile(req: AuthRequest, res: Response) {
    try {
      ...
      const decoded = await AssignmentService.verifySubmissionToken(token);
      const url = await AssignmentService.uploadProofFile(
        decoded,
        req.body.file,
      );
      ...
      const msg =
        error instanceof Error ? error.message : "Lỗi tải tệp minh chứng lên.";
      res.status(400).json({ success: false, error: msg });
    }
  }
  ...
  static async submitProof(req: AuthRequest, res: Response) {
    try {
      ...
      const msg =
        error instanceof Error ? error.message : "Lỗi nộp minh chứng bài tập.";
      res.status(400).json({ success: false, error: msg });
    }
  }
  ...
  static async cancelSubmission(req: AuthRequest, res: Response) {
    try {
      ...
      const msg =
        error instanceof Error ? error.message : "Lỗi hủy nộp minh chứng.";
      res.status(400).json({ success: false, error: msg });
    }
  }
}
