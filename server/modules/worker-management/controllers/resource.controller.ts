...
export class ResourceController {
  ...
  static async getDetail(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (!resource) {
        return res
          .status(404)
          .json({ success: false, error: "Không tìm thấy tài nguyên." });
      }
    ...
    try {
      ...
      if (!resource) {
        return res
          ...
          .json({
            success: false,
            error: "Không tìm thấy tài nguyên để cập nhật.",
          });
      }
      res.json({ success: true, data: resource });
    } catch (error: unknown) {
  ...
  static async delete(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (!resource) {
        return res
          .status(404)
          .json({ success: false, error: "Không tìm thấy tài nguyên để xóa." });
      }
      res.json({ success: true, data: resource });
    } catch (error: unknown) {
      next(error);
  ...
  static async book(req: AuthRequest, res: Response) {
    try {
      const ownerId = await getAllowedOwnerIds(req.user!);
      const resource = await ResourceService.bookResource(
        ownerId,
        req.params.id,
        req.body,
        req.user!.branchId,
  ...
  static async cancelBooking(req: AuthRequest, res: Response) {
    try {
      const ownerId = await getAllowedOwnerIds(req.user!);
      const resource = await ResourceService.cancelBooking(
        ownerId,
        req.params.id,
