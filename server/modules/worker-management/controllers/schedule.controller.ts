  ...
  static async getEvents(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (resolvedOwnerId !== "ALL") {
        examQuery.ownerId = Array.isArray(resolvedOwnerId)
          ? { $in: resolvedOwnerId }
          : resolvedOwnerId;
      }
      ...
      const bookings = await ResourceService.getBookingsInRange(
        resolvedOwnerId,
        from,
        to,
        req.user!.branchId,
      ...
      const classes = await BatchService.getClassEventsInRange(
        resolvedOwnerId,
        from,
        to,
        req.user!.branchId,
