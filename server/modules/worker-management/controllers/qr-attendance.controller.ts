...
import {
  QRAttendanceService,
  QrCheckinError,
} from "../services/qr-attendance.service";
...
export class QRAttendanceController {
    ...
    try {
      ...
      if (!batchId || !date) {
        return res
          ...
          .json({
            success: false,
            error: "Vui lòng cung cấp batchId và date.",
          });
      }
      ...
      if (allowedOwners !== "ALL") {
        query.ownerId = Array.isArray(allowedOwners)
          ? { $in: allowedOwners }
          : allowedOwners;
      }
      ...
      if (!batch) {
        return res
          ...
          .json({
            success: false,
            error: "Không tìm thấy lớp học hoặc bạn không có quyền.",
          });
      }
  ...
  static getSessionInfo(req: Request, res: Response) {
    try {
      ...
      if (!token) {
        return res
          .status(400)
          .json({ success: false, error: "Vui lòng cung cấp mã QR." });
      }
      ...
      res
        ...
        .json({
          success: false,
          error: error.message || "Không thể lấy thông tin phiên.",
        });
    }
  }
  ...
  static async getToken(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (!sessionId) {
        return res
          .status(400)
          .json({ success: false, error: "Thiếu sessionId." });
      }
  ...
  static async getStatus(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      res
        ...
        .json({
          success: false,
          error: error.message || "Không thể lấy trạng thái.",
        });
    }
  }
  ...
  static async closeSession(
    ...
    try {
      ...
      res.json({
        success: true,
        message: "Đã đóng phiên và lưu điểm danh thành công.",
      });
    } catch (error: any) {
      res
        .status(400)
        .json({
          success: false,
          error: error.message || "Không thể đóng phiên.",
        });
    }
  }
    ...
    try {
      ...
      if (!token || !phone) {
        return res
          ...
          .json({
            success: false,
            error: "Vui lòng cung cấp mã QR và số điện thoại.",
          });
      }
      if (!req.file) {
        return res
          ...
          .json({
            success: false,
            error: "Vui lòng chụp ảnh khuôn mặt để điểm danh.",
      ...
      res
        ...
        .json({
          success: false,
          error: error.message || "Điểm danh không thành công.",
        });
    }
  }
}
