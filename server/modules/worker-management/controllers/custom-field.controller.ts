...
} from "../utils/custom-field.util";
import { CustomFieldUploadService } from "../services/custom-field-upload.service";

type CustomFieldServiceContract = Pick<
  CustomFieldService,
  "list" | "create" | "update" | "archive" | "restore" | "delete"
>;
...
export class CustomFieldController {
  ...
  static async list(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const tenantId = await resolveCustomFieldTenant(
        req.user!,
        req.query?.tenantId,
      );
      const includeArchived =
        (req.query.includeArchived as unknown) === true &&
        canManageCustomFields(req.user!.role);
      const data = await CustomFieldController.service.list(
        tenantId,
  ...
  static async create(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const data = await CustomFieldController.service.create(
        {
          tenantId: await resolveCustomFieldTenant(
            req.user!,
            req.query?.tenantId,
          ),
  ...
  static async upload(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      if (!req.file)
        return res
          .status(400)
          .json({ success: false, error: "Không tìm thấy tệp tải lên." });
      ...
      if (!sendExpectedError(error, res)) {
        const status =
          typeof (error as { status?: unknown })?.status === "number"
            ...
            : undefined;
        if (status)
          res.status(status).json({ success: false, error: messageFor(error) });
        else next(error);
      }
    }
  }
