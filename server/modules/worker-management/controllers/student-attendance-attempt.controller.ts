...
export class StudentAttendanceAttemptController {
  ...
  static async list(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      let attempts = await StudentAttendanceAttemptModel.find(query)
        .sort({ attemptedAt: -1 })
        .limit(500)
        .lean();
      ...
      if (date) {
        ...
        attempts = attempts.filter(
          (a) => a.attemptedAt >= dayStart && a.attemptedAt <= dayEnd,
        );
      }
