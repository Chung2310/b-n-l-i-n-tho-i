...
import {
  StudentOnlineAttendanceService,
  OnlineCheckinError,
} from "../services/student-online-attendance.service";
...
export class StudentOnlineAttendanceController {
  ...
  static async createSessions(
    ...
    try {
      ...
      const result = await StudentOnlineAttendanceService.createSessions(
        req.user!.uid,
        batch.ownerId,
        batchId,
        date,
    ...
    try {
      ...
      if (!phone || !code || !batchId || !date) {
        return res
          ...
          .json({
            success: false,
            error: "Vui lòng cung cấp đầy đủ thông tin điểm danh.",
          });
      }
      if (!req.file) {
        return res
