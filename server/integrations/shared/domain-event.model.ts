...
const DeliverySchema = new Schema({ consumer: { type: String, required: true }, status: { type: String, enum: ["pending", "processing", "done", "skipped", "failed"], default: "pending" }, attempts: { type: Number, default: 0 }, lastError: String, nextAttemptAt: Date, completedAt: Date, leaseUntil: Date, claimToken: String }, { _id: false });
...
DomainEventSchema.index({ "deliveries.status": 1, "deliveries.leaseUntil": 1 });
