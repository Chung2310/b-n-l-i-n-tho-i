...
export const recruitmentController = {
  ...
  getJobAttachment: asyncHandler(async (req, res) => ok(res, await attachments.getOwnerAttachment(scope(req), "job", req.params.jobId))),
  uploadJobAttachment: asyncHandler(async (req, res) => { if (!req.file) throw new Error("Attachment is required"); ok(res, await attachments.uploadOwnerAttachment(scope(req), actor(req), "job", req.params.jobId, req.file, req.body.version === undefined ? undefined : Number(req.body.version)), 201); }),
  getApplicantAttachment: asyncHandler(async (req, res) => ok(res, await attachments.getOwnerAttachment(scope(req), "applicant", req.params.applicantId))),
