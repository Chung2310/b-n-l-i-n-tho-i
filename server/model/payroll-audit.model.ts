...
const schema = new Schema<IPayrollAudit>({ companyCode: { type: String, required: true, index: true },
    branchId: { type: String, index: true }, periodKey: { type: String, required: true, index: true }, action: { type: String, enum: ["snapshot", "lock", "calculate", "approve", "close", "adjustment", "reset", "create_run", "sync_attendance", "lock_attendance"], required: true }, actorId: { type: String, required: true }, metadata: { type: Schema.Types.Mixed } }, { timestamps: { createdAt: true, updatedAt: false } });
