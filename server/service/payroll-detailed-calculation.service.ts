...
export function calculateDetailedPayroll(input: DetailedCalculationInput): {
  ...
  const lines = input.segments.map((segment) => {
    ...
    const vietnam = input.vietnam && calculateVietnamPayroll(input.vietnam.policy, {
      ...
      advances: (input.advances ?? 0) * ratio,
    ...
    const adjustmentEcho = {
      ...
      advances: Math.round((input.advances ?? 0) * ratio),
      adjustments: Math.round(input.adjustments * ratio),
    };
    return {
      employeeId: input.employeeId,
      calculation: vietnam
        ? { ...calculation, ...adjustmentEcho, commission: Math.round((input.commission ?? 0) * ratio), overtime: vietnam.overtime.total, gross: vietnam.income.totalIncome, deductions: vietnam.deductions.total, net: vietnam.netPay, monthlySalary: Math.round(segment.monthlySalary * ratio), workedMinutes: Math.round(input.workedMinutes * ratio) }
        : { ...calculation, ...adjustmentEcho, commission: Math.round((input.commission ?? 0) * ratio), deductions: Math.round((input.deductions + Math.max(-input.adjustments, 0)) * ratio), monthlySalary: Math.round(segment.monthlySalary * ratio), workedMinutes: Math.round(input.workedMinutes * ratio) },
      ...(vietnam ? { vietnam } : {}),
