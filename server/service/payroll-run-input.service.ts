...
import type { DetailedCalculationInput } from "./payroll-detailed-calculation.service";
...
const emptyAdjustmentTotals = (): AdjustmentTotals => ({ allowances: 0, bonuses: 0, deductions: 0, advances: 0, adjustments: 0 });
...
export async function buildRunCalculationInputs(
  ...
  const [users, contracts, profiles, dependents, policies, adjustments, formulas, periodInputs, customVariables] = await Promise.all([
    UserModel.find({ companyCode: scope.companyCode, _id: { $in: employeeIds } }).select("monthlySalary").lean(),
  ...
  return Promise.all(snapshot.employees.map(async (employee: any) => {
    ...
    const empCommission = commissions.get(employeeId) ?? 0;
    const employeeContext = {
      monthlySalary: sourceSalary,
    ...
    const resolved = await resolveDetailedPayrollInput(employeeId, {
      period,
      salaryTerms,
      policy: { id: String((policy as any)._id), version: Number((policy as any).version ?? 0) },
    });
    ...
    return {
      ...
      advances: employeeAdjustments.advances,
